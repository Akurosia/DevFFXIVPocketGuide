...
def addTraitDetails(job):
    ...
    for _id, trait_data in job_trait_data.items():
        ...
        result += f'        level: "{level}"\n'
        result += f'        icon: "{getImage(trait_data["Icon"]['path'].replace(".tex", "_hr1.webp"))}"\n'
        result += '        description:\n'
        for lang in LANGUAGES:
...
def addQuestkDetails(job, pvp):
    ...
    for key, quest in quests.items():
        ...
        if quest['ClassJobCategory0']['Name_de'] in [newjob, newclass] or quest['ClassJobCategory1']['Name_de'] in [newjob, newclass]:
            ...
            #print(key, quest['Expansion'])
            newquest = {
                "name": quest['Name_de'],
                "id": key,
                "expansion": quest['Expansion'].get('Name_de', ""),
...
roleiconsextra = {
    "2": ("062000/062588_hr1.webp", "Primärheiler"),
    "6": ("062000/062589_hr1.webp", "Barriereheiler")
}
...
def addKlassJobs():
    ...
    for k in ncj:
        ...
        job = job_d['Name_de']
        if job == "":
            print_color_yellow(f"Skip Job is empty")
            continue
...
if __name__ == "__main__":
    #print_pretty_json(classjob)
    run()
    #test([{'Ort': 'Abyssos - Fünfter Kreis', 'Gegner': 'Proto-Karfunkel'}, {'Ort': 'Abyssos - Fünfter Kreis (episch)', 'Gegner': 'Proto-Karfunkel'}, {'Ort': 'Das Fenn', 'Gegner': 'Mahisha'}, {'Ort': 'Die Nichts-Arche', 'Gegner': 'Cuchulainn'}, {'Ort': 'Die Welt der Dunkelheit', 'Gegner': 'Cerberus'}, {'Ort': 'Himmelssäule (Ebenen 61-70)', 'Gegner': 'Kenko'}, {'Ort': 'Himmelssäule (Ebenen 81-90)', 'Gegner': 'Himmelssäulen-Gozu'}, {'Ort': 'Historisches Amdapor', 'Gegner': 'Verrottender Gourmet'}, {'Ort': 'Sankt Mocianne-Arboretum (schwer)', 'Gegner': 'Nonechu'}, {'Ort': 'Thavnair', 'Gegner': 'Yilan'}, {'Ort': 'Verschlungene Schatten 1', 'Gegner': '(InGame Hinweis)'}, {'Ort': 'Verschlungene Schatten 2 - 1', 'Gegner': 'Rafflesia'}, {'Ort': 'Verschlungene Schatten 3 - 4', 'Gegner': 'Schmerz Von Meracydia'}])
