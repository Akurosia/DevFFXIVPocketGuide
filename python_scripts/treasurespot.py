...
def write_map_translation_data() -> None:
    ...
    os.chdir("..")
    for lang in LANGUAGES:
        writeJsonFile(f'{path_of_main_script}/assets/translations/treasuremaps/{LANGUAGES_MAPPING[lang]}.json', map_translations[lang])
    os.chdir(origin)
...
def show_image(circle_coords: dict[str, dict[str, Any]]):
    ...
    for tmap, locations in circle_coords.items():
        ...
        if not os.path.exists(f"{path_of_main_script}/_posts/treasuremaps/{_id}/"):
            os.makedirs(f"{path_of_main_script}/_posts/treasuremaps/{_id}/")
        #write _post file
        with open(f"{path_of_main_script}/_posts/treasuremaps/{_id}/{_date}--{patch2}--0--{active_treasuremap["Name_en"]}.md", "w", encoding="utf8") as f:
            f.write(_post)
