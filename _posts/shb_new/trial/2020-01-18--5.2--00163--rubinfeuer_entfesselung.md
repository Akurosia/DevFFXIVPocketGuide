---
wip: "True"
title:
  de: "Rubinfeuer - Entfesselung"
  en: "Cinder Drift"
  fr: "Les Nuées de Brandons"
  ja: "ルビーウェポン破壊作戦"
  cn: "红宝石神兵破坏作战"
  ko: "루비 웨폰 파괴작전"
layout: guide_post
page_type: guide
excel_line: "350"
categories: "shb"
patchNumber: "5.2"
patchLink: "https://de.finalfantasyxiv.com/lodestone/topics/detail/cc103274e6edc3c440533480dc9be0608f939607"
difficulty: "Schwer"
instanceType: "trial"
date: "2020.01.18"
slug: "rubinfeuer-entfesselung"
previous_slug: "/shb/letzte-laeuterung-hades"
next_slug: "/shb/rubinfeuer-trauma"
image:
  - url: "/112000/112379_hr1.png"
terms:
  - term: "Shadowbringers"
  - term: "ShB"
  - term: "Verteidigungslinie Ghimlyt"
  - term: "Cinder Drift"
  - term: "Les Nuées de Brandons"
  - term: "ギムリト防衛ライン"
  - term: "基姆利特防线"
  - term: "김리트 방어선"
  - term: "Rubinfeuer - Entfesselung"
  - term: "Cinder Drift"
  - term: "Les Nuées de Brandons"
  - term: "ルビーウェポン破壊作戦"
  - term: "红宝石神兵破坏作战"
  - term: "루비 웨폰 파괴작전"
  - term: "Schwer"
  - term: "5.20!"
  - term: "5.2!"
  - term: "Echoes of a Fallen Star"
  - term: "Rubinrote Konfrontation"
  - term: "Ruby Doomsday"
  - term: "Une bataille en rouge"
  - term: "ウェポン襲来"
  - term: "神兵来袭"
  - term: "웨폰 습격"
  - term: "tt_cards"
  - term: "Triple Triad Karte"
  - term: "Prüfung"
  - term: "Trial"
  - term: "Primae"
  - term: "Primal"
  - term: "trial"
  - term: "mentor"
  - term: "trial"
  - term: "Zufallsinhalt"
  - term: "roulette"
  - term: "Rubin-Waffe"
  - term: "Hard"
patchName: "Echoes of a Fallen Star"
mapid: "n4fe"
contentname:
  de: "Verteidigungslinie Ghimlyt"
  en: "Cinder Drift"
  fr: "Les Nuées de Brandons"
  ja: "ギムリト防衛ライン"
  cn: "基姆利特防线"
  ko: "김리트 방어선"
sortid: 163
plvl: 80
plvl_sync: 80
ilvl: 455
ilvl_sync: 0
quest:
  de: "Rubinrote Konfrontation"
  en: "Ruby Doomsday"
  fr: "Une bataille en rouge"
  ja: "ウェポン襲来"
  cn: "神兵来袭"
  ko: "웨폰 습격"
quest_location:
  de: "Das Fenn (36.5, 31.8)"
  en: "The Lochs (36.5, 31.8)"
  fr: "Les Lacs (36.5, 31.8)"
  ja: "ギラバニア湖畔地帯 (36.5, 31.8)"
  cn: "基拉巴尼亚湖区 (36.5, 31.8)"
  ko: "기라바니아 호반지대 (36.5, 31.8)"
quest_npc:
  de: "mhigischer Offizier"
  en: "Resistance officer"
  fr: "officier mhigois"
  ja: "解放軍の将校"
  cn: "解放军军官"
  ko: "해방군 장교"
order: 5200800163
tt_card:
  - name:
      de: "Rubin-Waffe"
      en: "Ruby Weapon"
      fr: "Arme Rubis"
      ja: "ルビーウェポン"
      cn: "红宝石神兵"
      ko: "루비 웨폰"
    id: "291"
mtqvid:
  - url: "https://www.youtube.com/watch?v=LSnIyMvPvnE"
mrhvid:
  - url: "https://www.youtube.com/watch?v=2S8w2mpYwuo"
rouletts:
    mentor: True
    trial: True
links:
    teamcraftlink: "20068"
    garlandtoolslink: "20068"
    gamerescapelink: "Cinder_Drift"
contentzoneids:
  - id: 80034E64
contentdescription:
    de: "Lange ist es her, seit Legatus Gaius van Baelsar als Anführer der XIV. Legion die alte allagische Ultima-Waffe exkavierte, wieder aktivierte und Eorzea damit zu unterjochen versuchte. Nun stellt sich heraus, dass die Gefahr noch nicht gebannt ist: Die Garlear haben die Entwicklung der Ultima-Waffe offenbar weiter vorangetrieben und ein verbessertes Modell geschaffen - die Rubin-Waffe. Stell dich dem Monstrum an der Ghimlyt-Finsternis entgegen!"
    en: "Though the original Ultima Weapon, piloted by Gaius Baelsar, was destroyed in the decisive battle at the Praetorium, Garlemald has been secretly developing its successor. Now, this diabolical melding of Allagan and Garlean technologies has been deployed to annihilate those who would oppose the Empire. At the behest of none other than Gaius Baelsar himself, you cross the scorched battlefield of the Ghimlyt Dark to halt this crimson menace before it lays waste to Eorzea."
    fr: "Après la destruction de l'Ultima Arma suite à la bataille de Castrum Meridianum, l'empire de Garlemald a continué à developper en secret de nouvelles machines de guerre anti-Primordiaux. Alors qu'une de ces nouvelles Armes se dirige sur Éorzéa, vous vous apprêtez à l'intercepter sur le front des Ténèbres de Ghimlyt."
    ja: "かつて、ガレマール帝国軍第XIV軍団を率いたガイウス・ヴァン・バエサルは、アラミゴにて発掘した古代アラグ帝国の対蛮神兵器「アルテマウェポン」を復活させ、エオルゼア侵攻の要とした。これに対し、エオルゼア同盟軍は冒険者部隊による破壊作戦を敢行。見事に撃破してみせたのだった。だが、帝国軍は諦めてはいなかった。密かにアルテマウェポンの後継機が開発されていたのである。新たなる脅威、ルビーウェポンをギムリトの地で迎え撃て！"
    cn: "曾经率领加雷马帝国军第十四军团的盖乌斯·范·巴埃萨修复了从阿拉米格发掘出的古代亚拉戈帝国对蛮神兵器“究极神兵”，并将其作为了进攻艾欧泽亚的王牌。对此，艾欧泽亚同盟军指挥冒险者部队强行展开破坏作战。最终成功破坏掉了究极神兵。可是帝国军并没有放弃。究极神兵的后续机体被悄然开发了出来。在基姆利特迎击红宝石神兵这一新的威胁！"
    ko: "가이우스 반 바일사르는 과거 갈레말 제국군 제XIV군단을 이끌던 시절, 알라미고에서 발굴한 고대 알라그 제국의 대 야만신 병기인 '알테마 웨폰'을 부활시켜 에오르제아 침공을 꾀했다. 이에 맞서 에오르제아 동맹군은 모험가 부대를 투입하여 파괴작전을 감행했고, 완벽하게 파괴했다. 하지만 제국군은 포기하지 않았다. 비밀리에 알테마 웨폰의 후속기를 개발했던 것이다. 새로운 위협이 된 루비 웨폰을 김리트 땅에서 요격하라!"
group:
    healer: "2"
    tank: "2"
    melee: "2"
    range: "2"
mechanics:
  - title:
      de: "Mechanic-Title 1"
      en: "Mechanic-Title 1"
      fr: "Mechanic-Title 1"
      ja: "Mechanic-Title 1"
      cn: "Mechanic-Title 1"
      ko: "Mechanic-Title 1"
    steps:
      - step: "09"
        notes:
          - note:
              de: "Mechanics-note 1"
              en: "Mechanics-note 1"
              fr: "Mechanics-note 1"
              ja: "Mechanics-note 1"
              cn: "Mechanics-note 1"
              ko: "Mechanics-note 1"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//akurosia.de/upload/test.mp4"
bosses:
  - title:
      de: "Rubin-Waffe"
      en: "The Ruby Weapon"
      fr: "Arme Rubis"
      ja: "ルビーウェポン"
      cn: "红宝石神兵"
      ko: "루비 웨폰"
    enemy_id: "9250"
    id: "boss02"
    hp:
      - min: 3397458
      - max: 4913273
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "368"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Bradamante"
          en: "Bradamante"
          fr: "Bradamante"
          ja: "ブラダマンテ"
          cn: "布拉达曼特"
          ko: "브라다만테"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4ABB"
            damage_type: "Blunt"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4ABC"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Dalamud-Sturz"
          en: "Dalamud Impact"
          fr: "Impact De Dalamud"
          ja: "ダラガブインパクト"
          cn: "卫月冲击"
          ko: "달라가브 낙하"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AAE"
            damage_type: "Magical"
            damage:
              - min: 117
              - max: 22146
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AAF"
            damage_type: "None"
            damage:
              - min: 117
              - max: 22146
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4E51"
            damage_type: "Magical"
            damage:
              - min: 117
              - max: 60533
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Einziehen"
          en: "Retract"
          fr: "Rétraction"
          ja: "引き抜く"
          cn: "拔起"
          ko: "빼내기"
        title_id: "4A98"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Empörung"
          en: "Outrage"
          fr: "Indignation"
          ja: "アウトレイジ"
          cn: "震怒"
          ko: "격노"
        title_id: "4AC8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 16162
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Flammenstachel"
          en: "Spike Of Flame"
          fr: "Explosion De Feu"
          ja: "爆炎"
          cn: "爆炎柱"
          ko: "폭염"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        add_status:
          - status: 6FD
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4A94"
            damage_type: "Darkness"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 12433
              - max: 22920
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4A95"
            damage_type: "Magical"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 256
              - max: 34465
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4A9A"
            damage_type: "Blunt"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 12433
              - max: 22920
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4B2E"
            damage_type: "Piercing"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 6712
              - max: 8764
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4D02"
            damage_type: "Piercing"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 12433
              - max: 22920
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4D03"
            damage_type: "Magical"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 3992
              - max: 8335
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Flex-Klauen"
          en: "Flexiclaw"
          fr: "Flexigriffes"
          ja: "フレキシブルクロー"
          cn: "潜地爪"
          ko: "가변 발톱"
        title_id: "4A92"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 7
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Flug Des Raben"
          en: "Ravensflight"
          fr: "Vol Du Rapace"
          ja: "レイヴェンダイブ"
          cn: "凶鸟冲"
          ko: "흉조 돌진"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AA1"
            damage_type: "Piercing"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA2"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA3"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA4"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA5"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA6"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA7"
            damage_type: "None"
            damage:
              - min: 4
              - max: 4
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4E69"
            damage_type: "Blunt"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 1024
              - max: 31145
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4E6A"
            damage_type: "Piercing"
            add_status:
              - status: 6FD
                icon: "017000/017101.png"
                name:
                 de: "Erhöhte Verwundbarkeit"
                 en: "Vulnerability Up"
                 fr: "Vulnérabilité augmentée"
                 ja: "被ダメージ上昇"
                 cn: "受伤加重"
                 ko: "받는 피해량 증가"
            damage:
              - min: 10976
              - max: 10976
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Flug Des Raben "
          en: "Ravensflight"
          fr: "Vol Du Rapace"
          ja: "レイヴェンダイブ"
          cn: "凶鸟冲"
          ko: "흉조 돌진"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4C21"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4C2A"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Hochenergie-Leitlaser"
          en: "High-Powered Homing Lasers"
          fr: "Lasers Autoguidés Surpuissants"
          ja: "高出力誘導レーザー"
          cn: "高功率诱导射线"
          ko: "고출력 유도 레이저"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AC4"
            damage_type: "Piercing"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AC5"
            damage_type: "Magical"
            add_status:
              - status: 82B
                icon: "015000/015057.png"
                name:
                 de: "Erhöhte Magie-Verwundbarkeit"
                 en: "Magic Vulnerability Up"
                 fr: "Vulnérabilité magique augmentée"
                 ja: "被魔法ダメージ増加"
                 cn: "魔法受伤加重"
                 ko: "받는 마법 피해량 증가"
            damage:
              - min: 21
              - max: 58405
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Leitlaser"
          en: "Homing Lasers"
          fr: "Lasers Autoguidés"
          ja: "誘導レーザー"
          cn: "诱导射线"
          ko: "유도 레이저"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AC2"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AC3"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 60437
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Magitek-Komet"
          en: "Magitek Comet"
          fr: "Comète Magitek"
          ja: "魔導コメット"
          cn: "魔导彗星"
          ko: "마도 혜성"
        title_id: "4AB0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Magitek-Komet Stufe II"
          en: "Mark II Magitek Comet"
          fr: "Comète Magitek II"
          ja: "魔導コメットIi"
          cn: "魔导彗星Ii"
          ko: "마도 혜성 2"
        title_id: "4AB6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Magitek-Meteor"
          en: "Magitek Meteor"
          fr: "Météore Magitek"
          ja: "魔導メテオ"
          cn: "魔导陨石"
          ko: "마도 메테오"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AB1"
            damage_type: "Piercing"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AB2"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Meteorflug"
          en: "Meteor Stream"
          fr: "Rayon Météore"
          ja: "メテオストリーム"
          cn: "陨石流"
          ko: "유성 폭풍"
        title_id: "4E71"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Persona Negativa"
          en: "Negative Personae"
          fr: "Ipséité Négative"
          ja: "ネガティブペルソナ"
          cn: "消极人格"
          ko: "부정적 페르소나"
        title_id: "4ABD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Projekt Meteor"
          en: "Meteor Project"
          fr: "Projet Météore"
          ja: "メテオ計劃"
          cn: "陨石计划"
          ko: "메테오 계획"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AAA"
            damage_type: "Darkness"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AAC"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Rabenklauen"
          en: "Ravensclaw"
          fr: "Griffes Du Rapace"
          ja: "レイヴェンクロー"
          cn: "凶鸟爪"
          ko: "흉조 발톱"
        title_id: "4A93"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Rubin-Dynamo"
          en: "Ruby Dynamics"
          fr: "Générateur Rubis"
          ja: "ルビーダイナモ"
          cn: "红宝石电圈"
          ko: "루비의 원동력"
        title_id: "4AA0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: 6FD
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
        damage:
          - min: 15270
          - max: 22920
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Rubin-Strahl"
          en: "Ruby Ray"
          fr: "Rayon Rubis"
          ja: "ルビーレイ"
          cn: "红宝石射线"
          ko: "루비 광선"
        title_id: "4AC6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 6FD
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
        damage:
          - min: 17952
          - max: 18798
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Spiralklauen"
          en: "Helicoclaw"
          fr: "Héliogriffes"
          ja: "スパイラルクロー"
          cn: "螺旋爪"
          ko: "나선 발톱"
        title_id: "4A99"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Ultima-System"
          en: "Optimized Ultima"
          fr: "Ultima Magitek"
          ja: "魔導アルテマ"
          cn: "魔导究极"
          ko: "마도 알테마"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "4AA8"
            damage_type: "Magical"
            damage:
              - min: 117
              - max: 60533
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "4AA9"
            damage_type: "Darkness"
            damage:
              - min: 117
              - max: 60533
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Unknown_4A9B"
          en: "Unknown_4A9B"
          fr: "Unknown_4A9B"
          ja: "Unknown_4A9B"
          cn: "Unknown_4A9B"
          ko: "Unknown_4A9B"
        title_id: "4A9B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_4BCD"
          en: "Unknown_4BCD"
          fr: "Unknown_4BCD"
          ja: "Unknown_4BCD"
          cn: "Unknown_4BCD"
          ko: "Unknown_4BCD"
        title_id: "4BCD"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_4CF6"
          en: "Unknown_4CF6"
          fr: "Unknown_4CF6"
          ja: "Unknown_4CF6"
          cn: "Unknown_4CF6"
          ko: "Unknown_4CF6"
        title_id: "4CF6"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 5751
        phases:
          - phase: "09"
      - title:
          de: "Unknown_4D57"
          en: "Unknown_4D57"
          fr: "Unknown_4D57"
          ja: "Unknown_4D57"
          cn: "Unknown_4D57"
          ko: "Unknown_4D57"
        title_id: "4D57"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_4E1C"
          en: "Unknown_4E1C"
          fr: "Unknown_4E1C"
          ja: "Unknown_4E1C"
          cn: "Unknown_4E1C"
          ko: "Unknown_4E1C"
        title_id: "4E1C"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Untergraben"
          en: "Undermine"
          fr: "Griffe Souterraine"
          ja: "クローマイン"
          cn: "掘地雷"
          ko: "발톱 지뢰"
        title_id: "4A97"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Verflüssigen"
          en: "Liquefaction"
          fr: "Sables Mouvants"
          ja: "リクェファクション"
          cn: "地面液化"
          ko: "융해"
        title_id: "4A96"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Zerstampfen"
          en: "Stamp"
          fr: "Piétinement Sévère"
          ja: "ストンピング"
          cn: "重踏"
          ko: "발구름"
        title_id: "4AC7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7108
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Erhöhte Magie-Verwundbarkeit"
          en: "Magic Vulnerability Up"
          fr: "Vulnérabilité Magique Augmentée"
          ja: "被魔法ダメージ増加"
          cn: "魔法受伤加重"
          ko: "받는 마법 피해량 증가"
        title_id: "82B"
        icon: "015000/015057_hr1.png"
        description:
          de: "Erlittener Magieschaden ist erhöht."
          en: "Magic damage taken is increased."
          fr: "Les dégâts magiques reçus sont augmentés."
          ja: "受ける魔法ダメージが増加する状態。"
          cn: "受到魔法攻击的伤害增加"
          ko: "마법 공격으로 받는 피해량이 증가하는 상태."
        durations: [2]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "6FD"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [89, 90]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Unbekannte Herkunft"
      en: "Unknown Source"
      fr: "Unknown Source"
      ja: "Unknown Source"
      cn: "Unknown Source"
      ko: "Unknown Source"
    enemy_id: ""
    id: "boss01"
    debuffs:
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "6FD"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [9]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Geistschild"
          en: "Unwavering Will"
          fr: "Barrière Psychologique"
          ja: "心理障壁"
          cn: "心理障壁"
          ko: "심리 장벽"
        title_id: "89F"
        icon: "015000/015024_hr1.png"
        description:
          de: "Durch eine geistige Barriere geschützt. Alle Attacken werden abgewehrt."
          en: "Protected by an unseen aegis, rendering all attacks ineffective."
          fr: "Le corps est protégé par l'apparition d'une barrière spectrale. Devient insensible à tous les coups."
          ja: "幻体による障壁で保護されている状態。すべての攻撃を無効化する。"
          cn: "被幻体形成的障壁保护，所有攻击无效化"
          ko: "환영 장벽으로 보호받는 상태. 모든 공격을 무효화한다."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Meteorfänger"
          en: "Atlas"
          fr: "Chute Encaissée"
          ja: "落下阻止"
          cn: "阻止降落"
          ko: "낙하 저지"
        title_id: "89E"
        icon: "015000/015231_hr1.png"
        description:
          de: "Hält Meteor auf. Am Ende des Effekts tritt K. o. ein."
          en: "Resisting the weight of a falling star. When this effect expires, impact will result in KO."
          fr: "Subit la chute d'un météore. Rend inconscient lorsque la durée de l'effet s'est écoulée."
          ja: "隕石を受け止めている状態。効果時間が終了すると、戦闘不能状態になる。"
          cn: "阻挡陨石，效果时间结束时会陷入无法战斗状态"
          ko: "떨어지는 운석을 막는 상태. 지속시간이 끝나면 전투불능이 된다."
        durations: [18]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Verringerte Verwundbarkeit"
          en: "Vulnerability Down"
          fr: "Vulnérabilité Diminuée"
          ja: "被ダメージ低下"
          cn: "受伤减轻"
          ko: "받는 피해량 감소"
        title_id: "896"
        icon: "015000/015021_hr1.png"
        description:
          de: "Erlittener Schaden ist verringert."
          en: "Damage taken is reduced."
          fr: "Les dégâts reçus sont réduits."
          ja: "受けるダメージが軽減される状態。"
          cn: "受到攻击的伤害减少"
          ko: "받는 피해량이 감소하는 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Versinkend"
          en: "Six Fulms Under"
          fr: "Enfoncement"
          ja: "沈下"
          cn: "下陷"
          ko: "침하"
        title_id: "237"
        icon: "015000/015769_hr1.png"
        description:
          de: "Der Körper versinkt langsam. Bewegungsgeschwindigeit verringert sich immer mehr. Wenn völlig versunken, tritt Kampfunfähigkeit ein."
          en: "You are slowly slipping down, your movement speed dropping as you descend deeper. Burial will result in KO."
          fr: "Le corps s'enfonce petit à petit, réduisant progressivement la vitesse de déplacement et causant la mort en cas d'ensevelissement total."
          ja: "徐々に身体が沈んでいく状態。移動速度が徐々に低下し、身体が沈みきると戦闘不能状態に陥る。"
          cn: "身体逐渐下沉，移动速度逐渐降低，身体完全沉下去会陷入无法战斗状态"
          ko: "서서히 몸이 가라앉는 상태. 이동 속도가 느려지며, 몸이 완전히 빠지면 전투불능 상태가 된다."
        durations: [4]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
adds:
  - title:
      de: "Komet"
      en: "Comet"
      fr: "Comète"
      ja: "コメット"
      cn: "彗星"
      ko: "혜성"
    enemy_id: "731"
    id: "add01"
    hp:
      - min: 10244
      - max: 38845
    attacks:
      - title:
          de: "Detonation"
          en: "Heavy Impact"
          fr: "Déflagration"
          ja: "大爆発"
          cn: "大爆炸"
          ko: "대폭발"
        title_id: "4AB9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Druck"
          en: "Sunder Pressure"
          fr: "Force De Pesanteur"
          ja: "重圧"
          cn: "超重压"
          ko: "중압"
        title_id: "4AB7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 4923
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Einschlag"
          en: "Landing"
          fr: "Chute De Météore"
          ja: "落着"
          cn: "落地"
          ko: "경착륙"
        title_id: "4E2B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 12010
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Explosion"
          en: "Burst"
          fr: "Éclats Cosmique"
          ja: "飛散"
          cn: "飞散"
          ko: "산산조각"
        title_id: "4AB4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 117
          - max: 635
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Impakt"
          en: "Impact"
          fr: "Impact"
          ja: "衝撃"
          cn: "冲击"
          ko: "충격"
        title_id: "4AB8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 3708
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_4E31"
          en: "Unknown_4E31"
          fr: "Unknown_4E31"
          ja: "Unknown_4E31"
          cn: "Unknown_4E31"
          ko: "Unknown_4E31"
        title_id: "4E31"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "6FD"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [89, 90]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Naels Trugbild"
      en: "Raven's Image"
      fr: "Spectre De Nael"
      ja: "ネールの幻影"
      cn: "奈尔的幻影"
      ko: "넬의 환영"
    enemy_id: "9254"
    id: "add02"
    hp:
      - min: 252280
      - max: 304358
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "367"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 6681
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Meteorflug"
          en: "Meteor Stream"
          fr: "Rayon Météore"
          ja: "メテオストリーム"
          cn: "陨石流"
          ko: "유성 폭풍"
        title_id: "4E67"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 61810
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Rubin-Klauen"
          en: "Ruby Claw"
          fr: "Griffe Rubis"
          ja: "ルビークロー"
          cn: "红宝石之爪"
          ko: "루비 발톱"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "4ABF"
            damage_type: "Piercing"
            damage:
              - min: 113
              - max: 61810
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "4AC0"
            damage_type: "Piercing"
            damage:
              - min: 7
              - max: 61810
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Unknown_4D0E"
          en: "Unknown_4D0E"
          fr: "Unknown_4D0E"
          ja: "Unknown_4D0E"
          cn: "Unknown_4D0E"
          ko: "Unknown_4D0E"
        title_id: "4D0E"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
---
