---
wip: "True"
title:
  de: "Eureka Pagos"
  en: "The Forbidden Land, Eureka Pagos"
  fr: "Eurêka Pagos"
  ja: "禁断の地 エウレカ：パゴス編"
  cn: "禁地优雷卡 恒冰之地"
  ko: "금단의 땅 에우레카: 파고스편"
layout: guide_post
page_type: guide
excel_line: "279"
categories: "sb"
patchNumber: "4.36"
patchLink: "https://de.finalfantasyxiv.com/lodestone/topics/detail/408dc62e12bf06b539575b777cf5b298f8e9afe0"
difficulty: "Normal"
instanceType: "feldexkursion"
date: "2018.08.07"
slug: "eureka-pagos"
previous_slug: "/sb/eureka-anemos"
next_slug: "/sb/eureka-pyros"
image:
  - url: "/112000/112308_hr1.png"
terms:
  - term: "Stormblood"
  - term: "SB"
  - term: "Eureka Pagos"
  - term: "Eureka Pagos"
  - term: "Eurêka Pagos"
  - term: "エウレカ：パゴス帯"
  - term: "优雷卡恒冰之地"
  - term: "에우레카: 파고스 지대"
  - term: "Eureka Pagos"
  - term: "The Forbidden Land, Eureka Pagos"
  - term: "Eurêka Pagos"
  - term: "禁断の地 エウレカ：パゴス編"
  - term: "禁地优雷卡 恒冰之地"
  - term: "금단의 땅 에우레카: 파고스편"
  - term: "Normal"
  - term: "4.36!"
  - term: "Under the Moonlight"
  - term: "Pagos, das eisige Land"
  - term: "And We Shall Call It Pagos"
  - term: "Pagos, la terre de glace"
  - term: "氷雪の地、その名はパゴス"
  - term: "冰雪之地，其名为恒冰地带"
  - term: "눈과 얼음의 땅 파고스"
  - term: "tt_cards"
  - term: "Triple Triad Karte"
  - term: "feldexkursion"
  - term: "Schneekönigin"
  - term: "Taxim"
  - term: "Aschedrache"
  - term: "Glavoid"
  - term: "Anapo"
  - term: "Hakutaku"
  - term: "Iglu-König"
  - term: "Asag"
  - term: "Surabhi"
  - term: "König Athro"
  - term: "niederer Minotaurus / höherer Minotaurus"
  - term: "Heilsbringer"
  - term: "Hadhayosh"
  - term: "Horus"
  - term: "Erz-Angra Mainyu"
  - term: "Louhi"
  - term: "kopierende Cassie"
patchName: "Under the Moonlight"
mapid: "z3fb"
contentname:
  de: "Eureka Pagos"
  en: "Eureka Pagos"
  fr: "Eurêka Pagos"
  ja: "エウレカ：パゴス帯"
  cn: "优雷卡恒冰之地"
  ko: "에우레카: 파고스 지대"
sortid: 1
plvl: 70
plvl_sync: 70
ilvl: 0
ilvl_sync: 300
quest:
  de: "Pagos, das eisige Land"
  en: "And We Shall Call It Pagos"
  fr: "Pagos, la terre de glace"
  ja: "氷雪の地、その名はパゴス"
  cn: "冰雪之地，其名为恒冰地带"
  ko: "눈과 얼음의 땅 파고스"
quest_location:
  de: "Kugane (8.5, 14.2)"
  en: "Kugane (8.5, 14.2)"
  fr: "Kugane (8.5, 14.2)"
  ja: "クガネ (8.5, 14.2)"
  cn: "黄金港 (8.5, 14.2)"
  ko: "쿠가네 (8.5, 14.2)"
quest_npc:
  de: "Rodney"
  en: "Rodney"
  fr: "Rodney"
  ja: "ロドニー"
  cn: "罗德尼"
  ko: "로드니"
order: 4360700001
tt_card:
  - name:
      de: "Flöckchen"
      en: "Yukinko"
      fr: "yukinko"
      ja: "ユキンコ"
      cn: "雪童子"
      ko: "눈요정"
    id: "223"
  - name:
      de: "Louhi"
      en: "Louhi"
      fr: "Louhi"
      ja: "ロウヒ"
      cn: "娄希"
      ko: "로우히"
    id: "227"
links:
    gamerescapelink: "The_Forbidden_Land,_Eureka_Pagos"
contentzoneids:
  - id: 80040004
contentdescription:
    de: "Eine urplötzlich im Glasmeer aufgetauchte Insel, die sich als Val entpuppt. Vertraute Regionen, in denen entfesselte Elemente Flora wie Fauna auf den Kopf stellen. Die Erforschung der Region Anemos brachte Antworten, warf aber auch neue Fragen auf. Nach erfolgreicher Justierung des Ätherytenrelais steht das Tor zu einem weiteren Teil der Insel nun offen: Pagos ist eine zerklüftete Bergregion unter starkem eiselementarem Einfluss. Welche Mysterien werden den Expeditionstrupp dort erwarten? Greif dir deine Magia-Tafel und finde es heraus!"
    en: "Though they have yet to determine how such a thing could even be possible, the expedition is nevertheless certain that the place they have dubbed Eureka is none other than the Isle of Val, previously believed to have been obliterated by a powerful magick. Having completed their survey of Anemos, the expedition now turns its sights to a new region: Pagos, a mountainous frozen waste filled with untold dangers..."
    fr: "Le bout de terre apparu dans l'Océan de Verre commence à livrer ses secrets. En effet, l'équipe d'exploration a désormais la certitude qu'Eurêka serait en fait l'île septentrionale de Val. Elle mérite toutefois bien son surnom de terre interdite, car même si sa géographie est restée inchangée, le chaos des éléments qui y règne en fait un lieu aux mille dangers. Les aventuriers ayant terminé leur exploration d'Anemos, ils font maintenant route vers Pagos, une région à flanc de montagne prisonnière des glaces et des neiges éternelles."
    ja: "瑠璃洋にて発見された新島の調査を請け負った冒険者たちは、そこが北洋に存在していたはずの「バル島」であることを突き止める。しかし、地形は変わらずとも属性の力が入り乱れ、環境は一変。凶暴化した魔物が徘徊する、まさに「禁断の地」と呼ぶにふさわしい危険地帯と化していた。<br/><br/>アネモス帯と名付けた一帯の調査を終えた冒険者たちが、次に向かうのは山岳部に面した地域。氷属性の力が強まったことで、寒冷地に変じたパゴス帯を調査せよ！"
    cn: "冒险者接受委托前往琉璃洋上新发现的未知岛屿调查，探明了那里就是从前位于北洋上的瓦尔岛。岛上的地形虽然未曾改变，但混乱的属性之力却引发了环境的剧变。凶暴的魔物四处徘徊，使此处成了名符其实的禁地。<br/>调查完常风之地后，冒险者前往了面向山岳的地域。开始在冰属性之力强盛，完全化为了寒冷地带的恒冰之地展开调查！"
    ko: "유리양에서 발견된 새로운 섬의 조사를 맡게 된 모험가들은, 그곳이 북해에 있었던 '발 섬'이라는 것을 알아냈다. 지형은 그대로지만 속성의 힘이 뒤섞여 환경이 완전히 달라진 발 섬은 그야말로 '금단의 땅'이라고 부름직한, 포악한 마물이 배회하는 위험 지대가 되었다.<br/>아네모스라고 이름 붙인 일대의 조사를 마친 모험가들은 산악부에 접한 지역으로 향한다. 얼음속성의 힘이 강해져서 한랭지로 변한 파고스 지대를 조사하라!"
mechanics:
  - title: "Mechanic-Title 1"
    steps:
      - step: "09"
        notes:
          - note: "Mechanics-note 1"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//akurosia.de/upload/test.mp4"
bosses:
  - title:
      de: "Schneekönigin"
      en: "The Snow Queen"
      fr: "Reine Des Neiges"
      ja: "雪の女王"
      cn: "雪之女王"
      ko: "눈의 여왕"
    enemy_id: "7526"
    id: "boss15"
    hp:
      - min: 4748803
      - max: 5853176
    attacks:
      - title:
          de: "Schneeregen"
          en: "Flurry"
          fr: "Averse De Neige"
          ja: "雪時雨"
          cn: "雨夹雪"
          ko: "진눈깨비"
        title_id: "2D2B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 4715
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schneewehe"
          en: "Snow Squall"
          fr: "Rafale De Neige"
          ja: "雪嵐"
          cn: "雪风暴"
          ko: "눈태풍"
        title_id: "2D2D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 60453
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D29"
          en: "Unknown_2D29"
          fr: "Unknown_2D29"
          ja: "Unknown_2D29"
          cn: "Unknown_2D29"
          ko: "Unknown_2D29"
        title_id: "2D29"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 113
          - max: 61811
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Taxim"
      en: "Taxim"
      fr: "Taxim"
      ja: "タキシム"
      cn: "塔克西姆"
      ko: "텍심"
    enemy_id: "7535"
    id: "boss17"
    hp:
      - min: 4134502
      - max: 5831824
    attacks:
      - title:
          de: "Glasschlag"
          en: "Glass Punch"
          fr: "Poing De Verre"
          ja: "ブン回し"
          cn: "挥舞"
          ko: "휘두르기"
        title_id: "2E88"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: 95
            icon: "015000/015004.png"
            name:
               de: "Betäubung"
               en: "Stun"
               fr: "Étourdissement"
               ja: "スタン"
               cn: "眩晕"
               ko: "기절"
        damage:
          - min: 7
          - max: 5421
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Katapult"
          en: "Catapult"
          fr: "Catapulte"
          ja: "カタパルト"
          cn: "投射"
          ko: "투석"
        title_id: "2E89"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schmetterlingsflug"
          en: "Butterfly Float"
          fr: "Vol De Papillon"
          ja: "カチ込み"
          cn: "追打"
          ko: "습격"
        title_id: "2E8A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 1516
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2E8C"
          en: "Unknown_2E8C"
          fr: "Unknown_2E8C"
          ja: "Unknown_2E8C"
          cn: "Unknown_2E8C"
          ko: "Unknown_2E8C"
        title_id: "2E8C"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 6876
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "95"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Aschedrache"
      en: "Ash Dragon"
      fr: "Dragon Cendré"
      ja: "アッシュドラゴン"
      cn: "灰烬龙"
      ko: "잿더미 드래곤"
    enemy_id: "7520"
    id: "boss04"
    hp:
      - min: 5968049
      - max: 7318287
    attacks:
      - title:
          de: "Schiefer Blick"
          en: "The Look"
          fr: "Regard Intense"
          ja: "怪視線"
          cn: "诡异视线"
          ko: "괴이한 시선"
        title_id: "2D39"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 14794
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schweifsturz"
          en: "Tail Drive"
          fr: "Offensive Caudale"
          ja: "テイルドライブ"
          cn: "尾部打击"
          ko: "꼬리쓸기"
        title_id: "2D3A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 4812
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Steinatem"
          en: "Lithic Breath"
          fr: "Souffle Lithique"
          ja: "リシックブレス"
          cn: "石质吐息"
          ko: "바위 숨결"
        title_id: "2D38"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 5E7
            icon: "015000/015001.png"
            name:
               de: "Stein"
               en: "Petrification"
               fr: "Pétrification"
               ja: "石化"
               cn: "石化"
               ko: "석화"
        damage:
          - min: 117
          - max: 16622
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D37"
          en: "Unknown_2D37"
          fr: "Unknown_2D37"
          ja: "Unknown_2D37"
          cn: "Unknown_2D37"
          ko: "Unknown_2D37"
        title_id: "2D37"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Gewicht"
          en: "Heavy"
          fr: "Pesanteur"
          ja: "ヘヴィ"
          cn: "加重"
          ko: "과중력"
        title_id: "0E"
        icon: "015000/015002_hr1.png"
        description:
          de: "Bewegungsgeschwindigkeit ist verringert."
          en: "Movement speed is reduced."
          fr: "La vitesse de déplacement est réduite."
          ja: "移動速度が低下した状態。"
          cn: "移动速度降低"
          ko: "이동 속도가 느려진 상태."
        durations: [7, 15, 30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Stein"
          en: "Petrification"
          fr: "Pétrification"
          ja: "石化"
          cn: "石化"
          ko: "석화"
        title_id: "5E7"
        icon: "015000/015001_hr1.png"
        description:
          de: "Die Versteinerung des Körpers verhindert das Ausführen jeglicher Kommandos."
          en: "Stone-like rigidity is preventing the execution of actions."
          fr: "Une rigidité semblable à celle de la pierre empêche toute action."
          ja: "手足が石化し、行動できなくなった状態。"
          cn: "手足被石化，无法做出任何行动"
          ko: "손발이 석화되어 행동이 불가능한 상태."
        durations: [4, 8, 16]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Glavoid"
      en: "Glavoid"
      fr: "Graboïde"
      ja: "グラヴォイド"
      cn: "异形魔虫"
      ko: "그라보이드"
    enemy_id: "7516"
    id: "boss06"
    hp:
      - min: 4523755
      - max: 6195093
    attacks:
      - title:
          de: "Erdbeben"
          en: "Earthquake"
          fr: "Tremblement De Terre"
          ja: "地震"
          cn: "地震"
          ko: "지진"
        title_id: "2D3E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 69
          - max: 5650
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Sandatem"
          en: "Sand Breath"
          fr: "Souffle De Sable"
          ja: "サンドブレス"
          cn: "黄沙吐息"
          ko: "모래 숨결"
        title_id: "2D40"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        damage:
          - min: 70
          - max: 4141
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Sandzyklon"
          en: "Sand Cyclone"
          fr: "Cyclone De Sable"
          ja: "サンドサイクロン"
          cn: "沙尘旋风"
          ko: "모래 태풍"
        title_id: "2D3F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 6432
          - max: 6432
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D3C"
          en: "Unknown_2D3C"
          fr: "Unknown_2D3C"
          ja: "Unknown_2D3C"
          cn: "Unknown_2D3C"
          ko: "Unknown_2D3C"
        title_id: "2D3C"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 10052
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Blind"
          en: "Blind"
          fr: "Cécité"
          ja: "暗闇"
          cn: "失明"
          ko: "실명"
        title_id: "0F"
        icon: "015000/015012_hr1.png"
        description:
          de: "Vorübergehende Blindheit senkt die Präzision."
          en: "Encroaching darkness is lowering accuracy."
          fr: "Un voile d'obscurité diminue la précision."
          ja: "闇に囚われ、命中率が低下した状態。"
          cn: "陷于黑暗之中，命中率降低"
          ko: "어둠에 사로잡혀 명중률이 떨어진 상태."
        durations: [4, 8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Anapo"
      en: "Anapos"
      fr: "Anapos"
      ja: "アナポ"
      cn: "安娜波"
      ko: "아나포"
    enemy_id: "7497"
    id: "boss02"
    attacks:
      - title:
          de: "Aquaga"
          en: "Water III"
          fr: "Méga Eau"
          ja: "ウォタガ"
          cn: "狂水"
          ko: "워터가"
        title_id: "2D48"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Einsaugen"
          en: "Inhale"
          fr: "Aspiration"
          ja: "吸引"
          cn: "吸引"
          ko: "흡인"
        title_id: "2D47"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Horden-Wiegenlied"
          en: "Horde Lullaby"
          fr: "Berceuse Monstrueuse"
          ja: "魔物達のララバイ"
          cn: "魔物们的摇篮曲"
          ko: "마물들의 자장가"
        title_id: "2D46"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Höllen-Elegie"
          en: "Massacre Elegy"
          fr: "Élégie Infernale"
          ja: "地獄のエレジー"
          cn: "地狱挽歌"
          ko: "지옥의 비가"
        title_id: "2D49"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D45"
          en: "Unknown_2D45"
          fr: "Unknown_2D45"
          ja: "Unknown_2D45"
          cn: "Unknown_2D45"
          ko: "Unknown_2D45"
        title_id: "2D45"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
      - title:
          de: "Ätherfunken"
          en: "Aetherial Spark"
          fr: "Étincelle Éthérée"
          ja: "エーテリックスパーク"
          cn: "以太火花"
          ko: "에테르 불꽃"
        title_id: "2D4A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "95"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "5E5"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [16]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Schlaf"
          en: "Sleep"
          fr: "Sommeil"
          ja: "睡眠"
          cn: "睡眠"
          ko: "수면"
        title_id: "03"
        icon: "015000/015013_hr1.png"
        description:
          de: "Tiefschlaf verhindert das Ausführen jeglicher Kommandos."
          en: "Overwhelming drowsiness is preventing the execution of actions."
          fr: "Le sommeil empêche toute action."
          ja: "眠りに落ち、行動できなくなった状態。"
          cn: "陷入沉睡，无法做出任何行动"
          ko: "잠에 빠져 행동이 불가능한 상태."
        durations: [16]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Hakutaku"
      en: "Hakutaku"
      fr: "Hakutaku"
      ja: "ハクタク"
      cn: "白泽"
      ko: "백택"
    enemy_id: "7518"
    id: "boss08"
    hp:
      - min: 41919
      - max: 7507320
    attacks:
      - title:
          de: "Dunkel-Feuga"
          en: "Dark Fire III"
          fr: "Méga Feu Ténébreux"
          ja: "ダークファイガ"
          cn: "黑暗爆炎"
          ko: "다크 파이가"
        title_id: "2D98"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Feuersbrunst"
          en: "Fire Whirl"
          fr: "Tourbillon Incendiaire"
          ja: "火災旋風"
          cn: "火灾旋风"
          ko: "화염 선풍"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D8F"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D90"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 20367
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Flamme"
          en: "Torch"
          fr: "Fournaise"
          ja: "火炎"
          cn: "炎爆"
          ko: "화염"
        title_id: "2D96"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Hex-Auge"
          en: "Hex Eye"
          fr: "Œil Néfaste"
          ja: "凶眼"
          cn: "凶眼"
          ko: "흉안"
        title_id: "2D97"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 11
            icon: "015000/015006.png"
            name:
               de: "Paralyse"
               en: "Paralysis"
               fr: "Paralysie"
               ja: "麻痺"
               cn: "麻痹"
               ko: "마비"
        damage:
          - min: 17
          - max: 17
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Infernoflammen"
          en: "Infernal Flames"
          fr: "Flammes Infernales"
          ja: "業火炎"
          cn: "业火"
          ko: "업화의 불꽃"
        title_id: "2D8C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 21
          - max: 12034
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Lebende Fackel"
          en: "Live Pyre"
          fr: "Bûcher Morbide"
          ja: "大凶の炎"
          cn: "大凶之炎"
          ko: "대흉의 불꽃"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D93"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D94"
            damage_type: "Magical"
            damage:
              - min: 13
              - max: 6244
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Lebende Kerze"
          en: "Live Candle"
          fr: "Bougie Funeste"
          ja: "小凶の炎"
          cn: "小凶之炎"
          ko: "소흉의 불꽃"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        add_status:
          - status: 232
            icon: "015000/015026.png"
            name:
               de: "Markiert"
               en: "Prey"
               fr: "Marquage"
               ja: "マーキング"
               cn: "猎物"
               ko: "표식"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D91"
            damage_type: "Piercing"
            add_status:
              - status: 232
                icon: "015000/015026.png"
                name:
                 de: "Markiert"
                 en: "Prey"
                 fr: "Marquage"
                 ja: "マーキング"
                 cn: "猎物"
                 ko: "표식"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D92"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Todesstrahl"
          en: "Death Ray"
          fr: "Rayon De La Mort"
          ja: "デスレイ"
          cn: "死亡射线"
          ko: "죽음의 광선"
        title_id: "2D95"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D8B"
          en: "Unknown_2D8B"
          fr: "Unknown_2D8B"
          ja: "Unknown_2D8B"
          cn: "Unknown_2D8B"
          ko: "Unknown_2D8B"
        title_id: "2D8B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 5524
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Markiert"
          en: "Prey"
          fr: "Marquage"
          ja: "マーキング"
          cn: "猎物"
          ko: "표식"
        title_id: "232"
        icon: "015000/015026_hr1.png"
        description:
          de: "Als Beute markiert."
          en: "Marked as prey."
          fr: "Fait l'objet du marquage d'un prédateur."
          ja: "獲物として狙いを付けられた状態。"
          cn: "作为猎物被盯上了"
          ko: "표적으로 점찍힌 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Iglu-König"
      en: "King Igloo"
      fr: "Roi Igloo"
      ja: "キングイグルー"
      cn: "雪屋王"
      ko: "이글루 왕"
    enemy_id: "7503"
    id: "boss11"
    hp:
      - min: 39313
      - max: 4567270
    attacks:
      - title:
          de: "Schneeblind"
          en: "Snowblind"
          fr: "Voile Neigeux"
          ja: "雪煙"
          cn: "雪烟"
          ko: "눈안개"
        title_id: "2D9F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 5176
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schneegebirge"
          en: "Snowcap"
          fr: "Chaîne Glaciaire"
          ja: "大雪山"
          cn: "大雪山"
          ko: "대설산"
        title_id: "2D9C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D99"
          en: "Unknown_2D99"
          fr: "Unknown_2D99"
          ja: "Unknown_2D99"
          cn: "Unknown_2D99"
          ko: "Unknown_2D99"
        title_id: "2D99"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D9B"
          en: "Unknown_2D9B"
          fr: "Unknown_2D9B"
          ja: "Unknown_2D9B"
          cn: "Unknown_2D9B"
          ko: "Unknown_2D9B"
        title_id: "2D9B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Weißer Tod"
          en: "White Death"
          fr: "Mort Blanche"
          ja: "氷霧"
          cn: "冰雾"
          ko: "얼음안개"
        title_id: "2D9E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 5826
          - max: 50213
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Zufrieren"
          en: "Bergy Bit"
          fr: "Gelée"
          ja: "結氷"
          cn: "结冰"
          ko: "결빙"
        title_id: "2DA1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 15909
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [6]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Asag"
      en: "Asag"
      fr: "Asag"
      ja: "アサグ"
      cn: "阿萨格"
      ko: "아사그"
    enemy_id: "7522"
    id: "boss03"
    hp:
      - min: 4617812
      - max: 5887035
    attacks:
      - title:
          de: "Gebläse"
          en: "Bellows"
          fr: "Soufflet"
          ja: "ベローズ"
          cn: "长啸"
          ko: "풀무질"
        title_id: "2DB6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        damage:
          - min: 48
          - max: 1120
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Nichts-Blitzra"
          en: "Void Thunder II"
          fr: "Extra Foudre Du Néant"
          ja: "ヴォイド・サンダラ"
          cn: "虚空震雷"
          ko: "보이드 선더라"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DB7"
            damage_type: "Magical"
            damage:
              - min: 85
              - max: 18785
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DF0"
            damage_type: "Magical"
            damage:
              - min: 85
              - max: 5812
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Unknown_2DB3"
          en: "Unknown_2DB3"
          fr: "Unknown_2DB3"
          ja: "Unknown_2DB3"
          cn: "Unknown_2DB3"
          ko: "Unknown_2DB3"
        title_id: "2DB3"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
      - title:
          de: "Unknown_30D3"
          en: "Unknown_30D3"
          fr: "Unknown_30D3"
          ja: "Unknown_30D3"
          cn: "Unknown_30D3"
          ko: "Unknown_30D3"
        title_id: "30D3"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: 232
            icon: "015000/015026.png"
            name:
               de: "Markiert"
               en: "Prey"
               fr: "Marquage"
               ja: "マーキング"
               cn: "猎物"
               ko: "표식"
        phases:
          - phase: "09"
      - title:
          de: "Vollkontakt"
          en: "Body Blow"
          fr: "Coup Au Corps"
          ja: "ボディブロー"
          cn: "躯体冲撞"
          ko: "몸통 치기"
        title_id: "2DB4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 9858
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Markiert"
          en: "Prey"
          fr: "Marquage"
          ja: "マーキング"
          cn: "猎物"
          ko: "표식"
        title_id: "232"
        icon: "015000/015026_hr1.png"
        description:
          de: "Als Beute markiert."
          en: "Marked as prey."
          fr: "Fait l'objet du marquage d'un prédateur."
          ja: "獲物として狙いを付けられた状態。"
          cn: "作为猎物被盯上了"
          ko: "표적으로 점찍힌 상태."
        durations: [5]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Surabhi"
      en: "Surabhi"
      fr: "Surabhi"
      ja: "スラビー"
      cn: "苏罗毗"
      ko: "수라비"
    enemy_id: "7512"
    id: "boss16"
    hp:
      - min: 4131726
      - max: 6832201
    attacks:
      - title:
          de: "Aufspießen"
          en: "Heave"
          fr: "Soulèvement"
          ja: "しゃくり上げ"
          cn: "掀地"
          ko: "흐느낌"
        title_id: "2D34"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Erdbeben"
          en: "Earthquake"
          fr: "Tremblement De Terre"
          ja: "地震"
          cn: "地震"
          ko: "지진"
        title_id: "2D35"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 345
          - max: 15962
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Kampfgebrüll"
          en: "Disorienting Groan"
          fr: "Cri Désorientant"
          ja: "雄叫び"
          cn: "吼叫"
          ko: "우렁찬 외침"
        title_id: "2D30"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 60533
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Khoomii"
          en: "Khoomii"
          fr: "Khoomii"
          ja: "ホーミー"
          cn: "喉歌"
          ko: "불러들이기"
        title_id: "2D36"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 38
          - max: 1761
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D2F"
          en: "Unknown_2D2F"
          fr: "Unknown_2D2F"
          ja: "Unknown_2D2F"
          cn: "Unknown_2D2F"
          ko: "Unknown_2D2F"
        title_id: "2D2F"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
      - title:
          de: "Zerberstende Erde"
          en: "Crumbling Crust"
          fr: "Croûte Croulante"
          ja: "地盤崩し"
          cn: "地面崩裂"
          ko: "지반 붕괴"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D32"
            damage_type: "Darkness"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D33"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "König Athro"
      en: "King Arthro"
      fr: "Roi Arthro"
      ja: "キングアースロ"
      cn: "亚瑟罗王"
      ko: "아스로 왕"
    enemy_id: "7508"
    id: "boss13"
    hp:
      - min: 3963562
      - max: 3963562
    attacks:
      - title:
          de: "100 Fäuste"
          en: "Hundred Fists"
          fr: "Cent Poings"
          ja: "百烈拳"
          cn: "百烈拳"
          ko: "백렬권"
        title_id: "2D6D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Aquaga IV"
          en: "Waterga IV"
          fr: "Omnieau IV"
          ja: "ウォタガIv"
          cn: "狂水Iv"
          ko: "워터가 IV"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D69"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D6A"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Blubberblasen"
          en: "Bubble Shower"
          fr: "Averse De Bulles"
          ja: "バブルシャワー"
          cn: "泡泡吐息"
          ko: "거품 세례"
        title_id: "2D62"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Flut"
          en: "Flood"
          fr: "Déluge"
          ja: "フラッド"
          cn: "洪水"
          ko: "플러드"
        title_id: "2D65"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Giftga"
          en: "Poisonga II"
          fr: "Omnipoison"
          ja: "ポイゾガ"
          cn: "烈毒"
          ko: "포이즌가"
        title_id: "2D66"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Hochwasser"
          en: "High Water"
          fr: "Crue Des Eaux"
          ja: "ハイウォーター"
          cn: "高压水流"
          ko: "만조"
        title_id: "2D6E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Kleine Klaue"
          en: "Small Claw"
          fr: "Petite Griffe"
          ja: "スモールクロウ"
          cn: "小钢钳"
          ko: "작은 발톱"
        title_id: "2D6C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D60"
          en: "Unknown_2D60"
          fr: "Unknown_2D60"
          ja: "Unknown_2D60"
          cn: "Unknown_2D60"
          ko: "Unknown_2D60"
        title_id: "2D60"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 4833
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D67"
          en: "Unknown_2D67"
          fr: "Unknown_2D67"
          ja: "Unknown_2D67"
          cn: "Unknown_2D67"
          ko: "Unknown_2D67"
        title_id: "2D67"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "100 Fäuste"
          en: "Hundred Fists"
          fr: "Cent Poings"
          ja: "百烈拳"
          cn: "百烈拳"
          ko: "백렬권"
        title_id: "63A"
        icon: "015000/015008_hr1.png"
        description:
          de: "Intervalle von Auto-Attacke sowie die (Re-)Aktivierungszeit von Waffenfertigkeiten und Zaubern sind verkürzt."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are reduced."
          fr: "La vitesse des attaques automatiques est augmentée, et les temps de lancement et de recharge des sorts et des techniques d'arme sont réduits."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが短縮された状態。"
          cn: "自动攻击间隔缩短，同时战技与魔法的咏唱及复唱时间也会缩短"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 줄어든 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erweiterter Radius"
          en: "Area Of Influence Up"
          fr: "Aire D'Effet Augmentée"
          ja: "アクション効果範囲拡大"
          cn: "扩大技能效果范围"
          ko: "기술 범위 확대"
        title_id: "26A"
        icon: "017000/017981_hr1.png"
        description:
          de: "Der Wirkungsradius von Kommandos ist erweitert."
          en: "Area of effect for actions is expanded."
          fr: "L'aire d'effet des actions est agrandie."
          ja: "アクションの効果範囲が拡大した状態。"
          cn: "发动的技能效果范围比以往要大"
          ko: "기술의 효과 범위가 확대된 상태."
        durations: [60]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gewicht"
          en: "Heavy"
          fr: "Pesanteur"
          ja: "ヘヴィ"
          cn: "加重"
          ko: "과중력"
        title_id: "63B"
        icon: "015000/015002_hr1.png"
        description:
          de: "Bewegungsgeschwindigkeit ist verringert."
          en: "Movement speed is reduced."
          fr: "La vitesse de déplacement est réduite."
          ja: "移動速度が低下した状態。"
          cn: "移动速度降低"
          ko: "이동 속도가 느려진 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Heilsbringer"
      en: "Holy Cow"
      fr: "Vache Sacrée D'Eurêka"
      ja: "エウレカの聖牛"
      cn: "优雷卡圣牛"
      ko: "에우레카의 신성한 소"
    enemy_id: "7499"
    id: "boss09"
    hp:
      - min: 40616
      - max: 5160513
    attacks:
      - title:
          de: "Apotheose"
          en: "Apotheosis"
          fr: "Apothéose"
          ja: "神格化"
          cn: "神化"
          ko: "신격화"
        title_id: "2DD1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 1582
          - max: 25856
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Ausholen"
          en: "Swinge"
          fr: "Brutalité"
          ja: "スイング"
          cn: "重击"
          ko: "징벌"
        title_id: "2DCD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Kraftballung"
          en: "Bull Charge"
          fr: "Charge Bovine"
          ja: "ブルチャージ"
          cn: "猛牛冲锋"
          ko: "뿔소 돌진"
        title_id: "2DC7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 104
          - max: 7412
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Leucht-Guillotine"
          en: "Luminous Guillotine"
          fr: "Guillotine Étincelante"
          ja: "ルミナスギロチン"
          cn: "光辉断头台"
          ko: "빛나는 단두대"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DCB"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DCC"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Sanctus Antiquus"
          en: "Ancient Holy"
          fr: "Miracle Ancien"
          ja: "エンシェントホーリー"
          cn: "古代神圣"
          ko: "에인션트 홀리"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DCE"
            damage_type: "Slashing"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DD0"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Schramme"
          en: "Scratch"
          fr: "Griffade"
          ja: "スクラッチ"
          cn: "抓击"
          ko: "생채기"
        title_id: "2DC9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Schriller Schrei"
          en: "Shrill Shriek"
          fr: "Cri Légumier"
          ja: "絶叫"
          cn: "绝叫"
          ko: "절규"
        title_id: "2DC8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 60533
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2DC4"
          en: "Unknown_2DC4"
          fr: "Unknown_2DC4"
          ja: "Unknown_2DC4"
          cn: "Unknown_2DC4"
          ko: "Unknown_2DC4"
        title_id: "2DC4"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 61810
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2DC5"
          en: "Unknown_2DC5"
          fr: "Unknown_2DC5"
          ja: "Unknown_2DC5"
          cn: "Unknown_2DC5"
          ko: "Unknown_2DC5"
        title_id: "2DC5"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 7492
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2DCF"
          en: "Unknown_2DCF"
          fr: "Unknown_2DCF"
          ja: "Unknown_2DCF"
          cn: "Unknown_2DCF"
          ko: "Unknown_2DCF"
        title_id: "2DCF"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        damage:
          - min: 4
          - max: 4
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2DEF"
          en: "Unknown_2DEF"
          fr: "Unknown_2DEF"
          ja: "Unknown_2DEF"
          cn: "Unknown_2DEF"
          ko: "Unknown_2DEF"
        title_id: "2DEF"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Wiederkunft"
          en: "Advent"
          fr: "Avènement"
          ja: "降臨"
          cn: "降临"
          ko: "강림"
        title_id: "2DD2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 1424
          - max: 25541
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Hadhayosh"
      en: "Hadhayosh"
      fr: "Hadhayosh"
      ja: "ハダヨッシュ"
      cn: "哈达约什"
      ko: "하다요쉬"
    enemy_id: "7523"
    id: "boss07"
    attacks:
      - title:
          de: "Cometeor"
          en: "Cometeor"
          fr: "Cométéore"
          ja: "コメテオ"
          cn: "微型陨石"
          ko: "코메테오"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DA5"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DA6"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Cometeorschlag"
          en: "Cometeor Strike"
          fr: "Impact Cométéoritique"
          ja: "コメテオストライク"
          cn: "微型陨石重击"
          ko: "코메테오 스트라이크"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DA7"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DA8"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DA9"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Ekliptik-Meteor"
          en: "Ecliptic Meteor"
          fr: "Météore Écliptique"
          ja: "エクリプスメテオ"
          cn: "黄道陨石"
          ko: "황도 메테오"
        title_id: "2DAC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Fetzenreißer"
          en: "Trounce"
          fr: "Écrabouillement"
          ja: "トラウンス"
          cn: "呵斥"
          ko: "호령"
        title_id: "2DAF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Komet"
          en: "Comet"
          fr: "Comète"
          ja: "コメット"
          cn: "彗星"
          ko: "혜성"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DA3"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DA4"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DAD"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Strudel Des Todes"
          en: "Death Spin"
          fr: "Tourbillon Mortel"
          ja: "デススピン"
          cn: "死亡回旋"
          ko: "죽음의 회전"
        title_id: "2DB0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2DA2"
          en: "Unknown_2DA2"
          fr: "Unknown_2DA2"
          ja: "Unknown_2DA2"
          cn: "Unknown_2DA2"
          ko: "Unknown_2DA2"
        title_id: "2DA2"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
      - title:
          de: "Wilder Blitz"
          en: "Wild Bolt"
          fr: "Éclair Sauvage"
          ja: "ワイルドボルト"
          cn: "野性电击"
          ko: "사나운 번개"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2DAA"
            damage_type: "Blunt"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2DAB"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Wildes Horn"
          en: "Wild Horn"
          fr: "Corne Sauvage"
          ja: "ワイルドホーン"
          cn: "野性利角"
          ko: "사나운 뿔"
        title_id: "2DB1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Horus"
      en: "Horus"
      fr: "Horus"
      ja: "ホルス"
      cn: "荷鲁斯"
      ko: "호루스"
    enemy_id: "7524"
    id: "boss10"
    hp:
      - min: 42571
      - max: 5292931
    attacks:
      - title:
          de: "Feuerschweif"
          en: "Blazing Trail"
          fr: "Trace Embrasée"
          ja: "ブレイジングトレイル"
          cn: "炽热弥漫"
          ko: "작열지옥"
        title_id: "2D8A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Hitzschlag"
          en: "Sunstroke"
          fr: "Insolation"
          ja: "フレイムストーム"
          cn: "火焰风暴"
          ko: "화염 폭풍"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D83"
            damage_type: "Darkness"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D86"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 12398
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Höllenschmelze"
          en: "Liquid Hell"
          fr: "Enfer Liquide"
          ja: "ヘルリキッド"
          cn: "液体地狱"
          ko: "지옥의 늪"
        title_id: "2D89"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Sonneneruption"
          en: "Solar Flare"
          fr: "Éruption Solaire"
          ja: "ホットブラスト"
          cn: "炽热冲击"
          ko: "고온 폭발"
        title_id: "2D82"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Supereruption"
          en: "Superflare"
          fr: "Coup De Chaud"
          ja: "ヒートショック"
          cn: "热震荡"
          ko: "열기 충격"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D84"
            damage_type: "Blunt"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D87"
            damage_type: "Magical"
            add_status:
              - status: 63F
                icon: "015000/015647.png"
                name:
                 de: "Pyretisch"
                 en: "Pyretic"
                 fr: "Pyromanie"
                 ja: "ヒート"
                 cn: "热病"
                 ko: "열병"
            damage:
              - min: 582
              - max: 16630
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Todesurteil"
          en: "Death Sentence"
          fr: "Peine De Mort"
          ja: "デスセンテンス"
          cn: "死刑"
          ko: "사형 선고"
        title_id: "2D88"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_2D81"
          en: "Unknown_2D81"
          fr: "Unknown_2D81"
          ja: "Unknown_2D81"
          cn: "Unknown_2D81"
          ko: "Unknown_2D81"
        title_id: "2D81"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 20996
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Pyretisch"
          en: "Pyretic"
          fr: "Pyromanie"
          ja: "ヒート"
          cn: "热病"
          ko: "열병"
        title_id: "63F"
        icon: "015000/015647_hr1.png"
        description:
          de: "Der Körper ist extrem überhitzt. Jede Handlung verursacht Feuerschaden."
          en: "Fire-aspected damage is taken with every action."
          fr: "Des dégâts de feu sont subis après chaque action."
          ja: "身体が極度に熱せられた状態。何か行動をすると火属性ダメージを受ける。"
          cn: "身体极度发热，无论做出任何行动都会受到火属性伤害"
          ko: "몸이 극도로 뜨거워진 상태. 행동을 할 때마다 불속성 피해를 받는다."
        durations: [5]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Erz-Angra Mainyu"
      en: "Arch Angra Mainyu"
      fr: "Archi Angra Mainyu"
      ja: "アーチ・アンラ・マンユ"
      cn: "总领安哥拉·曼纽"
      ko: "우두머리 앙그라 마이뉴"
    enemy_id: "7495"
    id: "boss05"
    hp:
      - min: 27801
      - max: 5806021
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "DB4"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 196718
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Beschwörung"
          en: "Summon"
          fr: "Invocation"
          ja: "召喚"
          cn: "召唤"
          ko: "소환"
        title_id: "2D70"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Grässlicher Blick"
          en: "Ghastly Gaze"
          fr: "Regard Désemparant"
          ja: "喪失の妖気"
          cn: "丧失的妖气"
          ko: "상실의 요기"
        title_id: "2D7C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 58E
            icon: "015000/015809.png"
            name:
               de: "Plötzliche Panik"
               en: "Temporary Misdirection"
               fr: "Démence"
               ja: "心神喪失"
               cn: "精神失常"
               ko: "심신상실"
        damage:
          - min: 623
          - max: 53248
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Starren"
          en: "Stare"
          fr: "Regard Blême"
          ja: "怪光線"
          cn: "怪光线"
          ko: "괴이한 광선"
        title_id: "2D7E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Tödlicher Blick"
          en: "Mortal Gaze"
          fr: "Regard Mortel"
          ja: "死の視線"
          cn: "死亡视线"
          ko: "죽음의 시선"
        title_id: "2D7D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 38E
            icon: "015000/015503.png"
            name:
               de: "Verhängnis"
               en: "Doom"
               fr: "Glas"
               ja: "死の宣告"
               cn: "死亡宣告"
               ko: "죽음의 선고"
        damage:
          - min: 735
          - max: 15634
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Violetter Schleier"
          en: "Purple Haze"
          fr: "Nuage Violet"
          ja: "パープルヘイズ"
          cn: "紫烟"
          ko: "보랏빛 안개"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2D7A"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2D7B"
            damage_type: "Magical"
            damage:
              - min: 117
              - max: 11471
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
    debuffs:
      - title:
          de: "Plötzliche Panik"
          en: "Temporary Misdirection"
          fr: "Démence"
          ja: "心神喪失"
          cn: "精神失常"
          ko: "심신상실"
        title_id: "58E"
        icon: "015000/015809_hr1.png"
        description:
          de: "Von Furcht überwältigt. Läuft bei Bewegung in Richtung des Cursors."
          en: "Overcome with temporary insanity. Can only move in the direction indicated."
          fr: "Tout déplacement a lieu dans la direction vers laquelle pointe le curseur tournoyant au-dessus de la tête."
          ja: "心神喪失した状態。移動した際に、指カーソルの方向へ動いてしまう。"
          cn: "精神失常的状态，只能朝光标指示的方向移动"
          ko: "심신상실로 인하여 손가락 커서가 가리키는 방향을 따라 움직이게 되는 상태."
        durations: [11]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Verhängnis"
          en: "Doom"
          fr: "Glas"
          ja: "死の宣告"
          cn: "死亡宣告"
          ko: "죽음의 선고"
        title_id: "38E"
        icon: "015000/015503_hr1.png"
        description:
          de: "Wird kampfunfähig, sobald der Timer des Fluchs auf 0 sinkt."
          en: "Certain death when counter reaches zero."
          fr: "Mort certaine lorsque le compteur atteint zéro."
          ja: "カウント0で戦闘不能に陥る状態。"
          cn: "倒计时为0时会陷入无法战斗状态"
          ko: "지속시간이 끝나면 전투불능이 되는 상태."
        durations: [5]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Louhi"
      en: "Louhi"
      fr: "Louhi"
      ja: "ロウヒ"
      cn: "娄希"
      ko: "로우히"
    enemy_id: "7529"
    id: "boss14"
    hp:
      - min: 29756
      - max: 4652161
    attacks:
      - title:
          de: "Eisspalter Links"
          en: "Sinistral Icecleave"
          fr: "Fendage Glacé Sénestre"
          ja: "左辺氷刃斬"
          cn: "左侧冰刃斩"
          ko: "빙결 좌측베기"
        title_id: "2EA0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: CA
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 256
          - max: 37187
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Eisspalter Rechts"
          en: "Dextral Icecleave"
          fr: "Fendage Glacé Dextre"
          ja: "右辺氷刃斬"
          cn: "右侧冰刃斩"
          ko: "빙결 우측베기"
        title_id: "2E9F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: CA
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 7
          - max: 34165
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Kraft Des Willens"
          en: "Force Of Will"
          fr: "Force Mentale"
          ja: "妖気解放"
          cn: "妖气解放"
          ko: "요기 해방"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2EAB"
            damage_type: "Blunt"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EAC"
            damage_type: "Magical"
            damage:
              - min: 105
              - max: 10798
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Kältetritt"
          en: "Cold Kick"
          fr: "Coup De Pied Gelé"
          ja: "冷脚"
          cn: "冻踢"
          ko: "냉각"
        title_id: "2EA1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        add_status:
          - status: CA
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 7
          - max: 37374
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schneeblüte"
          en: "Snowbloom"
          fr: "Éclosion Neigeuse"
          ja: "雪華斬"
          cn: "雪华斩"
          ko: "눈꽃 베기"
        title_id: "2EA3"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: CA
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 487
          - max: 12521
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Schneewirbel"
          en: "Snowstorm"
          fr: "Tourmente De Neige"
          ja: "暴風雪"
          cn: "暴风雪"
          ko: "폭풍설"
        title_id: "2EA2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 25064
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Unknown_213F"
          en: "Unknown_213F"
          fr: "Unknown_213F"
          ja: "Unknown_213F"
          cn: "Unknown_213F"
          ko: "Unknown_213F"
        title_id: "213F"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E9C"
          en: "Unknown_2E9C"
          fr: "Unknown_2E9C"
          ja: "Unknown_2E9C"
          cn: "Unknown_2E9C"
          ko: "Unknown_2E9C"
        title_id: "2E9C"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 61809
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E9D"
          en: "Unknown_2E9D"
          fr: "Unknown_2E9D"
          ja: "Unknown_2E9D"
          cn: "Unknown_2E9D"
          ko: "Unknown_2E9D"
        title_id: "2E9D"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E9E"
          en: "Unknown_2E9E"
          fr: "Unknown_2E9E"
          ja: "Unknown_2E9E"
          cn: "Unknown_2E9E"
          ko: "Unknown_2E9E"
        title_id: "2E9E"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unterkühlung"
          en: "Supercool"
          fr: "Surfusion"
          ja: "過冷却"
          cn: "过冷却"
          ko: "과냉각"
        title_id: "2EA5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Windpeitsche"
          en: "Windlash"
          fr: "Froid Mordant"
          ja: "寒風旋"
          cn: "寒风旋"
          ko: "찬바람 회전"
        title_id: "2EA4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: CA
            icon: "017000/017101.png"
            name:
               de: "Erhöhte Verwundbarkeit"
               en: "Vulnerability Up"
               fr: "Vulnérabilité augmentée"
               ja: "被ダメージ上昇"
               cn: "受伤加重"
               ko: "받는 피해량 증가"
        damage:
          - min: 1892
          - max: 33586
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "CA"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [6]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Zahn Um Zahn"
          en: "Blood For Blood"
          fr: "Du Sang Pour Du Sang"
          ja: "捨身"
          cn: "舍身"
          ko: "필사의 각오"
        title_id: "75"
        icon: "010000/010304_hr1.png"
        description:
          de: "Ausgeteilter und erlittener Schaden sind erhöht. "
          en: "Damage dealt and damage taken are increased."
          fr: "Les dégâts infligés et reçus sont augmentés."
          ja: "与ダメージが上昇する代わりに被ダメージも上昇している状態。"
          cn: "攻击所造成的伤害提高，受到攻击的伤害也会增加"
          ko: "적에게 주는 피해량이 증가하지만, 자신이 받는 피해량도 증가하는 상태."
        durations: [60]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
  - title:
      de: "Unbekannte Herkunft"
      en: "Unknown Source"
      fr: "Unknown Source"
      ja: "Unknown Source"
      cn: "Unknown Source"
      ko: "Unknown Source"
    enemy_id: "7469"
    id: "boss01"
    attacks:
      - title:
          de: "111-Tonzen-Schwung"
          en: "111-Tonze Swing"
          fr: "Swing De 111 Tonz"
          ja: "111トンズ・スイング"
          cn: "百十一吨回转"
          ko: "111톤즈 휘두르기"
        title_id: "2E12"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F80"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Anemochorie"
          en: "Anemochory"
          fr: "Ébarbement Balistique"
          ja: "バリスティックバリ"
          cn: "刺边"
          ko: "탄도 드릴"
        title_id: "2DFC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2EBC"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2ED9"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EDB"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EE1"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EE3"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EE6"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EE7"
            damage_type: "None"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EE9"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EEA"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EEB"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EEC"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EED"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EEE"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EEF"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EF3"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EF4"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EF6"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EF8"
            damage_type: "Piercing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EF9"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EFC"
            damage_type: "None"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EFD"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2EFF"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F01"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F04"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F05"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F06"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F08"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F09"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F0B"
            damage_type: "None"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F0C"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F0D"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F0F"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F11"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F13"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F14"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F17"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F18"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F19"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F1A"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F1B"
            damage_type: "Piercing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F1C"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F1D"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "542"
            damage_type: "Magical"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "543"
            damage_type: "Magical"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Blitz"
          en: "Thunder"
          fr: "Foudre"
          ja: "サンダー"
          cn: "闪雷"
          ko: "선더"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2E58"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2E60"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Blubberblasen"
          en: "Bubble Shower"
          fr: "Averse De Bulles"
          ja: "バブルシャワー"
          cn: "泡泡吐息"
          ko: "거품 세례"
        title_id: "2E2E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Carpomission"
          en: "Carpomission"
          fr: "Carpomission"
          ja: "カルポミッション"
          cn: "石腕飞拳"
          ko: "맹속 타격"
        title_id: "2E83"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Dreischnitt"
          en: "Triclip"
          fr: "Triclip"
          ja: "トライクリップ"
          cn: "三连爪"
          ko: "삼단베기"
        title_id: "2E76"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Eis"
          en: "Blizzard"
          fr: "Glace"
          ja: "ブリザド"
          cn: "冰结"
          ko: "블리자드"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2E5B"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2F6C"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Eiszyklon"
          en: "Chilling Cyclone"
          fr: "Cyclone Glaçant"
          ja: "チリングサイクロン"
          cn: "寒颤旋风"
          ko: "싸늘한 폭풍"
        title_id: "2D80"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Erhobene Schere"
          en: "Standing Chine"
          fr: "Échine Verticale"
          ja: "スタンディングチャイン"
          cn: "站裂斩"
          ko: "세로베기"
        title_id: "2DDF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Felsschleuder"
          en: "Boulder Toss"
          fr: "Jet De Pierre"
          ja: "投石"
          cn: "投石"
          ko: "투석"
        title_id: "2DE2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Fesselklinge"
          en: "Spellsword"
          fr: "Lame Occulte"
          ja: "呪縛斬"
          cn: "咒缚斩"
          ko: "저주의 칼날"
        title_id: "2E72"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Feuer"
          en: "Fire"
          fr: "Feu"
          ja: "ファイア"
          cn: "火炎"
          ko: "파이어"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        notes:
          - note: "Variation-note BIG"
        variation:
          - title_id: "2E5A"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
          - title_id: "2E5E"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
            notes:
              - note: "Variation-note 1"
      - title:
          de: "Feuerball"
          en: "Fireball"
          fr: "Boule De Feu"
          ja: "ファイアーボール"
          cn: "火球"
          ko: "화염구"
        title_id: "2DBB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Flügelschlag"
          en: "Bearclaw"
          fr: "Griffe D'Ours"
          ja: "ベアクロウ"
          cn: "熊爪"
          ko: "곰발톱"
        title_id: "2DF5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Geisterhauch"
          en: "Breath Of The Wild Ones"
          fr: "Souffle Des Esprits"
          ja: "精霊の息吹"
          cn: "元灵的气息"
          ko: "정령의 숨결"
        title_id: "2746"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Geladene Salve"
          en: "Charged Volley Fire"
          fr: "Tir De Volée Chargé"
          ja: "チャージドファイア"
          cn: "蓄能齐射"
          ko: "충전 포격"
        title_id: "E04"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Giftga"
          en: "Poisonga II"
          fr: "Omnipoison"
          ja: "ポイゾガ"
          cn: "烈毒"
          ko: "포이즌가"
        title_id: "2D68"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Hypothermale Verbrennung"
          en: "Hypothermal Combustion"
          fr: "Combustion Hypothermique"
          ja: "砕氷"
          cn: "冰碎"
          ko: "쇄빙"
        title_id: "2D2E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Kohlkopfstoß"
          en: "Ram"
          fr: "Coup De Chou"
          ja: "頭突き"
          cn: "撞击"
          ko: "머리 들이받기"
        title_id: "2DEC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Komet"
          en: "Comet"
          fr: "Comète"
          ja: "コメット"
          cn: "彗星"
          ko: "혜성"
        title_id: "2DAD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Kraft Des Willens"
          en: "Force Of Will"
          fr: "Force Mentale"
          ja: "妖気解放"
          cn: "妖气解放"
          ko: "요기 해방"
        title_id: "2EAC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Lauerndes Fangbein"
          en: "Kneeling Snath"
          fr: "Faucille Agenouillée"
          ja: "ニーリングシックル"
          cn: "俯跪冲"
          ko: "거대 낫질"
        title_id: "2DE0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Lebende Fackel"
          en: "Live Pyre"
          fr: "Bûcher Morbide"
          ja: "大凶の炎"
          cn: "大凶之炎"
          ko: "대흉의 불꽃"
        title_id: "2D94"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Lumisphäre"
          en: "Lumisphere"
          fr: "Lumisphère"
          ja: "ルミスフィア"
          cn: "光球"
          ko: "빛의 구체"
        title_id: "2E26"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Modrige Bandage"
          en: "Rotting Bandages"
          fr: "Bandages En Putréfaction"
          ja: "ロッティング・バンデージ"
          cn: "腐尸绷带"
          ko: "썩은 붕대"
        title_id: "2E8B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2EB5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Prankenhieb"
          en: "Rake"
          fr: "Griffes"
          ja: "ひっかき"
          cn: "利爪"
          ko: "할퀴기"
        title_id: "2E41"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Rippenstoß"
          en: "Elbow Drop"
          fr: "Descente De Coude"
          ja: "エルボードロップ"
          cn: "落肘"
          ko: "팔꿈치 찍기"
        title_id: "2DBF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Rostbiss"
          en: "Rustbite"
          fr: "Morsure Corrosive"
          ja: "ラスティバイト"
          cn: "锈咬"
          ko: "녹슨 화살"
        title_id: "2E87"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Ruhm"
          en: "Glory"
          fr: "Gloire"
          ja: "グローリーブレード"
          cn: "荣耀之剑"
          ko: "영광의 검"
        title_id: "2DCA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Salve"
          en: "Volley Fire"
          fr: "Tir De Volée"
          ja: "ボレーファイア"
          cn: "齐射"
          ko: "집중 사격"
        title_id: "B4B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Sandsäule"
          en: "Sand Pillar"
          fr: "Pilier De Sable"
          ja: "サンドピラー"
          cn: "沙柱"
          ko: "모래기둥"
        title_id: "2D44"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Schneegebirge"
          en: "Snowcap"
          fr: "Chaîne Glaciaire"
          ja: "大雪山"
          cn: "大雪山"
          ko: "대설산"
        title_id: "2D9D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Schneeregen"
          en: "Flurry"
          fr: "Averse De Neige"
          ja: "雪時雨"
          cn: "雨夹雪"
          ko: "진눈깨비"
        title_id: "2D2C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Seismischer Bruch"
          en: "Seismic Rift"
          fr: "Fissure Sismique"
          ja: "サイズミックリフト"
          cn: "地层断裂"
          ko: "지진 균열"
        title_id: "2E29"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Seismischer Schrei"
          en: "Seismic Scream"
          fr: "Cri Sismique"
          ja: "サイズミックスクリーム"
          cn: "地震嚎"
          ko: "괴성"
        title_id: "2DEA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Starrender Blick"
          en: "Quarry Lake"
          fr: "Œil Pétrifiant"
          ja: "石化の瞳"
          cn: "石化之瞳"
          ko: "석화의 눈동자"
        title_id: "2DE6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Steinwurf"
          en: "Stone Throw"
          fr: "Lapidation"
          ja: "つぶて打ち"
          cn: "投石"
          ko: "돌 던지기"
        title_id: "2DE1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Säuleneinschlag"
          en: "Pillar Impact"
          fr: "Chute"
          ja: "崩落"
          cn: "崩落"
          ko: "고드름 낙하"
        title_id: "2EA9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D2A"
          en: "Unknown_2D2A"
          fr: "Unknown_2D2A"
          ja: "Unknown_2D2A"
          cn: "Unknown_2D2A"
          ko: "Unknown_2D2A"
        title_id: "2D2A"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D42"
          en: "Unknown_2D42"
          fr: "Unknown_2D42"
          ja: "Unknown_2D42"
          cn: "Unknown_2D42"
          ko: "Unknown_2D42"
        title_id: "2D42"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E67"
          en: "Unknown_2E67"
          fr: "Unknown_2E67"
          ja: "Unknown_2E67"
          cn: "Unknown_2E67"
          ko: "Unknown_2E67"
        title_id: "2E67"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E68"
          en: "Unknown_2E68"
          fr: "Unknown_2E68"
          ja: "Unknown_2E68"
          cn: "Unknown_2E68"
          ko: "Unknown_2E68"
        title_id: "2E68"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2E8D"
          en: "Unknown_2E8D"
          fr: "Unknown_2E8D"
          ja: "Unknown_2E8D"
          cn: "Unknown_2E8D"
          ko: "Unknown_2E8D"
        title_id: "2E8D"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_3104"
          en: "Unknown_3104"
          fr: "Unknown_3104"
          ja: "Unknown_3104"
          cn: "Unknown_3104"
          ko: "Unknown_3104"
        title_id: "3104"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_3118"
          en: "Unknown_3118"
          fr: "Unknown_3118"
          ja: "Unknown_3118"
          cn: "Unknown_3118"
          ko: "Unknown_3118"
        title_id: "3118"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
      - title:
          de: "Wilder Blitz"
          en: "Wild Bolt"
          fr: "Éclair Sauvage"
          ja: "ワイルドボルト"
          cn: "野性电击"
          ko: "사나운 번개"
        title_id: "2DAB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Wirbelkick"
          en: "Spinkick"
          fr: "Coup De Pied Circulaire"
          ja: "回し蹴り"
          cn: "回旋踢"
          ko: "돌려차기"
        title_id: "2E6F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
      - title:
          de: "Zerberstende Erde"
          en: "Crumbling Crust"
          fr: "Croûte Croulante"
          ja: "地盤崩し"
          cn: "地面崩裂"
          ko: "지반 붕괴"
        title_id: "2D33"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
        notes:
          - note: "note"
      - title:
          de: "Zufrieren"
          en: "Bergy Bit"
          fr: "Gelée"
          ja: "結氷"
          cn: "结冰"
          ko: "결빙"
        title_id: "2DA1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
        notes:
          - note: "note"
    debuffs:
      - title:
          de: "Ausweich-Malus"
          en: "Evasion Down"
          fr: "Malus D'Esquive"
          ja: "回避力ダウン"
          cn: "回避力降低"
          ko: "회피력 감소"
        title_id: "20"
        icon: "015000/015063_hr1.png"
        description:
          de: "Die Fähigkeit des Ausweichens ist verringert."
          en: "Evasion is reduced."
          fr: "L'esquive est réduite."
          ja: "回避力が低下した状態。"
          cn: "回避力减弱"
          ko: "회피력이 떨어진 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "02"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        durations: [1, 2]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Blind"
          en: "Blind"
          fr: "Cécité"
          ja: "暗闇"
          cn: "失明"
          ko: "실명"
        title_id: "0F"
        icon: "015000/015012_hr1.png"
        description:
          de: "Vorübergehende Blindheit senkt die Präzision."
          en: "Encroaching darkness is lowering accuracy."
          fr: "Un voile d'obscurité diminue la précision."
          ja: "闇に囚われ、命中率が低下した状態。"
          cn: "陷于黑暗之中，命中率降低"
          ko: "어둠에 사로잡혀 명중률이 떨어진 상태."
        durations: [3, 7, 8, 15, 30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Bonus Für Elementar-Ep"
          en: "Elemental Exp Up"
          fr: "Bonus D'Expérience Élémentaire"
          ja: "エレメンタル経験値アップ"
          cn: "元素经验值提高"
          ko: "엘리멘탈 경험치 증가"
        title_id: "6C4"
        icon: "015000/015836_hr1.png"
        description:
          de: "In Kämpfen erlangte Elementar-EP sind erhöht, solange die Elementarstufe weniger als 35 beträgt."
          en: "Elemental EXP earned through battle while at an elemental level lower than 35 is increased."
          fr: "En dessous du niveau élémentaire 35, l'expérience élémentaire obtenue en abattant des monstres est augmentée."
          ja: "エレメンタルレベル35未満で獲得できる、敵討伐時の獲得エレメンタル経験値が増加する状態。"
          cn: "打倒敌人时获得的元素经验值增加，元素等级不满35级时可获得此状态"
          ko: "엘리멘탈레벨이 35레벨 미만일 때, 적 처치 시 얻는 엘리멘탈 경험치가 증가하는 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Elementarer Segen: Routine"
          en: "Elemental Blessing: Exp"
          fr: "Bénédiction Des Esprits : Bonus D'Expérience"
          ja: "精霊の加護：経験値アップ"
          cn: "元灵加护：经验值提高"
          ko: "정령의 가호: 경험치 증가"
        title_id: "5B5"
        icon: "015000/015887_hr1.png"
        description:
          de: "Hat den Segen der Elemente von Eureka erhalten. Die Elementarroutine beim Besiegen von Feinden ist erhöht."
          en: "Receiving the blessing of Eureka's elementals. Elemental EXP earned through battle is increased."
          fr: "Les élémentaires d'Eurêka accordent leur bénédiction. L'expérience élémentaire obtenue au combat est augmentée."
          ja: "エウレカエレメンタルの加護を受けた状態。敵討伐時の獲得エレメンタル経験値が増加する。"
          cn: "受到了优雷卡元灵的加护，打倒敌人时获得的元素经验值增加"
          ko: "에우레카 정령의 가호를 받은 상태. 적 처치 시 얻는 엘리멘탈 경험치가 증가한다."
        durations: [3600]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Elementarer Segen: Schaden"
          en: "Elemental Blessing: Damage"
          fr: "Bénédiction Des Esprits : Bonus De Combativité"
          ja: "精霊の加護：戦闘能力アップ"
          cn: "元灵加护：战斗力提高"
          ko: "정령의 가호: 전투 능력 향상"
        title_id: "5B7"
        icon: "015000/015835_hr1.png"
        description:
          de: "Hat den Segen der Elemente von Eureka erhalten. Ausgeteilter Schaden ist erhöht und LP werden über Zeit wiederhergestellt."
          en: "Receiving the blessing of Eureka's elementals. Damage dealt is increased and restoring HP over time."
          fr: "Les élémentaires d'Eurêka accordent leur bénédiction. Les dégâts infligés sont augmentés et des PV sont récupérés périodiquement."
          ja: "エウレカエレメンタルの加護を受けた状態。与えるダメージが上昇し、ＨＰが徐々に回復する。"
          cn: "受到了优雷卡元灵的加护，攻击所造成的伤害提高，体力会随时间逐渐恢复"
          ko: "에우레카 정령의 가호를 받은 상태. 주는 피해량이 증가하고 HP가 서서히 회복된다."
        durations: [3600]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Magie-Verwundbarkeit"
          en: "Magic Vulnerability Up"
          fr: "Vulnérabilité Magique Augmentée"
          ja: "被魔法ダメージ増加"
          cn: "魔法受伤加重"
          ko: "받는 마법 피해량 증가"
        title_id: "1EE"
        icon: "015000/015057_hr1.png"
        description:
          de: "Erlittener Magieschaden ist erhöht."
          en: "Magic damage taken is increased."
          fr: "Les dégâts magiques reçus sont augmentés."
          ja: "受ける魔法ダメージが増加する状態。"
          cn: "受到魔法攻击的伤害增加"
          ko: "마법 공격으로 받는 피해량이 증가하는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Physische Verwundbarkeit"
          en: "Physical Vulnerability Up"
          fr: "Vulnérabilité Physique Augmentée"
          ja: "被物理ダメージ増加"
          cn: "物理受伤加重"
          ko: "받는 물리 피해량 증가"
        title_id: "1ED"
        icon: "015000/015053_hr1.png"
        description:
          de: "Erlittener physischer Schaden ist erhöht."
          en: "Physical damage taken is increased."
          fr: "Les dégâts physiques reçus sont augmentés."
          ja: "受ける物理ダメージが増加する状態。"
          cn: "受到物理攻击的伤害增加"
          ko: "물리 공격으로 받는 피해량이 증가하는 상태."
        durations: [4]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "4B8"
        icon: "015000/015020_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [10]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "CA"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [2]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gaias Zorn"
          en: "Razed Earth"
          fr: "Fureur Tellurique"
          ja: "大地の怒り"
          cn: "大地之怒"
          ko: "대지의 분노"
        title_id: "138"
        icon: "012000/012702_hr1.png"
        description:
          de: "Hält eine erdbasierte Schadenszone aufrecht."
          en: "Jagged shards protrude from the ground, dealing earth damage to any who tread upon them."
          fr: "Des dégâts magiques de terre sont infligés aux ennemis alentour."
          ja: "土属性のダメージエリアを発生させている状態。"
          cn: "产生土属性攻击区域"
          ko: "땅속성 지속 피해 영역을 발생시킨 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gebrechlichkeit"
          en: "Infirmity"
          fr: "Infirmité"
          ja: "虚弱"
          cn: "虚弱"
          ko: "허약"
        title_id: "AC"
        icon: "015000/015531_hr1.png"
        description:
          de: "Wirkung von Heilmagie auf sich ist verringert. "
          en: "HP recovery via healing magic is reduced."
          fr: "L'effet des sorts de restauration des PV est réduit."
          ja: "回復魔法による自身に対するＨＰ回復効果が低下した状態。"
          cn: "自身所受的治疗魔法效果降低"
          ko: "자신이 받는 회복마법의 HP 회복 효과가 떨어진 상태."
        durations: [8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "09"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [10, 20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "231"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gewicht"
          en: "Heavy"
          fr: "Pesanteur"
          ja: "ヘヴィ"
          cn: "加重"
          ko: "과중력"
        title_id: "0E"
        icon: "015000/015002_hr1.png"
        description:
          de: "Bewegungsgeschwindigkeit ist verringert."
          en: "Movement speed is reduced."
          fr: "La vitesse de déplacement est réduite."
          ja: "移動速度が低下した状態。"
          cn: "移动速度降低"
          ko: "이동 속도가 느려진 상태."
        durations: [10]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Glühender Schild"
          en: "Radiant Shield"
          fr: "Bouclier Radiant"
          ja: "光輝の盾"
          cn: "辉煌盾"
          ko: "광휘의 방패"
        title_id: "139"
        icon: "012000/012711_hr1.png"
        description:
          de: "Verursacht bei erlittenem Angriff Schaden beim Angreifer."
          en: "Increasing physical damage taken by attackers."
          fr: "Les attaques assénées par un assaillant augmentent les dégâts physiques que celui-ci reçoit."
          ja: "攻撃を受けた場合に、攻撃者に被物理ダメージ上昇効果を与える状態。"
          cn: "受到攻击时对攻击方附加状态，令其所受物理伤害提高"
          ko: "공격을 받으면 적에게 물리 공격으로 받는 피해량 증가 효과를 부여하는 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Lp-Malus"
          en: "Hp Penalty"
          fr: "Malus De Pv"
          ja: "最大Ｈｐダウン"
          cn: "体力衰减"
          ko: "최대 Hp 감소"
        title_id: "16"
        icon: "015000/015076_hr1.png"
        description:
          de: "Maximal-LP sind verringert."
          en: "Maximum HP is reduced."
          fr: "Les PV maximums sont réduits."
          ja: "最大ＨＰが減少した状態。"
          cn: "体力最大值降低"
          ko: "HP 최대치가 감소한 상태."
        durations: [30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Nausea"
          en: "Nausea"
          fr: "Nausée"
          ja: "吐き気"
          cn: "呕吐"
          ko: "구역질"
        title_id: "954"
        icon: "015000/015274_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht, ausgeteilter Schaden und Maximal-LP verringert."
          en: "Feeling as if you might vomit at any moment. Damage dealt and maximum HP are decreased."
          fr: "Une tenace envie de vomir réduit les dégâts infligés et les PV maximums."
          ja: "吐き気を感じている状態。与ダメージと最大ＨＰが低下する。"
          cn: "想要呕吐，攻击所造成的伤害降低，体力最大值降低"
          ko: "구역질이 나는 상태. 주는 피해량과 최대 HP가 감소한다."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Präzisions-Malus"
          en: "Accuracy Down"
          fr: "Malus De Précision"
          ja: "命中率ダウン"
          cn: "命中率降低"
          ko: "명중률 감소"
        title_id: "1C"
        icon: "015000/015061_hr1.png"
        description:
          de: "Die Präzision ist verringert."
          en: "Accuracy is reduced."
          fr: "La précision est réduite."
          ja: "命中率が低下した状態。"
          cn: "命中率减弱"
          ko: "명중률이 떨어진 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Pyretisch"
          en: "Pyretic"
          fr: "Pyromanie"
          ja: "ヒート"
          cn: "热病"
          ko: "열병"
        title_id: "63F"
        icon: "015000/015647_hr1.png"
        description:
          de: "Der Körper ist extrem überhitzt. Jede Handlung verursacht Feuerschaden."
          en: "Fire-aspected damage is taken with every action."
          fr: "Des dégâts de feu sont subis après chaque action."
          ja: "身体が極度に熱せられた状態。何か行動をすると火属性ダメージを受ける。"
          cn: "身体极度发热，无论做出任何行动都会受到火属性伤害"
          ko: "몸이 극도로 뜨거워진 상태. 행동을 할 때마다 불속성 피해를 받는다."
        durations: [5]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Stein"
          en: "Petrification"
          fr: "Pétrification"
          ja: "石化"
          cn: "石化"
          ko: "석화"
        title_id: "5E7"
        icon: "015000/015001_hr1.png"
        description:
          de: "Die Versteinerung des Körpers verhindert das Ausführen jeglicher Kommandos."
          en: "Stone-like rigidity is preventing the execution of actions."
          fr: "Une rigidité semblable à celle de la pierre empêche toute action."
          ja: "手足が石化し、行動できなくなった状態。"
          cn: "手足被石化，无法做出任何行动"
          ko: "손발이 석화되어 행동이 불가능한 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Stumm"
          en: "Silence"
          fr: "Silence"
          ja: "沈黙"
          cn: "沉默"
          ko: "침묵"
        title_id: "07"
        icon: "015000/015005_hr1.png"
        description:
          de: "Zauber können nicht gewirkt werden."
          en: "A stifling magic is preventing casts."
          fr: "Le lancement de sorts est impossible."
          ja: "魔法を詠唱できなくなった状態。"
          cn: "无法咏唱魔法"
          ko: "마법을 시전할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [12]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Verbrennung"
          en: "Suppuration"
          fr: "Morsure Du Feu"
          ja: "熱傷"
          cn: "灼伤"
          ko: "열화상"
        title_id: "177"
        icon: "017000/017181_hr1.png"
        description:
          de: "Maximale LP sind verringert, erlittener Schaden ist erhöht."
          en: "Maximum HP is reduced and damage taken is increased."
          fr: "Les PV maximums sont réduits et les dégâts reçus sont augmentés."
          ja: "最大HPが低下し、受けるダメージが増加する状態。"
          cn: "最大体力降低，受到攻击的伤害增加"
          ko: "HP 최대치가 감소하고, 받는 피해량이 증가하는 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
        name: "phase_name"
        alerts:
          - alert: "Die folgenden Angriffe haben sind entweder unbekannt oder haben keine klare Herkunft"
        mechanics:
          - title: "sequence-mechanic-01"
            notes:
              - note: "sequence-mechanic-note-01"
        attacks:
          - attack: "sequence-attack-01"
        images:
          - url: "/assets/img/test.jpg"
            alt: "/assets/img/test.jpg"
            height: "250px"
        videos:
          - url: "https&#58;//ffxivguide.akurosia.de/upload/test.mp4"
adds:
  - title:
      de: "Alter Büffel"
      en: "Elder Buffalo"
      fr: "Buffle Ancien"
      ja: "エルダーバッファロー"
      cn: "古老水牛"
      ko: "장로 물소"
    enemy_id: "7447"
    id: "add01"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F06"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 113
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Seismischer Bruch"
          en: "Seismic Rift"
          fr: "Fissure Sismique"
          ja: "サイズミックリフト"
          cn: "地层断裂"
          ko: "지진 균열"
        title_id: "2E29"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Ameretat"
      en: "Ameretat"
      fr: "Ameretât"
      ja: "アムルタート"
      cn: "阿米雷戴"
      ko: "아메레타트"
    enemy_id: "7462"
    id: "add02"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F15"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Rankenstich"
          en: "Vine Probe"
          fr: "Sarment Empalant"
          ja: "串刺し"
          cn: "藤鞭穿刺"
          ko: "꼬챙이"
        title_id: "2E44"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 61810
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Schlechter Atem"
          en: "Bad Breath"
          fr: "Mauvaise Haleine"
          ja: "臭い息"
          cn: "臭气"
          ko: "역한 입냄새"
        title_id: "2E43"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 96
          - max: 127
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Blind"
          en: "Blind"
          fr: "Cécité"
          ja: "暗闇"
          cn: "失明"
          ko: "실명"
        title_id: "0F"
        icon: "015000/015012_hr1.png"
        description:
          de: "Vorübergehende Blindheit senkt die Präzision."
          en: "Encroaching darkness is lowering accuracy."
          fr: "Un voile d'obscurité diminue la précision."
          ja: "闇に囚われ、命中率が低下した状態。"
          cn: "陷于黑暗之中，命中率降低"
          ko: "어둠에 사로잡혀 명중률이 떨어진 상태."
        durations: [15, 30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "09"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [10, 20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gewicht"
          en: "Heavy"
          fr: "Pesanteur"
          ja: "ヘヴィ"
          cn: "加重"
          ko: "과중력"
        title_id: "0E"
        icon: "015000/015002_hr1.png"
        description:
          de: "Bewegungsgeschwindigkeit ist verringert."
          en: "Movement speed is reduced."
          fr: "La vitesse de déplacement est réduite."
          ja: "移動速度が低下した状態。"
          cn: "移动速度降低"
          ko: "이동 속도가 느려진 상태."
        durations: [10]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Lp-Malus"
          en: "Hp Penalty"
          fr: "Malus De Pv"
          ja: "最大Ｈｐダウン"
          cn: "体力衰减"
          ko: "최대 Hp 감소"
        title_id: "16"
        icon: "015000/015076_hr1.png"
        description:
          de: "Maximal-LP sind verringert."
          en: "Maximum HP is reduced."
          fr: "Les PV maximums sont réduits."
          ja: "最大ＨＰが減少した状態。"
          cn: "体力最大值降低"
          ko: "HP 최대치가 감소한 상태."
        durations: [30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Nausea"
          en: "Nausea"
          fr: "Nausée"
          ja: "吐き気"
          cn: "呕吐"
          ko: "구역질"
        title_id: "954"
        icon: "015000/015274_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht, ausgeteilter Schaden und Maximal-LP verringert."
          en: "Feeling as if you might vomit at any moment. Damage dealt and maximum HP are decreased."
          fr: "Une tenace envie de vomir réduit les dégâts infligés et les PV maximums."
          ja: "吐き気を感じている状態。与ダメージと最大ＨＰが低下する。"
          cn: "想要呕吐，攻击所造成的伤害降低，体力最大值降低"
          ko: "구역질이 나는 상태. 주는 피해량과 최대 HP가 감소한다."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Schaden -"
          en: "Damage Down"
          fr: "Malus De Dégâts"
          ja: "ダメージ低下"
          cn: "伤害降低"
          ko: "주는 피해량 감소"
        title_id: "3E"
        icon: "015000/015520_hr1.png"
        description:
          de: "Ausgeteilter Schaden ist verringert."
          en: "Damage dealt is reduced."
          fr: "Les dégâts infligés sont réduits."
          ja: "与ダメージが低下した状態。"
          cn: "攻击所造成的伤害降低"
          ko: "적에게 주는 피해량이 감소하는 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Stumm"
          en: "Silence"
          fr: "Silence"
          ja: "沈黙"
          cn: "沉默"
          ko: "침묵"
        title_id: "07"
        icon: "015000/015005_hr1.png"
        description:
          de: "Zauber können nicht gewirkt werden."
          en: "A stifling magic is preventing casts."
          fr: "Le lancement de sorts est impossible."
          ja: "魔法を詠唱できなくなった状態。"
          cn: "无法咏唱魔法"
          ko: "마법을 시전할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Anapo-Elementar"
      en: "Anapos Elemental"
      fr: "Élémentaire D'Anapos"
      ja: "アナポ・エレメンタル"
      cn: "安娜波元精"
      ko: "아나포 정령"
    enemy_id: "7498"
    id: "add03"
    attacks:
      - title:
          de: "Flut"
          en: "Flood"
          fr: "Déluge"
          ja: "フラッド"
          cn: "洪水"
          ko: "플러드"
        title_id: "2D4B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Anila"
      en: "Anila"
      fr: "Anila"
      ja: "アニラ"
      cn: "阿尼罗"
      ko: "아닐라"
    enemy_id: "7506"
    id: "add04"
    hp:
      - min: 305185
      - max: 426585
    attacks:
      - title:
          de: "Antiker Wind"
          en: "Ancient Aero"
          fr: "Vent Ancien"
          ja: "エンシェントエアロ"
          cn: "古代疾风"
          ko: "에인션트 에어로"
        title_id: "2E55"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 53
          - max: 8361
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2E54"
          en: "Unknown_2E54"
          fr: "Unknown_2E54"
          ja: "Unknown_2E54"
          cn: "Unknown_2E54"
          ko: "Unknown_2E54"
        title_id: "2E54"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 53
          - max: 22600
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Arkane Sphäre"
      en: "Arcane Sphere"
      fr: "Sphère Arcanique"
      ja: "立体魔法陣"
      cn: "立体魔法阵"
      ko: "입체마법진"
    enemy_id: "7502"
    id: "add05"
    hp:
      - min: 99553
      - max: 99553
    sequence:
      - phase: "09"
  - title:
      de: "Bernstein-Exergon"
      en: "Ember Sprite"
      fr: "Élémentaire D'Ambre"
      ja: "エンバースプライト"
      cn: "余烬元精"
      ko: "잉걸불 정령"
    enemy_id: "7472"
    id: "add06"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F7B"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2F7F"
            damage_type: "Piercing"
            add_status:
              - status: 62D
                icon: "015000/015884.png"
                name:
                 de: "Adaption"
                 en: "Adaptation"
                 fr: "Adaptation"
                 ja: "環境適応"
                 cn: "环境适应"
                 ko: "환경 적응"
            damage:
              - min: 41472
              - max: 41472
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F24"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Feuer"
          en: "Fire"
          fr: "Feu"
          ja: "ファイア"
          cn: "火炎"
          ko: "파이어"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2E5A"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 3995
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E5E"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 3995
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E62"
            damage_type: "Magical"
            damage:
              - min: 21
              - max: 30655
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F51"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Blase"
      en: "Bubble"
      fr: "Bulle"
      ja: "バブル"
      cn: "泡泡"
      ko: "거품"
    enemy_id: "7510"
    id: "add07"
    attacks:
      - title:
          de: "Ebbe"
          en: "Riptide"
          fr: "Jusant"
          ja: "引潮"
          cn: "退潮"
          ko: "썰물"
        title_id: "2D63"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Wasserkäfig"
          en: "Watery Grave"
          fr: "Geôle Aqueuse"
          ja: "水牢"
          cn: "水牢"
          ko: "수중 감옥"
        title_id: "2D64"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "95"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Wasserkäfig"
          en: "Watery Grave"
          fr: "Geôle Aqueuse"
          ja: "水牢"
          cn: "水牢"
          ko: "수중 감옥"
        title_id: "1E6"
        icon: "015000/015038_hr1.png"
        description:
          de: "Im magisch geladenen Wasser gefangen und handlungsunfähig."
          en: "You are trapped in a magicked prison of water and are unable to execute actions."
          fr: "Une bulle d'eau magique emprisonne le corps, empêchant toute action."
          ja: "魔力が込められた水に閉じ込められ、行動できなくなった状態。"
          cn: "被带有魔力的水封锁住行动"
          ko: "마력이 담긴 물속에 갇혀 행동이 불가능한 상태."
        durations: [8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Blutdämon"
      en: "Blood Demon"
      fr: "Démon De Sang"
      ja: "ブラッドデーモン"
      cn: "血魔"
      ko: "흡혈 악마"
    enemy_id: "7434"
    id: "add08"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Abgründiger Schwung"
          en: "Abyssal Swing"
          fr: "Fauchage Abyssal"
          ja: "アビサルスイング"
          cn: "深渊回转"
          ko: "심연의 공격"
        title_id: "2E03"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Abyssal-Transfixion"
          en: "Abyssal Transfixion"
          fr: "Transfixion Abyssale"
          ja: "アビサルトランスフィクション"
          cn: "深渊贯穿"
          ko: "심연의 관통"
        title_id: "2E04"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 5858
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF9"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Dicotite"
      en: "Decotitus"
      fr: "Dicholitus"
      ja: "ディコタイタス"
      cn: "迪戈泰塔斯"
      ko: "디코타이터스"
    enemy_id: "7440"
    id: "add09"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EFF"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Wirbelkick"
          en: "Spinkick"
          fr: "Coup De Pied Circulaire"
          ja: "回し蹴り"
          cn: "回旋踢"
          ko: "돌려차기"
        title_id: "3008"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Dämon Der Inkunabel"
      en: "Demon Of The Incunable"
      fr: "Démon De L'Incunable"
      ja: "デモン・オブ・レアブック"
      cn: "珍卷恶魔"
      ko: "희귀본 악마"
    enemy_id: "7465"
    id: "add10"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F71"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 37888
          - max: 37888
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F18"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 110
          - max: 62065
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Dreischnitt"
          en: "Triclip"
          fr: "Triclip"
          ja: "トライクリップ"
          cn: "三连爪"
          ko: "삼단베기"
        title_id: "2E76"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 15
          - max: 62976
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mahd"
          en: "Mow"
          fr: "Fauche"
          ja: "モウ"
          cn: "毒波"
          ko: "살육"
        title_id: "2E77"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        add_status:
          - status: 12
            icon: "015000/015007.png"
            name:
               de: "Gift"
               en: "Poison"
               fr: "Poison"
               ja: "毒"
               cn: "中毒"
               ko: "독"
        damage:
          - min: 462
          - max: 787
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Blind"
          en: "Blind"
          fr: "Cécité"
          ja: "暗闇"
          cn: "失明"
          ko: "실명"
        title_id: "0F"
        icon: "015000/015012_hr1.png"
        description:
          de: "Vorübergehende Blindheit senkt die Präzision."
          en: "Encroaching darkness is lowering accuracy."
          fr: "Un voile d'obscurité diminue la précision."
          ja: "闇に囚われ、命中率が低下した状態。"
          cn: "陷于黑暗之中，命中率降低"
          ko: "어둠에 사로잡혀 명중률이 떨어진 상태."
        durations: [3, 7, 15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Eis-Speglidae"
      en: "Ice Mirrorknight"
      fr: "Chevalier Miroir Des Glaces"
      ja: "アイス・ミラーナイト"
      cn: "寒冰镜骑士"
      ko: "얼음 거울기사"
    enemy_id: "7426"
    id: "add11"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF1"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Böe"
          en: "Gust"
          fr: "Bourrasque"
          ja: "旋風"
          cn: "狂风"
          ko: "선풍"
        title_id: "2DF4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 53
          - max: 365
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Flügelschlag"
          en: "Bearclaw"
          fr: "Griffe D'Ours"
          ja: "ベアクロウ"
          cn: "熊爪"
          ko: "곰발톱"
        title_id: "2DF5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Eisblume"
      en: "Snow Bee"
      fr: "Cristal De Glace"
      ja: "六花"
      cn: "雪花"
      ko: "육화"
    enemy_id: "7528"
    id: "add12"
    hp:
      - min: 116985
      - max: 128956
    attacks:
      - title:
          de: "Hypothermale Verbrennung"
          en: "Hypothermal Combustion"
          fr: "Combustion Hypothermique"
          ja: "砕氷"
          cn: "冰碎"
          ko: "쇄빙"
        title_id: "2D2E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 196
          - max: 4145
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [11, 12]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Eishaut-Peiste"
      en: "Iceskin Peiste"
      fr: "Peiste Peau-De-Givre"
      ja: "アイススキン・ペイスト"
      cn: "冰皮毒蜥蜴"
      ko: "얼음비늘 독뿔도마뱀"
    enemy_id: "7439"
    id: "add13"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EFE"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Versteinernder Blick "
          en: "Stone Gaze"
          fr: "Regard Pétrifiant"
          ja: "ストーンゲイズ"
          cn: "石化凝视"
          ko: "석화의 눈빛"
        title_id: "2E0D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Eiszapfen"
      en: "Icicle"
      fr: "Stalactite"
      ja: "アイシクル"
      cn: "冰柱"
      ko: "고드름"
    enemy_id: "3045"
    id: "add14"
    hp:
      - min: 31059
      - max: 31059
    attacks:
      - title:
          de: "Säulendurchschlag"
          en: "Pillar Pierce"
          fr: "Empalement"
          ja: "激突"
          cn: "激突"
          ko: "격돌"
        title_id: "2EAA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Säuleneinschlag"
          en: "Pillar Impact"
          fr: "Chute"
          ja: "崩落"
          cn: "崩落"
          ko: "고드름 낙하"
        title_id: "2EA9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Entlaufener Tyrannosaurus"
      en: "Escaped Tyrannosaur"
      fr: "Tyrannosaure Évadé"
      ja: "エスケープド・ティラノサウルス"
      cn: "脱逃暴龙"
      ko: "탈출한 티라노사우루스"
    enemy_id: "7441"
    id: "add15"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F00"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F3B"
            damage_type: "Slashing"
            damage:
              - min: 113
              - max: 61809
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Drittes Bein"
          en: "Third Leg Forward"
          fr: "Fauchage"
          ja: "サードレッグ"
          cn: "第三腿后扫"
          ko: "제3의 다리 전진"
        title_id: "2E78"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F68"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Urgebrüll"
          en: "Primordial Roar"
          fr: "Rugissement Primitif"
          ja: "プライモーディアルロア"
          cn: "原始嚎叫"
          ko: "태고의 포효"
        title_id: "2E10"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Vorbiss"
          en: "Underbite"
          fr: "Prognathisme"
          ja: "アンダーバイト"
          cn: "低咬"
          ko: "반대 교합"
        title_id: "2E0F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 1920
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Eureka-Vindthur"
      en: "Eurekan Vindthurs"
      fr: "Vindthurs D'Eurêka"
      ja: "エウレカ・ヴィンドスルス"
      cn: "优雷卡风巨魔"
      ko: "에우레카 빈드두르스"
    enemy_id: "7405"
    id: "add16"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EDB"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30B6"
            damage_type: "Slashing"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F52"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Rippenstoß"
          en: "Elbow Drop"
          fr: "Descente De Coude"
          ja: "エルボードロップ"
          cn: "落肘"
          ko: "팔꿈치 찍기"
        title_id: "2DBF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 12952
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Schmetterfäuste"
          en: "Double Smash"
          fr: "Double Fracas"
          ja: "ダブルスマッシュ"
          cn: "双重碎击"
          ko: "연속 타격"
        title_id: "2DC0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 2951
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Experimenteller Minotaurus"
      en: "Lab Minotaur"
      fr: "Minotaure Du Laboratoire"
      ja: "ラボラトリー・ミノタウロス"
      cn: "研究所弥诺陶洛斯"
      ko: "연구실 미노타우로스"
    enemy_id: "7442"
    id: "add17"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "11-Tonzen-Hieb"
          en: "11-Tonze Swipe"
          fr: "Fauche De 11 Tonz"
          ja: "11トンズ・スワイプ"
          cn: "十一吨横扫"
          ko: "11톤즈 후려치기"
        title_id: "2E11"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 260
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "111-Tonzen-Schwung"
          en: "111-Tonze Swing"
          fr: "Swing De 111 Tonz"
          ja: "111トンズ・スイング"
          cn: "百十一吨回转"
          ko: "111톤즈 휘두르기"
        title_id: "2E12"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F01"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Faulige Apa"
      en: "Foul Apa"
      fr: "Apa Fétide"
      ja: "ファウル・アーパス"
      cn: "秽浊阿帕斯"
      ko: "불결한 아파"
    enemy_id: "7507"
    id: "add18"
    hp:
      - min: 305185
      - max: 426585
    attacks:
      - title:
          de: "Aquaga"
          en: "Water III"
          fr: "Méga Eau"
          ja: "ウォタガ"
          cn: "狂水"
          ko: "워터가"
        title_id: "2E56"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 101
          - max: 14613
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2E53"
          en: "Unknown_2E53"
          fr: "Unknown_2E53"
          ja: "Unknown_2E53"
          cn: "Unknown_2E53"
          ko: "Unknown_2E53"
        title_id: "2E53"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 101
          - max: 60517
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Flamme"
      en: "Fire Devil"
      fr: "Tourbillon De Feu"
      ja: "火焔"
      cn: "火焰"
      ko: "불꽃"
    enemy_id: "7519"
    id: "add19"
    hp:
      - min: 164221
      - max: 236547
    attacks:
      - title:
          de: "Feuersturm"
          en: "Firestorm"
          fr: "Tempête De Feu"
          ja: "ファイアストーム"
          cn: "火焰流"
          ko: "불보라"
        title_id: "2D8E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Heißluft"
          en: "Red Hot"
          fr: "Mains Brûlantes"
          ja: "熱気"
          cn: "热气"
          ko: "열기"
        title_id: "2D8D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 21
          - max: 10591
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Flammenkugel"
      en: "Ball Of Fire"
      fr: "Boule Ardente"
      ja: "火炎球"
      cn: "火焰球"
      ko: "화염구"
    enemy_id: "7525"
    id: "add20"
    attacks:
      - title:
          de: "Scharlachroter Pfad"
          en: "Scarlet Thread"
          fr: "Rayon Écarlate"
          ja: "真紅の熱線"
          cn: "真红射线"
          ko: "진홍빛 열선"
        title_id: "2D85"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Flöckchen"
      en: "Yukinko"
      fr: "Yukinko"
      ja: "ユキンコ"
      cn: "雪童子"
      ko: "눈요정"
    enemy_id: "7413"
    id: "add21"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EE3"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2EE6"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 60531
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Schneeregen"
          en: "Flurry"
          fr: "Averse De Neige"
          ja: "雪時雨"
          cn: "雨夹雪"
          ko: "진눈깨비"
        title_id: "2DDE"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Fromme Anhängerin"
      en: "Devout Disciple"
      fr: "Disciple D'Eurêka"
      ja: "エウレカの聖徒"
      cn: "优雷卡圣徒"
      ko: "에우레카의 신자"
    enemy_id: "7501"
    id: "add22"
    attacks:
      - title:
          de: "Ruhm"
          en: "Glory"
          fr: "Gloire"
          ja: "グローリーブレード"
          cn: "荣耀之剑"
          ko: "영광의 검"
        title_id: "2DCA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2DC6"
          en: "Unknown_2DC6"
          fr: "Unknown_2DC6"
          ja: "Unknown_2DC6"
          cn: "Unknown_2DC6"
          ko: "Unknown_2DC6"
        title_id: "2DC6"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Frostiger Avis"
      en: "Frozen Aevis"
      fr: "Eibis Gelé"
      ja: "フローズン・エイビス"
      cn: "冰霜龙鸟"
      ko: "얼음 에이비스"
    enemy_id: "7448"
    id: "add23"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F07"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 1177
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F32"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 1177
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Elektrische Velitation"
          en: "Electric Velitation"
          fr: "Rixe Électrique"
          ja: "エレクトリックファイト"
          cn: "电争"
          ko: "전기 파동"
        title_id: "2E2A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Kreischender Schrei"
          en: "Strident Scream"
          fr: "Hurlement Strident"
          ja: "ストライデントハウル"
          cn: "刺耳尖叫"
          ko: "불쾌한 비명"
        title_id: "3116"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F5F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Vliesabreibung"
          en: "Lunge"
          fr: "Charge"
          ja: "体当たり"
          cn: "冲撞"
          ko: "몸통 박치기"
        title_id: "2E2B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Frostiger Nichts-Drache"
      en: "Frozen Void Dragon"
      fr: "Dragon Gelé Du Néant"
      ja: "ヴォイド・フローズンドラゴン"
      cn: "虚无冰雪龙"
      ko: "보이드 얼음드래곤"
    enemy_id: "7473"
    id: "add24"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F80"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F1C"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Frostiger Atem"
          en: "Frost Breath"
          fr: "Souffle Glacé"
          ja: "フロストブレス"
          cn: "寒霜吐息"
          ko: "서리 숨결"
        title_id: "3126"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        damage:
          - min: 2646
          - max: 17611
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_3104"
          en: "Unknown_3104"
          fr: "Unknown_3104"
          ja: "Unknown_3104"
          cn: "Unknown_3104"
          ko: "Unknown_3104"
        title_id: "3104"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_3118"
          en: "Unknown_3118"
          fr: "Unknown_3118"
          ja: "Unknown_3118"
          cn: "Unknown_3118"
          ko: "Unknown_3118"
        title_id: "3118"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: 63C
            icon: "015000/015013.png"
            name:
               de: "Schlaf"
               en: "Sleep"
               fr: "Sommeil"
               ja: "睡眠"
               cn: "睡眠"
               ko: "수면"
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Schlaf"
          en: "Sleep"
          fr: "Sommeil"
          ja: "睡眠"
          cn: "睡眠"
          ko: "수면"
        title_id: "63C"
        icon: "015000/015013_hr1.png"
        description:
          de: "Tiefschlaf verhindert das Ausführen jeglicher Kommandos."
          en: "Overwhelming drowsiness is preventing the execution of actions."
          fr: "Le sommeil empêche toute action."
          ja: "眠りに落ち、行動できなくなった状態。"
          cn: "陷入沉睡，无法做出任何行动"
          ko: "잠에 빠져 행동이 불가능한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Frostiges Gelato"
      en: "Frozen Gelato"
      fr: "Gelato Gelé"
      ja: "フローズン・ジェラート"
      cn: "冰霜明胶怪"
      ko: "얼음 젤라토"
    enemy_id: "7427"
    id: "add25"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F73"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 38400
          - max: 38400
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF2"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Eis"
          en: "Blizzard"
          fr: "Glace"
          ja: "ブリザド"
          cn: "冰结"
          ko: "블리자드"
        title_id: "2F6C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 14032
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Goldzunge"
          en: "Golden Tongue"
          fr: "Langue Dorée"
          ja: "ゴールデンタン"
          cn: "金口舌"
          ko: "황금 혀"
        title_id: "2DF7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 39
            icon: "015000/015054.png"
            name:
               de: "Magieschaden +"
               en: "Magic Damage Up"
               fr: "Bonus de dégâts magiques"
               ja: "魔法ダメージ上昇"
               cn: "魔法伤害提高"
               ko: "주는 마법 피해량 증가"
        damage:
          - min: 57
          - max: 57
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Magieschaden +"
          en: "Magic Damage Up"
          fr: "Bonus De Dégâts Magiques"
          ja: "魔法ダメージ上昇"
          cn: "魔法伤害提高"
          ko: "주는 마법 피해량 증가"
        title_id: "39"
        icon: "015000/015054_hr1.png"
        description:
          de: "Ausgeteilter Magieschaden ist erhöht."
          en: "Magic damage dealt is increased."
          fr: "Les dégâts magiques infligés sont augmentés."
          ja: "魔法ダメージが上昇した状態。"
          cn: "魔法攻击所造成的伤害提高"
          ko: "마법 공격으로 주는 피해량이 증가한 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Gespenst"
      en: "Geshunpest"
      fr: "Gespenst"
      ja: "ゲシュンペスト"
      cn: "死魂"
      ko: "게슌페스트"
    enemy_id: "7466"
    id: "add26"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F76"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 39168
          - max: 39168
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F19"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Dunkelung"
          en: "Darkness"
          fr: "Obscurité"
          ja: "ダーク"
          cn: "黑暗"
          ko: "다크"
        title_id: "2E71"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Kohlkopfstoß"
          en: "Ram"
          fr: "Coup De Boule"
          ja: "頭突き"
          cn: "撞击"
          ko: "머리 들이받기"
        title_id: "2E70"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Gewitter-Exergon"
      en: "Thunderstorm Sprite"
      fr: "Élémentaire Des Tempêtes De Foudre"
      ja: "サンダーストームスプライト"
      cn: "雷暴元精"
      ko: "뇌우 정령"
    enemy_id: "7470"
    id: "add27"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F7D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Blitz"
          en: "Thunder"
          fr: "Foudre"
          ja: "サンダー"
          cn: "闪雷"
          ko: "선더"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2E58"
            damage_type: "Magical"
            damage:
              - min: 85
              - max: 1393
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E5C"
            damage_type: "Magical"
            damage:
              - min: 85
              - max: 9489
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E60"
            damage_type: "Magical"
            damage:
              - min: 85
              - max: 1393
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F5D"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "30AB"
            damage_type: "None"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Stein"
          en: "Stone"
          fr: "Terre"
          ja: "ストーン"
          cn: "飞石"
          ko: "스톤"
        title_id: "2FAA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Verbannen"
          en: "Banish"
          fr: "Bannissement"
          ja: "バニシュ"
          cn: "放逐"
          ko: "배니시"
        title_id: "30D6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Verbannga"
          en: "Banish III"
          fr: "Méga Bannissement"
          ja: "バニシュガ"
          cn: "强放逐"
          ko: "배니시가"
        title_id: "30D5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Giftiges Gas"
      en: "Poison Gas"
      fr: "Gaz Empoisonné"
      ja: "ポイズンガス"
      cn: "毒气"
      ko: "독가스"
    enemy_id: "7511"
    id: "add28"
    attacks:
      - title:
          de: "Giftga"
          en: "Poisonga II"
          fr: "Omnipoison"
          ja: "ポイゾガ"
          cn: "烈毒"
          ko: "포이즌가"
        title_id: "2D68"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Glavoid-Larve"
      en: "Iceworm"
      fr: "Ver De Glace"
      ja: "グラヴォイドの幼体"
      cn: "异形幼虫"
      ko: "작은 그라보이드"
    enemy_id: "7517"
    id: "add29"
    hp:
      - min: 44091
      - max: 108937
    attacks:
      - title:
          de: "Giftatem"
          en: "Poison Breath"
          fr: "Haleine Empoisonnée"
          ja: "ポイズンブレス"
          cn: "剧毒吐息"
          ko: "독 숨결"
        title_id: "2D41"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Sandsäule"
          en: "Sand Pillar"
          fr: "Pilier De Sable"
          ja: "サンドピラー"
          cn: "沙柱"
          ko: "모래기둥"
        title_id: "2D44"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2D3D"
          en: "Unknown_2D3D"
          fr: "Unknown_2D3D"
          ja: "Unknown_2D3D"
          cn: "Unknown_2D3D"
          ko: "Unknown_2D3D"
        title_id: "2D3D"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D42"
          en: "Unknown_2D42"
          fr: "Unknown_2D42"
          ja: "Unknown_2D42"
          cn: "Unknown_2D42"
          ko: "Unknown_2D42"
        title_id: "2D42"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D43"
          en: "Unknown_2D43"
          fr: "Unknown_2D43"
          ja: "Unknown_2D43"
          cn: "Unknown_2D43"
          ko: "Unknown_2D43"
        title_id: "2D43"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Glubscher"
      en: "Gawper"
      fr: "Ahuri"
      ja: "ガウパー"
      cn: "瞪视之眼"
      ko: "주시자"
    enemy_id: "7420"
    id: "add30"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EEB"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 113
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Augen Auf Mich"
          en: "Eyes On Me"
          fr: "Œil Sur Moi"
          ja: "アイズオンミー"
          cn: "凶眼注目"
          ko: "아이즈 온 미"
        title_id: "2DE5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 23732
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Stein"
          en: "Stone"
          fr: "Terre"
          ja: "ストーン"
          cn: "飞石"
          ko: "스톤"
        title_id: "2DE4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 69
          - max: 10661
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Glückshase"
      en: "Happy Bunny"
      fr: "Lapin Du Bonheur"
      ja: "しあわせうさぎ"
      cn: "幸福兔"
      ko: "행운토끼"
    enemy_id: "7248"
    id: "add31"
    hp:
      - min: 21720
      - max: 218322
    attacks:
      - title:
          de: "Unknown_2E67"
          en: "Unknown_2E67"
          fr: "Unknown_2E67"
          ja: "Unknown_2E67"
          cn: "Unknown_2E67"
          ko: "Unknown_2E67"
        title_id: "2E67"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 5
          - max: 115
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2F89"
          en: "Unknown_2F89"
          fr: "Unknown_2F89"
          ja: "Unknown_2F89"
          cn: "Unknown_2F89"
          ko: "Unknown_2F89"
        title_id: "2F89"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 5
          - max: 115
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Hagelbomber"
      en: "Icefall"
      fr: "Tombeglace"
      ja: "アイスフォール"
      cn: "冰瀑"
      ko: "얼음폭포"
    enemy_id: "7513"
    id: "add32"
    hp:
      - min: 33231
      - max: 54951
    attacks:
      - title:
          de: "Hypothermale Verbrennung"
          en: "Hypothermal Combustion"
          fr: "Combustion Hypothermique"
          ja: "砕氷"
          cn: "冰碎"
          ko: "쇄빙"
        title_id: "2D31"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 11276
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Haunt"
      en: "Haunt"
      fr: "Tourmenteur"
      ja: "ハウント"
      cn: "鬼灵"
      ko: "바다 유령"
    enemy_id: "7467"
    id: "add33"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F7A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 40192
          - max: 40192
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F1A"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Erschütternde Oszillation"
          en: "Concussive Oscillation"
          fr: "Oscillation Commotionnelle"
          ja: "コンカシヴ・オレーション"
          cn: "振荡气流"
          ko: "격렬한 진동"
        title_id: "2E42"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 53
          - max: 387
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Huwasi"
      en: "Huwasi"
      fr: "Huwasi"
      ja: "フワシ"
      cn: "胡瓦西"
      ko: "후와시"
    enemy_id: "7419"
    id: "add34"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EEA"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 6179
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Felsschleuder"
          en: "Boulder Toss"
          fr: "Jet De Pierre"
          ja: "投石"
          cn: "投石"
          ko: "투석"
        title_id: "2DE2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mure"
          en: "Landslip"
          fr: "Glissement De Terrain"
          ja: "土石流"
          cn: "泥石流"
          ko: "토석류"
        title_id: "2DE3"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Präzisions-Malus"
          en: "Accuracy Down"
          fr: "Malus De Précision"
          ja: "命中率ダウン"
          cn: "命中率降低"
          ko: "명중률 감소"
        title_id: "1C"
        icon: "015000/015061_hr1.png"
        description:
          de: "Die Präzision ist verringert."
          en: "Accuracy is reduced."
          fr: "La précision est réduite."
          ja: "命中率が低下した状態。"
          cn: "命中率减弱"
          ko: "명중률이 떨어진 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Höherer Minotaurus"
      en: "Eldertaur"
      fr: "Ancestrotaure"
      ja: "エルダータウルス"
      cn: "牛头魔长老"
      ko: "장로 타우루스"
    enemy_id: "7514"
    id: "add35"
    hp:
      - min: 1552772
      - max: 1728303
    attacks:
      - title:
          de: "Blutsbruder"
          en: "Blood Brother"
          fr: "Frère De Sang"
          ja: "ブラザーブラッド"
          cn: "兄弟之血"
          ko: "형제의 피"
        title_id: "2D51"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 638
            icon: "017000/017061.png"
            name:
               de: "Schaden +"
               en: "Damage Up"
               fr: "Bonus de dégâts"
               ja: "ダメージ上昇"
               cn: "伤害提高"
               ko: "주는 피해량 증가"
          - status: 639
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 117
          - max: 47616
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Dreischnitt"
          en: "Triclip"
          fr: "Triclip"
          ja: "トライクリップ"
          cn: "三连爪"
          ko: "삼단베기"
        title_id: "2D53"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Furchtbares Brüllen"
          en: "Frightful Roar"
          fr: "Rugissement Effroyable"
          ja: "フライトフルロア"
          cn: "骇人嚎叫"
          ko: "끔찍한 포효"
        title_id: "2D55"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Himmelsstrahl"
          en: "Cerulean Ray"
          fr: "Rayon Céruléen"
          ja: "セルリアンレイ"
          cn: "青蓝射线"
          ko: "진청색 광선"
        title_id: "2D52"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mahd"
          en: "Mow"
          fr: "Fauche"
          ja: "モウ"
          cn: "毒波"
          ko: "살육"
        title_id: "2D54"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2D50"
          en: "Unknown_2D50"
          fr: "Unknown_2D50"
          ja: "Unknown_2D50"
          cn: "Unknown_2D50"
          ko: "Unknown_2D50"
        title_id: "2D50"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "639"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "40"
        icon: "015000/015020_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Schaden +"
          en: "Damage Up"
          fr: "Bonus De Dégâts"
          ja: "ダメージ上昇"
          cn: "伤害提高"
          ko: "주는 피해량 증가"
        title_id: "638"
        icon: "017000/017061_hr1.png"
        description:
          de: "Ausgeteilter Schaden ist erhöht."
          en: "Damage dealt is increased."
          fr: "Les dégâts infligés sont augmentés."
          ja: "与ダメージが上昇した状態。"
          cn: "攻击所造成的伤害提高"
          ko: "적에게 주는 피해량이 증가하는 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Verhängnis"
          en: "Doom"
          fr: "Glas"
          ja: "死の宣告"
          cn: "死亡宣告"
          ko: "죽음의 선고"
        title_id: "38E"
        icon: "015000/015503_hr1.png"
        description:
          de: "Wird kampfunfähig, sobald der Timer des Fluchs auf 0 sinkt."
          en: "Certain death when counter reaches zero."
          fr: "Mort certaine lorsque le compteur atteint zéro."
          ja: "カウント0で戦闘不能に陥る状態。"
          cn: "倒计时为0时会陷入无法战斗状态"
          ko: "지속시간이 끝나면 전투불능이 되는 상태."
        durations: [10]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Iglu"
      en: "Igloo"
      fr: "Igloo"
      ja: "イグルー"
      cn: "雪屋"
      ko: "이글루"
    enemy_id: "7504"
    id: "add36"
    hp:
      - min: 35260
      - max: 35260
    attacks:
      - title:
          de: "Schneeblind"
          en: "Snowblind"
          fr: "Voile Neigeux"
          ja: "雪煙"
          cn: "雪烟"
          ko: "눈안개"
        title_id: "2DA0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 4766
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Schneegebirge"
          en: "Snowcap"
          fr: "Chaîne Glaciaire"
          ja: "大雪山"
          cn: "大雪山"
          ko: "대설산"
        title_id: "2D9D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_127B"
          en: "Unknown_127B"
          fr: "Unknown_127B"
          ja: "Unknown_127B"
          cn: "Unknown_127B"
          ko: "Unknown_127B"
        title_id: "127B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2D9A"
          en: "Unknown_2D9A"
          fr: "Unknown_2D9A"
          ja: "Unknown_2D9A"
          cn: "Unknown_2D9A"
          ko: "Unknown_2D9A"
        title_id: "2D9A"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 1552
        phases:
          - phase: "09"
      - title:
          de: "Unknown_2DB2"
          en: "Unknown_2DB2"
          fr: "Unknown_2DB2"
          ja: "Unknown_2DB2"
          cn: "Unknown_2DB2"
          ko: "Unknown_2DB2"
        title_id: "2DB2"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Irrseele"
      en: "Unquiet Dead"
      fr: "Défunt Errant"
      ja: "さまよえる亡者"
      cn: "徘徊的亡者"
      ko: "떠도는 망자"
    enemy_id: "7521"
    id: "add37"
    hp:
      - min: 240998
      - max: 295522
    attacks:
      - title:
          de: "Felssplitter"
          en: "Rocksplitter"
          fr: "Fendoir De Roche"
          ja: "ロックスプリッター"
          cn: "裂岩"
          ko: "암석 파쇄"
        title_id: "2D3B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 113
          - max: 7630
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Juwelenhorn"
      en: "Gemhorn"
      fr: "Cristallocéros"
      ja: "ジュエルホーン"
      cn: "宝石角"
      ko: "보석뿔"
    enemy_id: "7531"
    id: "add38"
    hp:
      - min: 1502813
      - max: 2391269
    attacks:
      - title:
          de: "Unknown_2F8B"
          en: "Unknown_2F8B"
          fr: "Unknown_2F8B"
          ja: "Unknown_2F8B"
          cn: "Unknown_2F8B"
          ko: "Unknown_2F8B"
        title_id: "2F8B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 113
          - max: 14414
        phases:
          - phase: "09"
      - title:
          de: "Wirbelkick"
          en: "Spinkick"
          fr: "Coup De Pied Circulaire"
          ja: "回し蹴り"
          cn: "回旋踢"
          ko: "돌려차기"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F8C"
            damage_type: "Blunt"
            damage:
              - min: 4889
              - max: 5864
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
          - title_id: "2F8D"
            damage_type: "Blunt"
            damage:
              - min: 106
              - max: 12070
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Korallen-Uragnit"
      en: "Coralline Uragnite"
      fr: "Uragnite Coralligène"
      ja: "コラリン・ウラグナイト"
      cn: "珊瑚乌菊石"
      ko: "산호 오징어조개"
    enemy_id: "7437"
    id: "add39"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EFC"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Gasgranate"
          en: "Gas Shell"
          fr: "Conque De Gaz"
          ja: "ガスシェル"
          cn: "毒气壳"
          ko: "가스 연막"
        title_id: "2E09"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Geißelung"
          en: "Painful Whip"
          fr: "Fouet Douloureux"
          ja: "ペインフルウィップ"
          cn: "痛苦鞭"
          ko: "고통의 채찍"
        title_id: "2E0A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Leibeigener"
      en: "Decotitus Drudge"
      fr: "Esclave"
      ja: "スレイヴ"
      cn: "迪戈泰塔斯仆从"
      ko: "슬레이브"
    enemy_id: "7532"
    id: "add40"
    attacks:
      - title:
          de: "Unknown_2F8A"
          en: "Unknown_2F8A"
          fr: "Unknown_2F8A"
          ja: "Unknown_2F8A"
          cn: "Unknown_2F8A"
          ko: "Unknown_2F8A"
        title_id: "2F8A"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
      - title:
          de: "Wirbelkick"
          en: "Spinkick"
          fr: "Coup De Pied Circulaire"
          ja: "回し蹴り"
          cn: "回旋踢"
          ko: "돌려차기"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2877"
            damage_type: "Blunt"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E6F"
            damage_type: "Darkness"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Nichts-Syricta"
      en: "Void Syricta"
      fr: "Syricta Du Néant"
      ja: "ヴォイド・シリクタ"
      cn: "虚无希里科塔"
      ko: "보이드 시리크타"
    enemy_id: "7454"
    id: "add41"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F0D"
            damage_type: "Slashing"
            damage:
              - min: 113
              - max: 1197
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30C1"
            damage_type: "Blunt"
            damage:
              - min: 1959
              - max: 2994
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Feuerluft"
          en: "Bragget Breath"
          fr: "Haleine De Bragot"
          ja: "ブラゴットブレス"
          cn: "燃火吐息"
          ko: "타오르는 숨결"
        title_id: "3124"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Kochendes Blut"
          en: "Bloodboil"
          fr: "Surexcitation"
          ja: "大興奮"
          cn: "热血沸腾"
          ko: "대흥분"
        title_id: "3103"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F65"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62E
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 46592
          - max: 46592
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unbeugsamkeit"
          en: "Face Off"
          fr: "Défiguration"
          ja: "フェイスオフ"
          cn: "对决"
          ko: "경기 시작"
        title_id: "3123"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Wilde Rage"
          en: "Wild Charge"
          fr: "Charge Sauvage"
          ja: "ワイルドチャージ"
          cn: "狂野冲锋"
          ko: "야성의 돌진"
        title_id: "3102"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gebrechlichkeit"
          en: "Infirmity"
          fr: "Infirmité"
          ja: "虚弱"
          cn: "虚弱"
          ko: "허약"
        title_id: "AC"
        icon: "015000/015531_hr1.png"
        description:
          de: "Wirkung von Heilmagie auf sich ist verringert. "
          en: "HP recovery via healing magic is reduced."
          fr: "L'effet des sorts de restauration des PV est réduit."
          ja: "回復魔法による自身に対するＨＰ回復効果が低下した状態。"
          cn: "自身所受的治疗魔法效果降低"
          ko: "자신이 받는 회복마법의 HP 회복 효과가 떨어진 상태."
        durations: [8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Schaden +"
          en: "Damage Up"
          fr: "Bonus De Dégâts"
          ja: "ダメージ上昇"
          cn: "伤害提高"
          ko: "주는 피해량 증가"
        title_id: "3D"
        icon: "015000/015519_hr1.png"
        description:
          de: "Ausgeteilter Schaden ist erhöht."
          en: "Damage dealt is increased."
          fr: "Les dégâts infligés sont augmentés."
          ja: "与ダメージが上昇した状態。"
          cn: "攻击所造成的伤害提高"
          ko: "적에게 주는 피해량이 증가하는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Nichts-Vivel"
      en: "Void Vouivre"
      fr: "Vouivre Du Néant"
      ja: "ヴォイド・ヴィーヴル"
      cn: "虚无薇薇尔飞龙"
      ko: "보이드 뷔브르"
    enemy_id: "7445"
    id: "add42"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F04"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Lumisphäre"
          en: "Lumisphere"
          fr: "Lumisphère"
          ja: "ルミスフィア"
          cn: "光球"
          ko: "빛의 구체"
        title_id: "2E26"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 8850
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Nichts-Wyvern"
      en: "Zombie Wyvern"
      fr: "Wyverne Du Néant"
      ja: "ヴォイド・ワイバーン"
      cn: "虚无双足飞龙"
      ko: "보이드 와이번"
    enemy_id: "7436"
    id: "add43"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EFB"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Stachelschweif"
          en: "Spiked Tail"
          fr: "Queue Barbelée"
          ja: "スパイクテール"
          cn: "刺尾"
          ko: "독가시 꼬리"
        title_id: "2E07"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Niederer Minotaurus"
      en: "Mindertaur"
      fr: "Spiritaure"
      ja: "マインダータウルス"
      cn: "牛头魔看守"
      ko: "호위 타우루스"
    enemy_id: "7515"
    id: "add44"
    hp:
      - min: 1552772
      - max: 1728303
    attacks:
      - title:
          de: "Blutsbruder"
          en: "Blood Brother"
          fr: "Frère De Sang"
          ja: "ブラザーブラッド"
          cn: "兄弟之血"
          ko: "형제의 피"
        title_id: "2D57"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 638
            icon: "017000/017061.png"
            name:
               de: "Schaden +"
               en: "Damage Up"
               fr: "Bonus de dégâts"
               ja: "ダメージ上昇"
               cn: "伤害提高"
               ko: "주는 피해량 증가"
          - status: 639
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 1592
          - max: 47872
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Dreischnitt"
          en: "Triclip"
          fr: "Triclip"
          ja: "トライクリップ"
          cn: "三连爪"
          ko: "삼단베기"
        title_id: "2D59"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Furchtbares Brüllen"
          en: "Frightful Roar"
          fr: "Rugissement Effroyable"
          ja: "フライトフルロア"
          cn: "骇人嚎叫"
          ko: "끔찍한 포효"
        title_id: "2D5B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mahd"
          en: "Mow"
          fr: "Fauche"
          ja: "モウ"
          cn: "毒波"
          ko: "살육"
        title_id: "2D5A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Tödlicher Strahl"
          en: "Mortal Ray"
          fr: "Rayon Mortel"
          ja: "モータルレイ"
          cn: "致命射线"
          ko: "죽음의 광선"
        title_id: "2D58"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2D56"
          en: "Unknown_2D56"
          fr: "Unknown_2D56"
          ja: "Unknown_2D56"
          cn: "Unknown_2D56"
          ko: "Unknown_2D56"
        title_id: "2D56"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 6500
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "639"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Erhöhte Physische Verwundbarkeit"
          en: "Physical Vulnerability Up"
          fr: "Vulnérabilité Physique Augmentée"
          ja: "被物理ダメージ増加"
          cn: "物理受伤加重"
          ko: "받는 물리 피해량 증가"
        title_id: "38"
        icon: "015000/015053_hr1.png"
        description:
          de: "Erlittener physischer Schaden ist erhöht."
          en: "Physical damage taken is increased."
          fr: "Les dégâts physiques reçus sont augmentés."
          ja: "受ける物理ダメージが増加する状態。"
          cn: "受到物理攻击的伤害增加"
          ko: "물리 공격으로 받는 피해량이 증가하는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Schaden +"
          en: "Damage Up"
          fr: "Bonus De Dégâts"
          ja: "ダメージ上昇"
          cn: "伤害提高"
          ko: "주는 피해량 증가"
        title_id: "638"
        icon: "017000/017061_hr1.png"
        description:
          de: "Ausgeteilter Schaden ist erhöht."
          en: "Damage dealt is increased."
          fr: "Les dégâts infligés sont augmentés."
          ja: "与ダメージが上昇した状態。"
          cn: "攻击所造成的伤害提高"
          ko: "적에게 주는 피해량이 증가하는 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Niederer Nichts-Drache"
      en: "Lesser Void Dragon"
      fr: "Dragon Inférieur Du Néant"
      ja: "ヴォイド・レッサードラゴン"
      cn: "虚无小龙"
      ko: "보이드 하급 드래곤"
    enemy_id: "7460"
    id: "add45"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Atem Des Löwen"
          en: "The Lion's Breath"
          fr: "Souffle Du Lion"
          ja: "フレイムブレス"
          cn: "火焰吐息"
          ko: "화염 숨결"
        title_id: "2E40"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F13"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 11782
          - max: 11782
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Prankenhieb"
          en: "Rake"
          fr: "Griffes"
          ja: "ひっかき"
          cn: "利爪"
          ko: "할퀴기"
        title_id: "2E41"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 13850
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Nördlicher Fluturini"
      en: "Northern Fluturini"
      fr: "Fluturini Du Nord"
      ja: "ノーザン・フルトゥリーニ"
      cn: "北境浮蝶"
      ko: "북방 반짝나비"
    enemy_id: "7410"
    id: "add46"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F6D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 36864
          - max: 36864
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE0"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Tornado"
          en: "Tornado"
          fr: "Tornade"
          ja: "トルネド"
          cn: "龙卷"
          ko: "토네이도"
        title_id: "2DD6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Nördlicher Kolibri"
      en: "Northern Colibri"
      fr: "Colibri Du Nord"
      ja: "ノーザン・コリブリ"
      cn: "北境蜂鸟"
      ko: "북방 콜리브리"
    enemy_id: "7433"
    id: "add47"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EF8"
            damage_type: "Piercing"
            damage:
              - min: 114
              - max: 5600
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30B4"
            damage_type: "Blunt"
            damage:
              - min: 114
              - max: 5600
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Hartschnabel"
          en: "Breakbeak"
          fr: "Bec Dur"
          ja: "ハードビル"
          cn: "钢喙"
          ko: "딱딱한 부리"
        title_id: "30D1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F4E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Ruheruf"
          en: "Slowcall"
          fr: "Appel Paresseux"
          ja: "スロウコール"
          cn: "减速咒"
          ko: "둔화의 외침"
        title_id: "2E01"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Überschlag"
          en: "Loop"
          fr: "Looping"
          ja: "ループ"
          cn: "空翻"
          ko: "순환"
        title_id: "2E02"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 1008
          - max: 1791
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "231"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Anubis"
      en: "Pagos Anubys"
      fr: "Anubis Pagos"
      ja: "パゴス・アヌビス"
      cn: "恒冰阿努比斯"
      ko: "파고스 아누비스"
    enemy_id: "7474"
    id: "add48"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F1D"
            damage_type: "Slashing"
            damage:
              - min: 7
              - max: 14713
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F30"
            damage_type: "Slashing"
            damage:
              - min: 113
              - max: 5383
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Fesselklinge"
          en: "Spellsword"
          fr: "Lame Occulte"
          ja: "呪縛斬"
          cn: "咒缚斩"
          ko: "저주의 칼날"
        title_id: "2E72"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 3323
          - max: 3840
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Lichtriss"
          en: "Luminous Laceration"
          fr: "Lacération Lumineuse"
          ja: "ルミナスラサレーション"
          cn: "光流撕裂"
          ko: "빛의 열상"
        title_id: "3111"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 5699
          - max: 5699
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F67"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62E
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 32000
          - max: 32000
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Tor Zum Tod"
          en: "Death's Door"
          fr: "Portes De La Mort"
          ja: "葬送撃"
          cn: "葬送击"
          ko: "장송격"
        title_id: "2E73"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Vorpalklinge"
          en: "Vorpal Blade"
          fr: "Lame Vorpale"
          ja: "ボーパルブレード"
          cn: "致命斩"
          ko: "치명적인 칼날"
        title_id: "3112"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Bock"
      en: "Pagos Billygoat"
      fr: "Bouquetin Pagos"
      ja: "パゴス・ビリー"
      cn: "恒冰公山羊"
      ko: "파고스 숫염소"
    enemy_id: "7423"
    id: "add49"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EEE"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Kopfstoß"
          en: "Head Butt"
          fr: "Coup De Tête"
          ja: "ヘッドバット"
          cn: "冲顶"
          ko: "박치기"
        title_id: "2DEB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Seismischer Schrei"
          en: "Seismic Scream"
          fr: "Cri Sismique"
          ja: "サイズミックスクリーム"
          cn: "地震嚎"
          ko: "괴성"
        title_id: "2DEA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Schadenswert +"
          en: "Physical Damage Up"
          fr: "Bonus De Dégâts Physiques"
          ja: "物理ダメージ上昇"
          cn: "物理伤害提高"
          ko: "주는 물리 피해량 증가"
        title_id: "35"
        icon: "015000/015050_hr1.png"
        description:
          de: "Ausgeteilter physischer Schaden ist erhöht."
          en: "Physical damage dealt is increased."
          fr: "Les dégâts physiques infligés sont augmentés."
          ja: "物理ダメージが上昇した状態。"
          cn: "物理攻击所造成的伤害提高"
          ko: "물리 공격으로 주는 피해량이 증가한 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Bär"
      en: "Pagos Bear"
      fr: "Ours Pagos"
      ja: "パゴス・ベアー"
      cn: "恒冰巨熊"
      ko: "파고스 곰"
    enemy_id: "7428"
    id: "add50"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF3"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F50"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Schwere Attacke"
          en: "Savage Swipe"
          fr: "Coup Violent"
          ja: "強撃"
          cn: "强击"
          ko: "강력 공격"
        title_id: "2DF8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Domigator"
      en: "Pagos Croc"
      fr: "Crocodile Pagos"
      ja: "パゴス・ダイル"
      cn: "恒冰巨鳄"
      ko: "파고스 악어"
    enemy_id: "7452"
    id: "add51"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F0B"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30BA"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Einstampfen"
          en: "Flatten"
          fr: "Aplatissement"
          ja: "押しつぶす"
          cn: "跺脚"
          ko: "압사"
        title_id: "2E30"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 1031
          - max: 1031
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Fluchschlag"
          en: "Implosive Curse"
          fr: "Griffes De Malédiction"
          ja: "呪怨撃"
          cn: "咒怨击"
          ko: "원한의 일격"
        title_id: "30F5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F57"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62E
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 44800
          - max: 44800
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Zermalmen"
          en: "Crushing Fangs"
          fr: "Crocs Destructeurs"
          ja: "クラッシュファング"
          cn: "粉碎锐牙"
          ko: "압살 송곳니"
        title_id: "2E31"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Präzisions-Malus"
          en: "Accuracy Down"
          fr: "Malus De Précision"
          ja: "命中率ダウン"
          cn: "命中率降低"
          ko: "명중률 감소"
        title_id: "1C"
        icon: "015000/015061_hr1.png"
        description:
          de: "Die Präzision ist verringert."
          en: "Accuracy is reduced."
          fr: "La précision est réduite."
          ja: "命中率が低下した状態。"
          cn: "命中率减弱"
          ko: "명중률이 떨어진 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Elementar"
      en: "Pagos Elemental"
      fr: "Élémentaire Pagos"
      ja: "パゴス・エレメンタル"
      cn: "恒冰元灵"
      ko: "파고스 정령"
    enemy_id: "7567"
    id: "add52"
    hp:
      - min: 5306
      - max: 5306
    attacks:
      - title:
          de: "Geisterhauch"
          en: "Breath Of The Wild Ones"
          fr: "Souffle Des Esprits"
          ja: "精霊の息吹"
          cn: "元灵的气息"
          ko: "정령의 숨결"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        add_status:
          - status: 5B5
            icon: "015000/015887.png"
            name:
               de: "Elementarer Segen: Routine"
               en: "Elemental Blessing: EXP"
               fr: "Bénédiction des esprits : bonus d'expérience"
               ja: "精霊の加護：経験値アップ"
               cn: "元灵加护：经验值提高"
               ko: "정령의 가호: 경험치 증가"
          - status: 5B7
            icon: "015000/015835.png"
            name:
               de: "Elementarer Segen: Schaden"
               en: "Elemental Blessing: Damage"
               fr: "Bénédiction des esprits : bonus de combativité"
               ja: "精霊の加護：戦闘能力アップ"
               cn: "元灵加护：战斗力提高"
               ko: "정령의 가호: 전투 능력 향상"
        phases:
          - phase: "09"
        variation:
          - title_id: "2745"
            damage_type: "Piercing"
            add_status:
              - status: 5B5
                icon: "015000/015887.png"
                name:
                 de: "Elementarer Segen: Routine"
                 en: "Elemental Blessing: EXP"
                 fr: "Bénédiction des esprits : bonus d'expérience"
                 ja: "精霊の加護：経験値アップ"
                 cn: "元灵加护：经验值提高"
                 ko: "정령의 가호: 경험치 증가"
              - status: 5B7
                icon: "015000/015835.png"
                name:
                 de: "Elementarer Segen: Schaden"
                 en: "Elemental Blessing: Damage"
                 fr: "Bénédiction des esprits : bonus de combativité"
                 ja: "精霊の加護：戦闘能力アップ"
                 cn: "元灵加护：战斗力提高"
                 ko: "정령의 가호: 전투 능력 향상"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
          - title_id: "2746"
            damage_type: "Slashing"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "AoE"
    debuffs:
      - title:
          de: "Elementarer Segen: Routine"
          en: "Elemental Blessing: Exp"
          fr: "Bénédiction Des Esprits : Bonus D'Expérience"
          ja: "精霊の加護：経験値アップ"
          cn: "元灵加护：经验值提高"
          ko: "정령의 가호: 경험치 증가"
        title_id: "5B5"
        icon: "015000/015887_hr1.png"
        description:
          de: "Hat den Segen der Elemente von Eureka erhalten. Die Elementarroutine beim Besiegen von Feinden ist erhöht."
          en: "Receiving the blessing of Eureka's elementals. Elemental EXP earned through battle is increased."
          fr: "Les élémentaires d'Eurêka accordent leur bénédiction. L'expérience élémentaire obtenue au combat est augmentée."
          ja: "エウレカエレメンタルの加護を受けた状態。敵討伐時の獲得エレメンタル経験値が増加する。"
          cn: "受到了优雷卡元灵的加护，打倒敌人时获得的元素经验值增加"
          ko: "에우레카 정령의 가호를 받은 상태. 적 처치 시 얻는 엘리멘탈 경험치가 증가한다."
        durations: [1376, 1626, 1798, 3600]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Elementarer Segen: Schaden"
          en: "Elemental Blessing: Damage"
          fr: "Bénédiction Des Esprits : Bonus De Combativité"
          ja: "精霊の加護：戦闘能力アップ"
          cn: "元灵加护：战斗力提高"
          ko: "정령의 가호: 전투 능력 향상"
        title_id: "5B7"
        icon: "015000/015835_hr1.png"
        description:
          de: "Hat den Segen der Elemente von Eureka erhalten. Ausgeteilter Schaden ist erhöht und LP werden über Zeit wiederhergestellt."
          en: "Receiving the blessing of Eureka's elementals. Damage dealt is increased and restoring HP over time."
          fr: "Les élémentaires d'Eurêka accordent leur bénédiction. Les dégâts infligés sont augmentés et des PV sont récupérés périodiquement."
          ja: "エウレカエレメンタルの加護を受けた状態。与えるダメージが上昇し、ＨＰが徐々に回復する。"
          cn: "受到了优雷卡元灵的加护，攻击所造成的伤害提高，体力会随时间逐渐恢复"
          ko: "에우레카 정령의 가호를 받은 상태. 주는 피해량이 증가하고 HP가 서서히 회복된다."
        durations: [1376, 1626, 1798, 3600]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Glotzauge"
      en: "Pagos Deepeye"
      fr: "Oculus Pagos"
      ja: "パゴス・ディープアイ"
      cn: "恒冰深瞳"
      ko: "파고스 볼록눈"
    enemy_id: "7422"
    id: "add53"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EB9"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2EED"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Aufdringlichkeit"
          en: "Optical Intrusion"
          fr: "Intempestivité"
          ja: "出しゃばり"
          cn: "多嘴多事"
          ko: "나서기"
        title_id: "2DE9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Hypnotisieren"
          en: "Hypnotize"
          fr: "Hypnose"
          ja: "睨み"
          cn: "凝视"
          ko: "째려보기"
        title_id: "2DE8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2E8E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Gnu"
      en: "Pagos Wildebeest"
      fr: "Gnou Pagos"
      ja: "パゴス・ワイルドビースト"
      cn: "恒冰牛羚"
      ko: "파고스 혹등들소"
    enemy_id: "7421"
    id: "add54"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EEC"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30B7"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Froschblick"
          en: "Eye Of The Slithered"
          fr: "Œil Batracien"
          ja: "カエルの瞳"
          cn: "蛙变之瞳"
          ko: "개구리의 눈동자"
        title_id: "30F0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Kaltes Starren"
          en: "Cold Stare"
          fr: "Regard Froid"
          ja: "白眼視"
          cn: "白眼"
          ko: "백안시"
        title_id: "30F1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Kraftballung"
          en: "Bull Charge"
          fr: "Charge Bovine"
          ja: "ブルチャージ"
          cn: "猛牛冲锋"
          ko: "뿔소 돌진"
        title_id: "2DE7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F54"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Starrender Blick"
          en: "Quarry Lake"
          fr: "Œil Pétrifiant"
          ja: "石化の瞳"
          cn: "石化之瞳"
          ko: "석화의 눈동자"
        title_id: "2DE6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Frosch"
          en: "Toad"
          fr: "Crapaud"
          ja: "トード"
          cn: "蛙变"
          ko: "두꺼비"
        title_id: "1B7"
        icon: "015000/015580_hr1.png"
        description:
          de: "In einen Frosch verwandelt. Kommandos können nicht ausgeführt werden."
          en: "Transformed into a toad and unable to execute actions."
          fr: "La transformation en crapaud empêche toute action."
          ja: "カエルに変身した状態。アクション実行不可。"
          cn: "变成一只青蛙，无法发动任何技能"
          ko: "두꺼비로 변신한 상태. 기술을 실행할 수 없다."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Stein"
          en: "Petrification"
          fr: "Pétrification"
          ja: "石化"
          cn: "石化"
          ko: "석화"
        title_id: "5E7"
        icon: "015000/015001_hr1.png"
        description:
          de: "Die Versteinerung des Körpers verhindert das Ausführen jeglicher Kommandos."
          en: "Stone-like rigidity is preventing the execution of actions."
          fr: "Une rigidité semblable à celle de la pierre empêche toute action."
          ja: "手足が石化し、行動できなくなった状態。"
          cn: "手足被石化，无法做出任何行动"
          ko: "손발이 석화되어 행동이 불가능한 상태."
        durations: [5, 10]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Hippocerf"
      en: "Pagos Hippocerf"
      fr: "Hippocerf Pagos"
      ja: "パゴス・ヒッポセルフ"
      cn: "恒冰骏雕"
      ko: "파고스 히포세르프"
    enemy_id: "7438"
    id: "add55"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EFD"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Nervtötender Schrei"
          en: "Shriek"
          fr: "Beuglement"
          ja: "号叫"
          cn: "惊叫"
          ko: "새된 외침"
        title_id: "2E0B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Schnabelhieb"
          en: "Beak Snap"
          fr: "Claquement De Bec"
          ja: "ビークスナップ"
          cn: "号叫"
          ko: "부리 휘두르기"
        title_id: "2E0C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 8671
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "02"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        durations: [3]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Puk"
      en: "Pagos Puk"
      fr: "Puk Pagos"
      ja: "パゴス・プーク"
      cn: "恒冰跳蜥"
      ko: "파고스 푸크"
    enemy_id: "7403"
    id: "add56"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2ED9"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30B3"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 1126
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Feuerball"
          en: "Fireball"
          fr: "Boule De Feu"
          ja: "ファイアーボール"
          cn: "火球"
          ko: "화염구"
        title_id: "2DBB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2ECB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 634
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 43008
          - max: 43008
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Schwanzjagd"
          en: "Tail Chase"
          fr: "Chasse-Queue"
          ja: "テイルチェイス"
          cn: "圆尾扫"
          ko: "꼬리 휘두르기"
        title_id: "30D0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "634"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Verbrennung"
          en: "Suppuration"
          fr: "Morsure Du Feu"
          ja: "熱傷"
          cn: "灼伤"
          ko: "열화상"
        title_id: "177"
        icon: "017000/017181_hr1.png"
        description:
          de: "Maximale LP sind verringert, erlittener Schaden ist erhöht."
          en: "Maximum HP is reduced and damage taken is increased."
          fr: "Les PV maximums sont réduits et les dégâts reçus sont augmentés."
          ja: "最大HPが低下し、受けるダメージが増加する状態。"
          cn: "最大体力降低，受到攻击的伤害增加"
          ko: "HP 최대치가 감소하고, 받는 피해량이 증가하는 상태."
        durations: [89]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Tursus"
      en: "Pagos Tursus"
      fr: "Tursus Pagos"
      ja: "パゴス・トゥルスス"
      cn: "恒冰图苏斯水龙蜥"
      ko: "파고스 투르수스"
    enemy_id: "7418"
    id: "add57"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE9"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Eisspender"
          en: "Ice Dispenser"
          fr: "Givrage"
          ja: "アイスディスペンサー"
          cn: "散冰"
          ko: "얼음 공급기"
        title_id: "2D7F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Eiszyklon"
          en: "Chilling Cyclone"
          fr: "Cyclone Glaçant"
          ja: "チリングサイクロン"
          cn: "寒颤旋风"
          ko: "싸늘한 폭풍"
        title_id: "2D80"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "231"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [6]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Wolf"
      en: "Pagos Wolf"
      fr: "Loup Pagos"
      ja: "パゴス・ウルフ"
      cn: "恒冰白狼"
      ko: "파고스 늑대"
    enemy_id: "7411"
    id: "add58"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Abyssaler Schrei"
          en: "Abyssal Cry"
          fr: "Cri Abyssal"
          ja: "アビサルクライ"
          cn: "深渊嚎"
          ko: "심연의 비명"
        title_id: "30D4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EE1"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F3A"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Mordshunger"
          en: "Ravenous Bite"
          fr: "Morsure Vorace"
          ja: "ラヴェナスバイト"
          cn: "极饿咬"
          ko: "탐욕스러운 입질"
        title_id: "2E47"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F4C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Vampirbiss"
          en: "Sanguine Bite"
          fr: "Morsure Vampirique"
          ja: "サングインバイト"
          cn: "嗜血咬"
          ko: "흡혈 물어뜯기"
        title_id: "2DD7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "634"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Pagos-Zentaur"
      en: "Pagos Centaur"
      fr: "Centaure Pagos"
      ja: "パゴス・セントール"
      cn: "恒冰半人马"
      ko: "파고스 켄타우로스"
    enemy_id: "7444"
    id: "add59"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F03"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Aufbäumen"
          en: "Rear"
          fr: "Ruade"
          ja: "リアー"
          cn: "怒踏"
          ko: "일어서기"
        title_id: "2E16"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 1447
          - max: 1447
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Berserker"
          en: "Berserk"
          fr: "Furie"
          ja: "ベルセルク"
          cn: "狂暴"
          ko: "광포"
        title_id: "2E15"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 35
            icon: "015000/015050.png"
            name:
               de: "Schadenswert +"
               en: "Physical Damage Up"
               fr: "Bonus de dégâts physiques"
               ja: "物理ダメージ上昇"
               cn: "物理伤害提高"
               ko: "주는 물리 피해량 증가"
        damage:
          - min: 53
          - max: 53
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Schadenswert +"
          en: "Physical Damage Up"
          fr: "Bonus De Dégâts Physiques"
          ja: "物理ダメージ上昇"
          cn: "物理伤害提高"
          ko: "주는 물리 피해량 증가"
        title_id: "35"
        icon: "015000/015050_hr1.png"
        description:
          de: "Ausgeteilter physischer Schaden ist erhöht."
          en: "Physical damage dealt is increased."
          fr: "Les dégâts physiques infligés sont augmentés."
          ja: "物理ダメージが上昇した状態。"
          cn: "物理攻击所造成的伤害提高"
          ko: "물리 공격으로 주는 피해량이 증가한 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Qiongji"
      en: "Voretooth"
      fr: "Qiongji"
      ja: "キュウキ"
      cn: "穷奇"
      ko: "궁기"
    enemy_id: "7533"
    id: "add60"
    hp:
      - min: 372665
      - max: 3062336
    attacks:
      - title:
          de: "Eisnebel"
          en: "Frozen Mist"
          fr: "Brise Glaçante"
          ja: "フローズンミスト"
          cn: "冻雾"
          ko: "서리 안개"
        title_id: "2E6B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: 1E7
            icon: "017000/017621.png"
            name:
               de: "Tiefkühlung"
               en: "Deep Freeze"
               fr: "Congélation"
               ja: "氷結"
               cn: "冻结"
               ko: "빙결"
        damage:
          - min: 111
          - max: 10816
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Schwere Schrottlinie"
          en: "Heavy Scrapline"
          fr: "Lariat Violent"
          ja: "ヘビーラリアット"
          cn: "蛮力金刚臂"
          ko: "막강한 후려갈기기"
        title_id: "2E6E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 12022
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Schwerer Wurf"
          en: "Heavy Toss"
          fr: "Lancer Violent"
          ja: "ヘビートス"
          cn: "蛮力上投"
          ko: "막강한 던지기"
        title_id: "2E6D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 8696
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2E69"
          en: "Unknown_2E69"
          fr: "Unknown_2E69"
          ja: "Unknown_2E69"
          cn: "Unknown_2E69"
          ko: "Unknown_2E69"
        title_id: "2E69"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
      - title:
          de: "Zottel-Randale"
          en: "Buffet"
          fr: "Frappe"
          ja: "ぶん殴る"
          cn: "殴打"
          ko: "쥐어박기"
        title_id: "2E6C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Tiefkühlung"
          en: "Deep Freeze"
          fr: "Congélation"
          ja: "氷結"
          cn: "冻结"
          ko: "빙결"
        title_id: "1E7"
        icon: "017000/017621_hr1.png"
        description:
          de: "In Eis eingeschlossen. Keine Handlungen möglich und erleidet schrittweise Schaden."
          en: "Your body is encased in ice, preventing action and dealing damage over time."
          fr: "Le corps est entièrement gelé. Des dégâts périodiques de glace sont subis."
          ja: "身体が凍り付き、行動できなくなった状態。ＨＰが徐々に失われる。"
          cn: "身体被冻结无法行动，体力逐渐流失"
          ko: "몸이 얼어붙어 행동이 불가능한 상태. HP가 서서히 줄어든다."
        durations: [6]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Riesen-Amphitere"
      en: "Greater Amphiptere"
      fr: "Grand Amphiptère"
      ja: "グレーター・アンフィプテレ"
      cn: "大安菲瑟龙"
      ko: "거대 암피프테레"
    enemy_id: "7457"
    id: "add61"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F10"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Verzerrtes Jammern"
          en: "Warped Wail"
          fr: "Gémissement Déformé"
          ja: "ワープドウェイル"
          cn: "扭曲的哭声"
          ko: "비틀어진 통곡"
        title_id: "2E3B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Wildes Picken"
          en: "Feral Peck"
          fr: "Picorage Sauvage"
          ja: "フィラルピック"
          cn: "野性连啄"
          ko: "치명적 쪼기"
        title_id: "2E39"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 7
          - max: 12673
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gemach"
          en: "Slow"
          fr: "Lenteur"
          ja: "スロウ"
          cn: "减速"
          ko: "둔화"
        title_id: "231"
        icon: "015000/015009_hr1.png"
        description:
          de: "Die Intervalle zwischen Auto-Attacken und die (Re-)Aktivierungszeiten von Waffenfertigkeiten und Magie sind verlängert."
          en: "Weaponskill cast time and recast time, spell cast time and recast time, and auto-attack delay are increased."
          fr: "La vitesse des attaques automatiques est réduite, et les temps de lancement et de recharge des sorts et des techniques d'arme sont allongés."
          ja: "オートアタックの攻撃間隔と、ウェポンスキルおよび魔法のキャストタイムとリキャストタイムが延長された状態。"
          cn: "自动攻击间隔延长，同时战技与魔法的咏唱及复唱时间也会延长"
          ko: "자동 공격 주기, 무기 기술 및 마법 시전 시간과 재사용 대기 시간이 늘어난 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Ritterkrabbe"
      en: "Knight Crab"
      fr: "Crabe Chevalier"
      ja: "ナイトクラブ"
      cn: "陆蟹骑士"
      ko: "기사게"
    enemy_id: "7509"
    id: "add62"
    hp:
      - min: 200650
      - max: 200650
    attacks:
      - title:
          de: "Aqua"
          en: "Water"
          fr: "Eau"
          ja: "ウォータ"
          cn: "流水"
          ko: "워터"
        title_id: "2D6B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 101
          - max: 1663
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2D61"
          en: "Unknown_2D61"
          fr: "Unknown_2D61"
          ja: "Unknown_2D61"
          cn: "Unknown_2D61"
          ko: "Unknown_2D61"
        title_id: "2D61"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 587
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Rotblitz-Matamata"
      en: "Emberflash Matamata"
      fr: "Matamata Ambréclat"
      ja: "エンバーフラッシュ・マタマタ"
      cn: "余光闪烁玛塔蛇颈龟"
      ko: "잉걸불 마타마타"
    enemy_id: "7455"
    id: "add63"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F0E"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 7641
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30B8"
            damage_type: "Blunt"
            damage:
              - min: 2453
              - max: 2453
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Balsamierender Atem"
          en: "Embalming Earth"
          fr: "Souffle Embaumant"
          ja: "エンバームアース"
          cn: "不朽大地"
          ko: "썩지 않는 땅"
        title_id: "2E35"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F55"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 62E
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 44288
          - max: 44288
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Siedender Atem"
          en: "Scalding Breath"
          fr: "Souffle Bouillant"
          ja: "スカールドブレス"
          cn: "滚烫吐息"
          ko: "뜨거운 숨결"
        title_id: "2E36"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Schnee-Aurelia"
      en: "Snow Aurelia"
      fr: "Aurélie Des Neiges"
      ja: "スノウ・オーレリア"
      cn: "雪地海月水母"
      ko: "설원 달해파리"
    enemy_id: "7407"
    id: "add64"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F70"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EDD"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 61810
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Schnee-Schnecke"
      en: "Snow Slug"
      fr: "Limace Des Neiges"
      ja: "スノウ・スラッグ"
      cn: "雪地蛞蝓"
      ko: "설원 민달팽이"
    enemy_id: "7404"
    id: "add65"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EDA"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Wässriges Exkret"
          en: "Aqueous Discharge"
          fr: "Décharge Aqueuse"
          ja: "液汁散布"
          cn: "释放汁液"
          ko: "액체 살포"
        title_id: "2DBC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Schneeball"
      en: "Yukidama"
      fr: "Yukidama"
      ja: "ユキダマ"
      cn: "雪球"
      ko: "눈덩이"
    enemy_id: "7534"
    id: "add66"
    hp:
      - min: 76961
      - max: 120618
    attacks:
      - title:
          de: "Schneeregen"
          en: "Flurry"
          fr: "Averse De Neige"
          ja: "雪時雨"
          cn: "雨夹雪"
          ko: "진눈깨비"
        title_id: "2E6A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2E68"
          en: "Unknown_2E68"
          fr: "Unknown_2E68"
          ja: "Unknown_2E68"
          cn: "Unknown_2E68"
          ko: "Unknown_2E68"
        title_id: "2E68"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Schneediener"
      en: "Servile Snowflake"
      fr: "Flocon Fidèle"
      ja: "雪の下僕"
      cn: "雪之仆从"
      ko: "눈의 심복"
    enemy_id: "7527"
    id: "add67"
    hp:
      - min: 116985
      - max: 128956
    attacks:
      - title:
          de: "Schneeregen"
          en: "Flurry"
          fr: "Averse De Neige"
          ja: "雪時雨"
          cn: "雨夹雪"
          ko: "진눈깨비"
        title_id: "2D2C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 4106
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2D2A"
          en: "Unknown_2D2A"
          fr: "Unknown_2D2A"
          ja: "Unknown_2D2A"
          cn: "Unknown_2D2A"
          ko: "Unknown_2D2A"
        title_id: "2D2A"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Schneeschmelzen-Exergon"
      en: "Snowmelt Sprite"
      fr: "Élémentaire De Neige Fondue"
      ja: "スノウメルトスプライト"
      cn: "融雪元精"
      ko: "해빙 정령"
    enemy_id: "7471"
    id: "add68"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Aqua"
          en: "Water"
          fr: "Eau"
          ja: "ウォータ"
          cn: "流水"
          ko: "워터"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2E59"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E5D"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E61"
            damage_type: "Magical"
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Schneesturm-Exergon"
      en: "Snowstorm Sprite"
      fr: "Élémentaire Des Tempêtes De Neige"
      ja: "スノウストームスプライト"
      cn: "雪暴元精"
      ko: "눈보라 정령"
    enemy_id: "7469"
    id: "add69"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F74"
            damage_type: "Slashing"
            add_status:
              - status: 62D
                icon: "015000/015884.png"
                name:
                 de: "Adaption"
                 en: "Adaptation"
                 fr: "Adaptation"
                 ja: "環境適応"
                 cn: "环境适应"
                 ko: "환경 적응"
            damage:
              - min: 38656
              - max: 38656
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2F81"
            damage_type: "Blunt"
            damage:
              - min: 38656
              - max: 38656
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Eis"
          en: "Blizzard"
          fr: "Glace"
          ja: "ブリザド"
          cn: "冰结"
          ko: "블리자드"
        attack_in_use: "false"
        disable: "false"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2E57"
            damage_type: "Magical"
            damage:
              - min: 37
              - max: 5021
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E5B"
            damage_type: "Magical"
            damage:
              - min: 37
              - max: 4747
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
          - title_id: "2E5F"
            damage_type: "Magical"
            damage:
              - min: 37
              - max: 60453
            roles:
              - role: "Variation-role 1"
            tags:
              - tag: "Variation-tag 1"
              - tag: "Single"
      - title:
          de: "Kollaps"
          en: "Scathe"
          fr: "Blessure"
          ja: "コラプス"
          cn: "崩溃"
          ko: "컬랩스"
        title_id: "339F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F5B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Ätherfunken"
          en: "Aetherial Spark"
          fr: "Étincelle Éthérée"
          ja: "エーテリックスパーク"
          cn: "以太火花"
          ko: "에테르 불꽃"
        title_id: "310E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Schneesturm-Goobbue"
      en: "Blizzard Goobbue"
      fr: "Goobbue Des Blizzards"
      ja: "ブリザード・グゥーブー"
      cn: "暴雪古菩猩猩"
      ko: "눈보라 구부"
    enemy_id: "7432"
    id: "add70"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF7"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Erkälteter Nieser"
          en: "Sickly Sneeze"
          fr: "Éternuement Maladif"
          ja: "シックリィスニーズ"
          cn: "患病喷嚏"
          ko: "역겨운 재채기"
        title_id: "2DFF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Prankenhieb"
          en: "Beatdown"
          fr: "Écrasement"
          ja: "ビートダウン"
          cn: "击倒"
          ko: "두들겨 부수기"
        title_id: "2E00"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Fessel"
          en: "Bind"
          fr: "Entrave"
          ja: "バインド"
          cn: "止步"
          ko: "속박"
        title_id: "0D"
        icon: "015000/015003_hr1.png"
        description:
          de: "Bewegungsunfähig."
          en: "Unable to move."
          fr: "Tout déplacement est impossible."
          ja: "移動できなくなった状態。"
          cn: "无法自由移动"
          ko: "이동이 불가능한 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Sensenmeister"
      en: "Grim Reaper"
      fr: "La Faucheuse"
      ja: "グリムリーパー"
      cn: "死亡收割者"
      ko: "음산한 난도자"
    enemy_id: "7496"
    id: "add71"
    hp:
      - min: 27801
      - max: 46698
    attacks:
      - title:
          de: "Meißelstoß"
          en: "Gusting Gouge"
          fr: "Gouge Cisaillante"
          ja: "ガスティンググージ"
          cn: "削风"
          ko: "칼날 돌풍"
        title_id: "2D79"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 105
          - max: 33683
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Sensenschwung"
          en: "Cross Reaper"
          fr: "Fauchaison"
          ja: "クロスリーパー"
          cn: "交错收割"
          ko: "사신의 낫"
        title_id: "2D71"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Sturm-Manta"
      en: "Storm Manta"
      fr: "Raie Des Tempêtes"
      ja: "ストームマンタ"
      cn: "风暴鳐"
      ko: "폭풍 쥐가오리"
    enemy_id: "7425"
    id: "add72"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF0"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Hauptströmung"
          en: "Prevailing Current"
          fr: "Courant Dominant"
          ja: "激流"
          cn: "激流"
          ko: "격류"
        title_id: "2DF2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Sylvestre"
      en: "Sylvestre"
      fr: "Mandragore Sylvestre"
      ja: "シルベストル"
      cn: "野树灵"
      ko: "실베스트르"
    enemy_id: "7424"
    id: "add73"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2EAD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EEF"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Aufschrei"
          en: "Heirloom Scream"
          fr: "Cri Légumier"
          ja: "絶叫"
          cn: "绝叫"
          ko: "절규"
        title_id: "2DED"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Kohlkopfstoß"
          en: "Ram"
          fr: "Coup De Chou"
          ja: "頭突き"
          cn: "撞击"
          ko: "머리 들이받기"
        title_id: "2DEC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 839
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Betäubung"
          en: "Stun"
          fr: "Étourdissement"
          ja: "スタン"
          cn: "眩晕"
          ko: "기절"
        title_id: "02"
        icon: "015000/015004_hr1.png"
        description:
          de: "Betäubte Sinne verhindern das Ausführen jeglicher Kommandos."
          en: "Unable to execute actions."
          fr: "Toute action est impossible."
          ja: "行動できなくなった状態。"
          cn: "无法做出任何行动"
          ko: "행동이 불가능한 상태."
        durations: [3]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Tal-Manticore"
      en: "Valley Manticore"
      fr: "Manticore Des Vallées"
      ja: "バレー・マンティコア"
      cn: "山谷曼提克"
      ko: "골짜기 만티코어"
    enemy_id: "7449"
    id: "add74"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F08"
            damage_type: "Slashing"
            damage:
              - min: 113
              - max: 61809
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30BD"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 527
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Dreischnitt"
          en: "Triclip"
          fr: "Triclip"
          ja: "トライクリップ"
          cn: "三连爪"
          ko: "삼단베기"
        title_id: "30FB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Fetzklaue"
          en: "Ripper Claw"
          fr: "Griffe Éventreuse"
          ja: "リッパークロウ"
          cn: "撕裂利爪"
          ko: "발톱 난도질"
        title_id: "2E2D"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 1602
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Feuerball"
          en: "Fireball"
          fr: "Boule De Feu"
          ja: "ファイアボール"
          cn: "火球"
          ko: "화염구"
        title_id: "2E2C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        add_status:
          - status: AC
            icon: "015000/015531.png"
            name:
               de: "Gebrechlichkeit"
               en: "Infirmity"
               fr: "Infirmité"
               ja: "虚弱"
               cn: "虚弱"
               ko: "허약"
        damage:
          - min: 3596
          - max: 3596
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F60"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Nichtsblut"
          en: "Voidblood"
          fr: "Sang Du Néant"
          ja: "ヴォイドブラッド"
          cn: "虚无之血"
          ko: "보이드의 피"
        title_id: "30FA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Gebrechlichkeit"
          en: "Infirmity"
          fr: "Infirmité"
          ja: "虚弱"
          cn: "虚弱"
          ko: "허약"
        title_id: "AC"
        icon: "015000/015531_hr1.png"
        description:
          de: "Wirkung von Heilmagie auf sich ist verringert. "
          en: "HP recovery via healing magic is reduced."
          fr: "L'effet des sorts de restauration des PV est réduit."
          ja: "回復魔法による自身に対するＨＰ回復効果が低下した状態。"
          cn: "自身所受的治疗魔法效果降低"
          ko: "자신이 받는 회복마법의 HP 회복 효과가 떨어진 상태."
        durations: [8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
      - title:
          de: "Nichtsblut"
          en: "Voidblood"
          fr: "Sang Du Néant"
          ja: "妖異の血"
          cn: "妖异之血"
          ko: "요마의 피"
        title_id: "310"
        icon: "015000/015675_hr1.png"
        description:
          de: "Mit dem Blut Nichtsgesandter verschmiert, das den erlittenen Schaden erhöht."
          en: "Covered in voidsent blood. Damage taken is increased."
          fr: "Les dégâts subis sont augmentés à cause du sang du néant qui recouvre le corps."
          ja: "妖異の血を浴びた状態。被ダメージが上昇する。"
          cn: "被妖异之血所浸湿，受到攻击的伤害增加"
          ko: "요마의 피를 뒤집어쓴 상태. 받는 피해량이 증가한다."
        durations: [30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Taxims Page"
      en: "Taxim's Thrall"
      fr: "Serviteur De Taxim"
      ja: "タキシムの奴隷"
      cn: "塔克西姆的奴隶"
      ko: "텍심의 노예"
    enemy_id: "7536"
    id: "add75"
    hp:
      - min: 124229
      - max: 175229
    attacks:
      - title:
          de: "Modrige Bandage"
          en: "Rotting Bandages"
          fr: "Bandages En Putréfaction"
          ja: "ロッティング・バンデージ"
          cn: "腐尸绷带"
          ko: "썩은 붕대"
        title_id: "2E8B"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2E8D"
          en: "Unknown_2E8D"
          fr: "Unknown_2E8D"
          ja: "Unknown_2E8D"
          cn: "Unknown_2E8D"
          ko: "Unknown_2E8D"
        title_id: "2E8D"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Toter Giftmischer"
      en: "Sinister Subject"
      fr: "Cadavre Funeste"
      ja: "毒手の屍人兵"
      cn: "毒手尸人兵"
      ko: "독술사 시체병"
    enemy_id: "7568"
    id: "add76"
    hp:
      - min: 210763
      - max: 357455
    attacks:
      - title:
          de: "Rostbiss"
          en: "Rustbite"
          fr: "Morsure Corrosive"
          ja: "ラスティバイト"
          cn: "锈咬"
          ko: "녹슨 화살"
        title_id: "2EA8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 12
            icon: "015000/015007.png"
            name:
               de: "Gift"
               en: "Poison"
               fr: "Poison"
               ja: "毒"
               cn: "中毒"
               ko: "독"
        damage:
          - min: 587
          - max: 11346
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Unknown_2E9B"
          en: "Unknown_2E9B"
          fr: "Unknown_2E9B"
          ja: "Unknown_2E9B"
          cn: "Unknown_2E9B"
          ko: "Unknown_2E9B"
        title_id: "2E9B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 61810
        phases:
          - phase: "09"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Toter Schütze"
      en: "Shrewd Subject"
      fr: "Cadavre Preste"
      ja: "速射の屍人兵"
      cn: "速射尸人兵"
      ko: "속사의 시체병"
    enemy_id: "7530"
    id: "add77"
    hp:
      - min: 210763
      - max: 357455
    attacks:
      - title:
          de: "Schmerzender Regen"
          en: "Rain Of Pain"
          fr: "Pluie De Douleur"
          ja: "レイン・オブ・ペイン"
          cn: "痛苦之雨"
          ko: "고통의 화살비"
        title_id: "2EA7"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 29271
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Unknown_2E9B"
          en: "Unknown_2E9B"
          fr: "Unknown_2E9B"
          ja: "Unknown_2E9B"
          cn: "Unknown_2E9B"
          ko: "Unknown_2E9B"
        title_id: "2E9B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 61810
        phases:
          - phase: "09"
    sequence:
      - phase: "09"
  - title:
      de: "Toter Werfer"
      en: "Stout Subject"
      fr: "Cadavre Redoutable"
      ja: "豪腕の屍人兵"
      cn: "强腕尸人兵"
      ko: "힘센 시체병"
    enemy_id: "7569"
    id: "add78"
    hp:
      - min: 210763
      - max: 357455
    attacks:
      - title:
          de: "Unknown_2E9B"
          en: "Unknown_2E9B"
          fr: "Unknown_2E9B"
          ja: "Unknown_2E9B"
          cn: "Unknown_2E9B"
          ko: "Unknown_2E9B"
        title_id: "2E9B"
        attack_in_use: "false"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 7
          - max: 61810
        phases:
          - phase: "09"
      - title:
          de: "Wütende Attacke"
          en: "Raging Strikes"
          fr: "Tir Furieux"
          ja: "猛者の撃"
          cn: "猛者强击"
          ko: "용맹한 사격"
        title_id: "2EA6"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Wütende Attacke"
          en: "Raging Strikes"
          fr: "Tir Furieux"
          ja: "猛者の撃"
          cn: "猛者强击"
          ko: "용맹한 사격"
        title_id: "7D"
        icon: "010000/010354_hr1.png"
        description:
          de: "Ausgeteilter Schaden ist erhöht."
          en: "Damage dealt is increased."
          fr: "Les dégâts infligés sont augmentés."
          ja: "与ダメージが向上した状態。"
          cn: "攻击造成的伤害提高"
          ko: "적에게 주는 피해량이 증가하는 상태."
        durations: [30]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Tranauge"
      en: "Blubber Eyes"
      fr: "Onctueux Oculaire"
      ja: "ブラバーアイズ"
      cn: "啜泣百目妖"
      ko: "흐느끼는 눈"
    enemy_id: "7429"
    id: "add79"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF4"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Hex-Auge"
          en: "Hex Eye"
          fr: "Œil Néfaste"
          ja: "凶眼"
          cn: "凶眼"
          ko: "흉안"
        title_id: "2DFA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        add_status:
          - status: 11
            icon: "015000/015006.png"
            name:
               de: "Paralyse"
               en: "Paralysis"
               fr: "Paralysie"
               ja: "麻痺"
               cn: "麻痹"
               ko: "마비"
        damage:
          - min: 474
          - max: 19819
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Todesstrahl"
          en: "Death Ray"
          fr: "Rayon De La Mort"
          ja: "デスレイ"
          cn: "死亡射线"
          ko: "죽음의 광선"
        title_id: "2DF9"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 117
          - max: 1038
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Paralyse"
          en: "Paralysis"
          fr: "Paralysie"
          ja: "麻痺"
          cn: "麻痹"
          ko: "마비"
        title_id: "11"
        icon: "015000/015006_hr1.png"
        description:
          de: "Paralysierte Nerven lassen Kommandos ab und zu aussetzen."
          en: "Deadened nerves are sometimes preventing the execution of actions."
          fr: "Une paralysie empêche parfois d'effectuer des actions."
          ja: "身体が痺れ、時々行動できなくなる状態。"
          cn: "身体部分机能出现异常，偶尔会无法行动"
          ko: "몸이 마비되어 잠깐씩 행동할 수 없는 상태."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Tödlicher Dodo"
      en: "Deadly Dodo"
      fr: "Dodo Meurtrier"
      ja: "デッドリー・ドードー"
      cn: "致命渡渡鸟"
      ko: "치명적인 도도"
    enemy_id: "7406"
    id: "add80"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EDC"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Picken"
          en: "Peck"
          fr: "Picorage"
          ja: "つっつき"
          cn: "猛啄"
          ko: "쪼기"
        title_id: "2DC2"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Ameise"
      en: "Val Ant"
      fr: "Fourmi De La Val"
      ja: "バル・アント"
      cn: "瓦尔蚁"
      ko: "발 개미"
    enemy_id: "7414"
    id: "add81"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE4"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "None"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Fangkiefer"
          en: "Trap Jaws"
          fr: "Mâchoires Piégeantes"
          ja: "トラップジョーズ"
          cn: "捕猎钳"
          ko: "날카로운 턱"
        title_id: "2DDB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 2106
          - max: 2494
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Säure"
          en: "Acid Spray"
          fr: "Jet D'Acide"
          ja: "アシッドスプレー"
          cn: "酸液喷射"
          ko: "산성 분무"
        title_id: "2DDA"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Galago"
      en: "Val Galago"
      fr: "Galago De La Val"
      ja: "バル・ガラゴ"
      cn: "瓦尔婴猴"
      ko: "발 갈라고"
    enemy_id: "7417"
    id: "add82"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EB7"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2EE8"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Deftige Dachtel"
          en: "Browbeat"
          fr: "Torgnole"
          ja: "殴打"
          cn: "殴打"
          ko: "구타"
        title_id: "2EAE"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2EB5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Steinschleudern"
          en: "Stool Pelt"
          fr: "Catapultage"
          ja: "岩石投げ"
          cn: "扔岩石"
          ko: "암석 던지기"
        title_id: "2EAF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Steinwurf"
          en: "Stone Throw"
          fr: "Lapidation"
          ja: "つぶて打ち"
          cn: "投石"
          ko: "돌 던지기"
        title_id: "2DE1"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 744
          - max: 744
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "634"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Greif"
      en: "Val Griffin"
      fr: "Griffon De La Val"
      ja: "バル・グリフィン"
      cn: "瓦尔狮鹫"
      ko: "발 그리핀"
    enemy_id: "7463"
    id: "add83"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Atem Des Löwen"
          en: "The Lion's Breath"
          fr: "Souffle Du Lion"
          ja: "フレイムブレス"
          cn: "火焰吐息"
          ko: "화염 숨결"
        title_id: "3108"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F16"
            damage_type: "Slashing"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "30C4"
            damage_type: "Blunt"
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Gletscherluft"
          en: "Alpine Draft"
          fr: "Vent Alpin"
          ja: "アルペンドラフト"
          cn: "高山气流"
          ko: "고산 기류"
        title_id: "2E46"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Goldene Klauen"
          en: "Golden Talons"
          fr: "Serres Dorées"
          ja: "ゴールデンタロン"
          cn: "黄金爪"
          ko: "황금 발톱"
        title_id: "2E45"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Hüter"
      en: "Val Guardian"
      fr: "Gardien De La Val"
      ja: "バル・ガーディアン"
      cn: "瓦尔守护者"
      ko: "발 수호자"
    enemy_id: "7435"
    id: "add84"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2EFA"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 29794
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F2C"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 29794
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Felsschädel"
          en: "Stone Skull"
          fr: "Crâne De Pierre"
          ja: "ストーンスカル"
          cn: "岩石骷髅"
          ko: "돌머리"
        title_id: "2E05"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 9220
          - max: 9220
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F59"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 634
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 30976
          - max: 30976
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "634"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Leiche"
      en: "Val Corpse"
      fr: "Cadavre De La Val"
      ja: "バル・コープス"
      cn: "瓦尔腐尸"
      ko: "발 송장"
    enemy_id: "7468"
    id: "add85"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F1B"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Piercing"
        damage:
          - min: 114
          - max: 61810
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Rostbiss"
          en: "Rustbite"
          fr: "Morsure Corrosive"
          ja: "ラスティバイト"
          cn: "锈咬"
          ko: "녹슨 화살"
        title_id: "2E87"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 12
            icon: "015000/015007.png"
            name:
               de: "Gift"
               en: "Poison"
               fr: "Poison"
               ja: "毒"
               cn: "中毒"
               ko: "독"
        damage:
          - min: 2536
          - max: 9802
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Schmerzender Regen"
          en: "Rain Of Pain"
          fr: "Pluie De Douleur"
          ja: "レイン・オブ・ペイン"
          cn: "痛苦之雨"
          ko: "고통의 화살비"
        title_id: "2E86"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Mantis"
      en: "Val Mantis"
      fr: "Mante De La Val"
      ja: "バル・マンティス"
      cn: "瓦尔螳螂"
      ko: "발 사마귀"
    enemy_id: "7416"
    id: "add86"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F72"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 38144
          - max: 38144
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE7"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Erhobene Schere"
          en: "Standing Chine"
          fr: "Échine Verticale"
          ja: "スタンディングチャイン"
          cn: "站裂斩"
          ko: "세로베기"
        title_id: "2DDF"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Lauerndes Fangbein"
          en: "Kneeling Snath"
          fr: "Faucille Agenouillée"
          ja: "ニーリングシックル"
          cn: "俯跪冲"
          ko: "거대 낫질"
        title_id: "2DE0"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Mull"
      en: "Val Mole"
      fr: "Taupe De La Val"
      ja: "バル・モール"
      cn: "瓦尔鼹鼠"
      ko: "발 두더지"
    enemy_id: "7408"
    id: "add87"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EDE"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 61
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Erdmantel"
          en: "Soil Coat"
          fr: "Pelage De Terre"
          ja: "ソイルコート"
          cn: "泥土外衣"
          ko: "흙투성이"
        title_id: "2E63"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        add_status:
          - status: 3F
            icon: "015000/015021.png"
            name:
               de: "Verringerte Verwundbarkeit"
               en: "Vulnerability Down"
               fr: "Vulnérabilité diminuée"
               ja: "被ダメージ低下"
               cn: "受伤减轻"
               ko: "받는 피해량 감소"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Verringerte Verwundbarkeit"
          en: "Vulnerability Down"
          fr: "Vulnérabilité Diminuée"
          ja: "被ダメージ低下"
          cn: "受伤减轻"
          ko: "받는 피해량 감소"
        title_id: "3F"
        icon: "015000/015021_hr1.png"
        description:
          de: "Erlittener Schaden ist verringert."
          en: "Damage taken is reduced."
          fr: "Les dégâts reçus sont réduits."
          ja: "受けるダメージが軽減される状態。"
          cn: "受到攻击的伤害减少"
          ko: "받는 피해량이 감소하는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Rösling"
      en: "Val Roselet"
      fr: "Roselette De La Val"
      ja: "バル・ローズレット"
      cn: "瓦尔小红花"
      ko: "발 넝쿨장미"
    enemy_id: "7415"
    id: "add88"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE5"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Saatsalve"
          en: "Seedvolley"
          fr: "Volée De Graines"
          ja: "シードボレー"
          cn: "种弹齐射"
          ko: "씨앗 투척"
        title_id: "2DDC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Schnittiges Sausen"
          en: "Swift Sough"
          fr: "Murmure Rapide"
          ja: "スウィフトサフ"
          cn: "飒飒速弹"
          ko: "거친 바람"
        title_id: "2DDD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Schnippler"
      en: "Val Snipper"
      fr: "Cisailleur De La Val"
      ja: "バル・スニッパー"
      cn: "瓦尔利螯陆蟹"
      ko: "발 싹둑게"
    enemy_id: "7450"
    id: "add89"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F09"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Blubberblasen"
          en: "Bubble Shower"
          fr: "Averse De Bulles"
          ja: "バブルシャワー"
          cn: "泡泡吐息"
          ko: "거품 세례"
        title_id: "2E2E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 101
          - max: 4505
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Terrormilbe"
      en: "Val Banemite"
      fr: "Sarcopte De La Val"
      ja: "バル・ベーンマイト"
      cn: "瓦尔祸蛛蝎"
      ko: "발 독거미전갈"
    enemy_id: "7409"
    id: "add90"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EBC"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Erderschütterer"
          en: "Realm Shaker"
          fr: "Secousse Tellurique"
          ja: "レルムシェーカー"
          cn: "震撼领域"
          ko: "파멸 영역"
        title_id: "2DD5"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Tödliche Durchbohrung"
          en: "Deadly Thrust"
          fr: "Transpercement Mortel"
          ja: "デッドリースラスト"
          cn: "致命尾刺"
          ko: "치명적 일격"
        title_id: "2DD4"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Titanenwurz"
      en: "Val Corpse Flower"
      fr: "Fleur-Dépouille De La Val"
      ja: "バル・コープスフラワー"
      cn: "瓦尔尸生花"
      ko: "발 시체꽃"
    enemy_id: "7456"
    id: "add91"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Abstoßender Odem"
          en: "Odious Air"
          fr: "Pestilence"
          ja: "オディアスエアー"
          cn: "恶意毒气"
          ko: "끔찍한 공기"
        title_id: "311A"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F0F"
            damage_type: "Slashing"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F34"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F61"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Säurenebel"
          en: "Acid Mist"
          fr: "Brume Acide"
          ja: "アシッドミスト"
          cn: "酸雾"
          ko: "산성 안개"
        title_id: "2E38"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Tiefe Verwurzelung"
          en: "Deeproot"
          fr: "Racines Profondes"
          ja: "ディープルート"
          cn: "根系深埋"
          ko: "깊은뿌리"
        title_id: "3119"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Vampirranke"
          en: "Bloody Caress"
          fr: "Caresse Sanglante"
          ja: "ブラッディカレス"
          cn: "血腥的爱抚"
          ko: "피의 애무"
        title_id: "2E37"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 1835
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Wurm"
      en: "Val Worm"
      fr: "Ver De La Val"
      ja: "バル・ウォーム"
      cn: "瓦尔巨虫"
      ko: "발 지렁이"
    enemy_id: "7453"
    id: "add92"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F0C"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Sandatem"
          en: "Sand Breath"
          fr: "Souffle De Sable"
          ja: "サンドブレス"
          cn: "黄沙吐息"
          ko: "모래 숨결"
        title_id: "2E33"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Darkness"
        damage:
          - min: 148
          - max: 148
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Blind"
          en: "Blind"
          fr: "Cécité"
          ja: "暗闇"
          cn: "失明"
          ko: "실명"
        title_id: "0F"
        icon: "015000/015012_hr1.png"
        description:
          de: "Vorübergehende Blindheit senkt die Präzision."
          en: "Encroaching darkness is lowering accuracy."
          fr: "Un voile d'obscurité diminue la précision."
          ja: "闇に囚われ、命中率が低下した状態。"
          cn: "陷于黑暗之中，命中率降低"
          ko: "어둠에 사로잡혀 명중률이 떨어진 상태."
        durations: [4, 8]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Wächter"
      en: "Val Keeper"
      fr: "Protecteur De La Val"
      ja: "バル・キーパー"
      cn: "瓦尔守卫"
      ko: "발 지킴이"
    enemy_id: "7461"
    id: "add93"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adaptierung"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "适应环境"
          ko: "환경 적응"
        title_id: "2F79"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Piercing"
        add_status:
          - status: 62D
            icon: "015000/015884.png"
            name:
               de: "Adaption"
               en: "Adaptation"
               fr: "Adaptation"
               ja: "環境適応"
               cn: "环境适应"
               ko: "환경 적응"
        damage:
          - min: 39936
          - max: 39936
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F14"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Carpomission"
          en: "Carpomission"
          fr: "Carpomission"
          ja: "カルポミッション"
          cn: "石腕飞拳"
          ko: "맹속 타격"
        title_id: "2E83"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 1457
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Inselversenker"
          en: "Isle Drop"
          fr: "Île Plongeante"
          ja: "アイルドロップ"
          cn: "沉岛"
          ko: "섬 떨어뜨리기"
        title_id: "2E84"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Adaption"
          en: "Adaptation"
          fr: "Adaptation"
          ja: "環境適応"
          cn: "环境适应"
          ko: "환경 적응"
        title_id: "62D"
        icon: "015000/015884_hr1.png"
        description:
          de: "Angepasst an eine ätherisch instabile Umwelt."
          en: "Adapted to an aetherially unstable environment."
          fr: "Le corps s'est adapté à l'éther instable de l'environnement."
          ja: "属性エーテルが乱れた環境に適応した状態。"
          cn: "适应了属性以太紊乱的环境"
          ko: "속성 에테르가 흐트러진 환경에 적응한 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Val-Ymir"
      en: "Val Ymir"
      fr: "Bulot De La Val"
      ja: "バル・ユミール"
      cn: "瓦尔尤弥尔"
      ko: "발 위미르"
    enemy_id: "7458"
    id: "add94"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F11"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F39"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Blitz"
          en: "Thunder"
          fr: "Foudre"
          ja: "サンダー"
          cn: "闪雷"
          ko: "선더"
        title_id: "3128"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 4955
          - max: 36618
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Fühlerstoß"
          en: "Head Snatch"
          fr: "Arrache-Tête"
          ja: "ヘッドスナッチ"
          cn: "首角抢攻"
          ko: "머리 휘두르기"
        title_id: "2E3C"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 26237
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F66"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        durations: [9999]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Wandernder Opken"
      en: "Wandering Opken"
      fr: "Opken Errant"
      ja: "ワンダリング・オプケン"
      cn: "徘徊欧浦肯"
      ko: "떠도는 오프켄"
    enemy_id: "7431"
    id: "add95"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF6"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 7
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Gerade"
          en: "Straight Punch"
          fr: "Direct"
          ja: "ボーンブレイカー"
          cn: "碎骨拳"
          ko: "뼈대 꺾기"
        title_id: "2DFD"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Wuchtiges Stampfen"
          en: "Plain Pound"
          fr: "Martèlement Violent"
          ja: "プレインパウンド"
          cn: "平地捶打"
          ko: "평지 내리치기"
        title_id: "2DFE"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 675
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    sequence:
      - phase: "09"
  - title:
      de: "Weltenschlange"
      en: "World Serpent"
      fr: "Serpent-Monde"
      ja: "ワールドサーペント"
      cn: "尘世巨蟒"
      ko: "세계뱀"
    enemy_id: "7451"
    id: "add96"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F0A"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Auswürgen"
          en: "Regorge"
          fr: "Vomissure"
          ja: "リゴージ"
          cn: "喷毒"
          ko: "게워내기"
        title_id: "2E2F"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    debuffs:
      - title:
          de: "Gift"
          en: "Poison"
          fr: "Poison"
          ja: "毒"
          cn: "中毒"
          ko: "독"
        title_id: "12"
        icon: "015000/015007_hr1.png"
        description:
          de: "Erleidet schrittweise Schaden durch Gift."
          en: "Toxins are causing damage over time."
          fr: "Des dégâts périodiques de poison sont subis."
          ja: "毒に冒された状態。ＨＰが徐々に失われる。"
          cn: "毒素会导致体力逐渐减少"
          ko: "독에 중독된 상태. HP가 서서히 줄어든다."
        debuff_in_use: "true"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "Alle"
        tags:
          - tag: "Common"
    sequence:
      - phase: "09"
  - title:
      de: "Wilder Ruszor"
      en: "Savage Ruszor"
      fr: "Ruszor Féroce"
      ja: "サベッジ・ルスゾル"
      cn: "凶残鲁斯索尔"
      ko: "야생 루스조르"
    enemy_id: "7443"
    id: "add97"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F02"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 113
          - max: 61809
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Messerzahn"
          en: "Severing Fang"
          fr: "Croc Hachoir"
          ja: "包丁牙"
          cn: "菜刀牙"
          ko: "식칼 엄니"
        title_id: "2E13"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        damage:
          - min: 3797
          - max: 3797
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Wasserstoß"
          en: "Aqua Blast"
          fr: "Aqua Souffle"
          ja: "水撃"
          cn: "水击"
          ko: "물 공격"
        title_id: "2E14"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Wassersucht"
          en: "Dropsy"
          fr: "Œdème"
          ja: "水毒"
          cn: "水毒"
          ko: "물독"
        title_id: "110"
        icon: "015000/015529_hr1.png"
        description:
          de: "Erleidet schrittweise Wasserschaden."
          en: "Sustaining water damage over time."
          fr: "Des dégâts périodiques d'eau sont subis."
          ja: "水属性ダメージにより、ＨＰが徐々に失われる状態。"
          cn: "水属性持续伤害，体力逐渐流失"
          ko: "물속성 피해로 HP가 서서히 줄어드는 상태."
        durations: [15]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Wächter-Urolith"
      en: "Urolith Guard"
      fr: "Urolithe Gardien"
      ja: "ガード・ウロリス"
      cn: "乌洛里石守卫"
      ko: "경비 요결석"
    enemy_id: "7446"
    id: "add98"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        attack_in_use: "true"
        disable: "true"
        type: "variation"
        phases:
          - phase: "09"
        variation:
          - title_id: "2F05"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
          - title_id: "2F31"
            damage_type: "Blunt"
            damage:
              - min: 115
              - max: 61811
            roles:
              - role: "Tank"
            tags:
              - tag: "Auto-Angriff"
              - tag: "Single"
      - title:
          de: "Doppelstrahl"
          en: "Double Ray"
          fr: "Double Rayon"
          ja: "ダブルレイ"
          cn: "双重射线"
          ko: "이중 광선"
        title_id: "3114"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "None"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Dunkel-Eisga"
          en: "Dark Blizzard III"
          fr: "Méga Glace Ténébreuse"
          ja: "ダークブリザガ"
          cn: "黑暗冰封"
          ko: "다크 블리자가"
        title_id: "2E28"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        damage:
          - min: 37
          - max: 565
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "2F5E"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Slashing"
        add_status:
          - status: 62E
            icon: "015000/015885.png"
            name:
               de: "Mutation"
               en: "Mutation"
               fr: "Mutation"
               ja: "突然変異"
               cn: "突然变异"
               ko: "돌연변이"
        damage:
          - min: 32256
          - max: 32256
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Mutation"
          en: "Mutation"
          fr: "Mutation"
          ja: "突然変異"
          cn: "突然变异"
          ko: "돌연변이"
        title_id: "62E"
        icon: "015000/015885_hr1.png"
        description:
          de: "Eine ätherisch instabile Umwelt ruft Mutationen hervor."
          en: "Aetherial instability is triggering mutations."
          fr: "L'éther instable de l'environnement a entraîné une soudaine mutation."
          ja: "属性エーテルが乱れた環境の影響で、突然変異を起こした状態。"
          cn: "受到属性以太紊乱的影响，突然发生变异"
          ko: "속성 에테르가 흐트러진 환경의 영향을 받아 돌연변이를 일으킨 상태."
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Zauber-Sankchinni"
      en: "Withered Sankchinni"
      fr: "Sankchinni Rachitique"
      ja: "ウィザード・サンクチンニ"
      cn: "凋零山克芹尼"
      ko: "시든 상크친니"
    enemy_id: "7430"
    id: "add99"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Adventiv-Peitsche"
          en: "Adventitious Lash"
          fr: "Flagellation Fortuite"
          ja: "アドベントラッシュ"
          cn: "不定鞭打"
          ko: "우발적 가격"
        title_id: "2DFB"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Anemochorie"
          en: "Anemochory"
          fr: "Ébarbement Balistique"
          ja: "バリスティックバリ"
          cn: "刺边"
          ko: "탄도 드릴"
        title_id: "2DFC"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EF5"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 9166
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
    debuffs:
      - title:
          de: "Erhöhte Verwundbarkeit"
          en: "Vulnerability Up"
          fr: "Vulnérabilité Augmentée"
          ja: "被ダメージ上昇"
          cn: "受伤加重"
          ko: "받는 피해량 증가"
        title_id: "37D"
        icon: "017000/017101_hr1.png"
        description:
          de: "Erlittener Schaden ist erhöht."
          en: "Damage taken is increased."
          fr: "Les dégâts reçus sont augmentés."
          ja: "受けるダメージが増加する状態。"
          cn: "受到攻击的伤害增加"
          ko: "받는 피해량이 증가하는 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
  - title:
      de: "Zombie-Brobinyak"
      en: "Zombie Brobinyak"
      fr: "Brobinyak Zombi"
      ja: "ゾンビー・ブロビニャク"
      cn: "僵尸布罗宾雅克"
      ko: "좀비 브로비냐크"
    enemy_id: "7464"
    id: "add100"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2F17"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Slashing"
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Höllenschwinge"
          en: "The Serpent's Apple"
          fr: "Lacération Sauvage"
          ja: "サーペントアップル"
          cn: "蛇果"
          ko: "유혹의 사과"
        title_id: "2E74"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Blunt"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "AoE"
    sequence:
      - phase: "09"
  - title:
      de: "Zäher Yarzon"
      en: "Yarzon Survivor"
      fr: "Yarzon Miraculé"
      ja: "ヤーゾン・サバイバー"
      cn: "幸存盲头蟹"
      ko: "살아남은 긴다리거미"
    enemy_id: "7412"
    id: "add101"
    hp:
      - min: 133008
      - max: 133008
    attacks:
      - title:
          de: "Attacke"
          en: "Attack"
          fr: "Attaque"
          ja: "攻撃"
          cn: "攻击"
          ko: "공격"
        title_id: "2EE2"
        attack_in_use: "true"
        disable: "true"
        type: "regular"
        damage_type: "Blunt"
        damage:
          - min: 115
          - max: 61811
        phases:
          - phase: "09"
        roles:
          - role: "Tank"
        tags:
          - tag: "Auto-Angriff"
          - tag: "Single"
      - title:
          de: "Giftspucke"
          en: "Corrosive Spit"
          fr: "Crachat Corrosif"
          ja: "コロシヴスピット"
          cn: "溶解液"
          ko: "부식성 침"
        title_id: "2DD8"
        attack_in_use: "false"
        disable: "false"
        type: "regular"
        damage_type: "Magical"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
          - tag: "Single"
    debuffs:
      - title:
          de: "Erhöhte Physische Verwundbarkeit"
          en: "Physical Vulnerability Up"
          fr: "Vulnérabilité Physique Augmentée"
          ja: "被物理ダメージ増加"
          cn: "物理受伤加重"
          ko: "받는 물리 피해량 증가"
        title_id: "38"
        icon: "015000/015053_hr1.png"
        description:
          de: "Erlittener physischer Schaden ist erhöht."
          en: "Physical damage taken is increased."
          fr: "Les dégâts physiques reçus sont augmentés."
          ja: "受ける物理ダメージが増加する状態。"
          cn: "受到物理攻击的伤害增加"
          ko: "물리 공격으로 받는 피해량이 증가하는 상태."
        durations: [20]
        debuff_in_use: "false"
        disable: "false"
        phases:
          - phase: "09"
        roles:
          - role: "role"
        tags:
          - tag: "tag"
        notes:
          - note: "note"
    sequence:
      - phase: "09"
---
